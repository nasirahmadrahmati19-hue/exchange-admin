...
export default function DashboardPage() {
  // ✅ مرحله ۱: تمام هوک‌ها باید در بالاترین سطح و قبل از هرگونه return شرطی باشند
  ...
  // ✅ مرحله ۲: مدیریت ریدایرکت در useEffect (جلوگیری از پرش ناگهانی)
  useEffect(() => {
    if (!loading) {
      if (!user) {
  ...
  // ✅ مرحله ۳: خواندن تم بدون ایجاد Hydration Mismatch شدید
  useEffect(() => {
    try {
  ...
  // ✅ مرحله ۵: شرط رندر محافظ (Guard Clause) باید درست قبل از return اصلی باشد
  ...
  // ✅ مرحله ۶: رندر اصلی JSX (فقط وقتی کاربر لاگین است و ایمیلش صحیح است اجرا می‌شود)
  return (
    <div dir="rtl" className={dk ? "dark" : ""} suppressHydrationWarning>
