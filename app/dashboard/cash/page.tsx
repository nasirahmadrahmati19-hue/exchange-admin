...
const entryTypeLabels: Record<CashEntryType, string> = { customer_deposit: "واریز مشتری", customer_withdraw: "برداشت مشتری", owner_deposit: "واریز مالک به صرافی", owner_withdraw: "برداشت مالک از صرافی", adjustment: "اصلاح صندوق", fee: "کارمزد", commission_withdraw: "برداشت کارمزد", loan_given: "پرداخت قرض", loan_received: "دریافت قرض", exchange_account_in: "ورودی حساب صرافی", exchange_account_out: "خروجی حساب صرافی" };
const entryTypeColors: Record<CashEntryType, string> = { customer_deposit: "bg-teal-100 text-teal-700", customer_withdraw: "bg-orange-100 text-orange-700", owner_deposit: "bg-sky-100 text-sky-700", owner_withdraw: "bg-amber-100 text-amber-700", adjustment: "bg-violet-100 text-violet-700", fee: "bg-emerald-100 text-emerald-700", commission_withdraw: "bg-purple-100 text-purple-700", loan_given: "bg-rose-100 text-rose-700", loan_received: "bg-indigo-100 text-indigo-700", exchange_account_in: "bg-violet-100 text-violet-700", exchange_account_out: "bg-fuchsia-100 text-fuchsia-700" };
...
export default function CashPage() {
  ...
  // Save data
  useEffect(() => { if (!mounted) return; try { localStorage.setItem(CASH_KEY, JSON.stringify(entries)); window.dispatchEvent(new Event("db:updated")); } catch {} }, [entries, mounted]);
  ...
  // ✅ اصلاح حیاتی: خواندن مستقیم از c.balances (منبع واحد حقیقت)
  ...
  // ✅ اصلاح حیاتی: خواندن مستقیم موجودی حساب صرافی از customers
  const exchangeBalance = useMemo(() => {
    const exchCustomer = customers.find(c => c.id === EXCHANGE_ACCOUNT_ID);
    return exchCustomer ? exchCustomer.balances : { AFN: 0, USD: 0, EUR: 0, IRR: 0, PKR: 0 };
  }, [customers]);
  ...
  const validateForm = useCallback(() => {
    ...
    if (form.type === "customer_withdraw" && form.customerId && form.customerId !== CASH_BOX_ID && form.customerId !== EXCHANGE_ACCOUNT_ID) {
      ...
      const currentBal = c ? (c.balances[form.currency] || 0) : 0;
  ...
  const deleteEntry = useCallback((entry: CashEntry) => {
    ...
    if (entry.status !== "voided") setCustomers(prev => applyBalanceChanges(prev, getBalanceChangesForCashEntry(entry, "reverse")));
  ...
  const roInput = "cursor-default bg-slate-100 text-slate-500";
    ...
    <div dir="rtl">
      ...
      <div className="cs-font relative min-h-screen overflow-x-hidden antialiased transition-colors duration-500 bg-gradient-to-br from-emerald-50 via-teal-50 to-cyan-50 text-slate-800">
        ...
        <div className="relative z-10 mx-auto w-full max-w-7xl space-y-4 md:space-y-6 px-3 pb-16 pt-5 md:px-8 md:pt-9">
          ...
          <div className="cs-up space-y-4 md:space-y-5" style={{ animationDelay: "70ms" }}>
            {/* Physical Cash */}
            <div className="relative overflow-hidden rounded-xl md:rounded-2xl border-2 border-emerald-300 bg-gradient-to-br from-emerald-50 via-white to-teal-50 p-3 md:p-4 transition-all duration-300 hover:shadow-xl shadow-[0_10px_30px_-10px_rgba(16,185,129,0.25)]">
              <div className="relative flex items-center gap-3 mb-3">
                ...
                <div className="flex-1 min-w-0">
                  <b className="block text-sm md:text-base font-black text-emerald-700">💰 موجودی فیزیکی صندوق</b>
                  <span className="block text-[10px] md:text-[11px] font-bold mt-0.5 text-slate-500">مجموع خالص تمام ورودی‌ها و خروجی‌ها (شامل قرض)</span>
                </div>
              </div>
              <div className="relative grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-2 md:gap-3">
                {currencies.map(cur => {
                  ...
                  const isNeg = bal < 0;
                  return (
                    <div key={cur} className="group relative overflow-hidden rounded-xl bg-white/90 ring-1 ring-emerald-100 p-2.5 md:p-3 text-center transition-all duration-300 shadow-sm">
                      <div className="text-[11px] md:text-[12px] font-black mb-1 text-slate-500">{labels[cur]}</div>
                      <div className={`text-lg md:text-xl font-black tabular-nums leading-tight ${isNeg ? "text-rose-500" : "text-emerald-700"}`}>{fmt(bal)}</div>
                      <div className={`mt-1 text-[8px] md:text-[9px] font-black ${isNeg ? "text-rose-500" : "text-emerald-600/70"}`}>{isNeg ? "⚠️ کسری" : "✅ نقدی"}</div>
                    </div>
                  );
                })}
              </div>
            ...
            <div className="relative overflow-hidden rounded-xl md:rounded-2xl border-2 border-violet-300 bg-gradient-to-br from-violet-50 via-white to-indigo-50 p-3 md:p-4 transition-all duration-300 hover:shadow-xl">
              <div className="relative flex items-center gap-3 mb-3">
                ...
                <div className="flex-1 min-w-0">
                  <b className="block text-sm md:text-base font-black text-violet-700">💼 موجودی حساب صرافی</b>
                  <span className="block text-[10px] md:text-[11px] font-bold mt-0.5 text-slate-500">سرمایه صرافی (واریز/برداشت مالک + قرض)</span>
                </div>
              </div>
              <div className="relative grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-2 md:gap-3">
                {currencies.map(cur => {
                  ...
                  const isNeg = bal < 0;
                  return (
                    <div key={cur} className="group relative overflow-hidden rounded-xl bg-white/90 ring-1 ring-violet-100 p-2.5 md:p-3 text-center transition-all duration-300 shadow-sm">
                      <div className="text-[11px] md:text-[12px] font-black mb-1 text-slate-500">{labels[cur]}</div>
                      <div className={`text-lg md:text-xl font-black tabular-nums leading-tight ${isNeg ? "text-rose-500" : "text-violet-700"}`}>{fmt(bal)}</div>
                      <div className={`mt-1 text-[8px] md:text-[9px] font-black ${isNeg ? "text-rose-500" : "text-violet-600/70"}`}>{isNeg ? "⚠️ منفی" : "✅ مثبت"}</div>
                    </div>
                  );
                })}
              </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 md:gap-4">
              {/* Customer Deposits */}
              <div className="group relative overflow-hidden rounded-2xl border border-sky-200 bg-gradient-to-br from-sky-50 to-white p-4 md:p-5 transition-all duration-300 hover:shadow-xl hover:scale-[1.01]">
                <div className="relative flex items-center gap-3 mb-4">
                  <span className="grid h-11 w-11 shrink-0 place-items-center rounded-xl bg-sky-100 text-sky-600"><Ic n="user" className="h-5 w-5" /></span>
                  <div className="min-w-0">
                    <b className="block text-[13px] md:text-[14px] font-black leading-tight text-sky-700">👥 موجودی مشتریان</b>
                    <span className="block text-[10px] md:text-[11px] font-bold mt-0.5 text-slate-500">مجموع طلب مشتریان از صرافی</span>
                  </div>
                </div>
                <div className="relative space-y-1.5">
                  {currencies.map(cur => {
                    ...
                    return (
                      <div key={cur} className="flex items-center justify-between rounded-xl bg-white/80 px-3 py-2 transition-colors">
                        <span className="text-[12px] font-black text-slate-500">{labels[cur]}</span>
                        <span className={`text-[15px] md:text-base font-black tabular-nums ${bal > 0 ? "text-sky-700" : "text-slate-500"}`}>{fmt(bal)}</span>
                      </div>
                    );
                  })}
                </div>
              ...
              {/* Customer Debts */}
              <div className="group relative overflow-hidden rounded-2xl border border-rose-200 bg-gradient-to-br from-rose-50 to-white p-4 md:p-5 transition-all duration-300 hover:shadow-xl hover:scale-[1.01]">
                <div className="relative flex items-center gap-3 mb-4">
                  <span className="grid h-11 w-11 shrink-0 place-items-center rounded-xl bg-rose-100 text-rose-600"><Ic n="alert" className="h-5 w-5" /></span>
                  <div className="min-w-0">
                    <b className="block text-[13px] md:text-[14px] font-black leading-tight text-rose-700">📉 بدهی مشتریان</b>
                    <span className="block text-[10px] md:text-[11px] font-bold mt-0.5 text-slate-500">مجموع قرض‌های داده‌شده به مشتریان</span>
                  </div>
                </div>
                <div className="relative space-y-1.5">
                  {currencies.map(cur => {
                    ...
                    return (
                      <div key={cur} className="flex items-center justify-between rounded-xl bg-white/80 px-3 py-2 transition-colors">
                        <span className="text-[12px] font-black text-slate-500">{labels[cur]}</span>
                        <span className={`text-[15px] md:text-base font-black tabular-nums ${bal > 0 ? "text-rose-500" : "text-slate-500"}`}>{fmt(bal)}</span>
                      </div>
                    );
                  })}
                </div>
              ...
              <div className="group relative overflow-hidden rounded-2xl border border-amber-200 bg-gradient-to-br from-amber-50 to-white p-4 md:p-5 transition-all duration-300 hover:shadow-xl hover:scale-[1.01]">
                <div className="relative flex items-center gap-3 mb-4">
                  <span className="grid h-11 w-11 shrink-0 place-items-center rounded-xl bg-amber-100 text-amber-600"><Ic n="gem" className="h-5 w-5" /></span>
                  <div className="min-w-0">
                    <b className="block text-[13px] md:text-[14px] font-black leading-tight text-amber-700">💎 کارمزد قابل برداشت</b>
                    <span className="block text-[10px] md:text-[11px] font-bold mt-0.5 text-slate-500">درآمد خالص صرافی از معاملات</span>
                  </div>
                </div>
                <div className="relative space-y-1.5">
                  {currencies.map(cur => {
                    ...
                    return (
                      <div key={cur} className="flex items-center justify-between rounded-xl bg-white/80 px-3 py-2 transition-colors">
                        <span className="text-[12px] font-black text-slate-500">{labels[cur]}</span>
                        <span className={`text-[15px] md:text-base font-black tabular-nums ${bal > 0 ? "text-amber-700" : "text-slate-500"}`}>{fmt(bal)}</span>
                      </div>
                    );
                  })}
                </div>
          ...
          <div className={`cs-up flex gap-1.5 md:gap-2 rounded-xl md:rounded-2xl border p-1.5 md:p-2 shadow-sm backdrop-blur ${glassChip}`} style={{ animationDelay: "140ms" }}>
            {[{ id: "register" as const, label: "ثبت عملیات", icon: "plus" }, { id: "ledger" as const, label: "روزنامچه صندوق", icon: "history", count: entries.length }].map(tab => (
              <button key={tab.id} onClick={() => setActiveTab(tab.id)} className={`flex flex-1 cursor-pointer items-center justify-center gap-1.5 md:gap-2 rounded-lg md:rounded-xl px-3 md:px-5 py-2.5 md:py-3 text-xs md:text-sm font-black transition-all duration-300 active:scale-[0.97] ${activeTab === tab.id ? "bg-gradient-to-l from-emerald-500 via-teal-500 to-cyan-500 text-white shadow-lg" : "text-slate-500 hover:bg-emerald-50 hover:text-slate-800"}`}>
                <Ic n={tab.icon} className="h-4 w-4" />
                <span>{tab.label}</span>
                {tab.count !== undefined && tab.count > 0 && <span className={`rounded-full px-1.5 py-0.5 text-[9px] font-black ${activeTab === tab.id ? "bg-white/30 text-white" : "bg-emerald-100 text-emerald-700"}`}>{tab.count}</span>}
              </button>
            ))}
          </div>
          ...
          {activeTab === "register" && (
            <section className={`cs-up space-y-4 md:space-y-5 p-4 md:p-7 ${uiCard}`} style={{ animationDelay: "160ms" }}>
              <div className="flex flex-wrap items-center gap-3"><span className={`grid h-11 w-11 place-items-center rounded-xl bg-gradient-to-br ring-1 ${identIcon}`}><Ic n="plus" className="h-5 w-5" /></span><div className="flex-1 min-w-0"><h2 className="cs-display text-xl md:text-2xl leading-none text-slate-900">ثبت عملیات صندوق</h2><p className="mt-1 text-[11px] font-bold text-slate-500">واریز و برداشت مشتری یا مالک</p></div></div>
              {editingEntryId && editingEntry && (
                <div className="flex flex-wrap items-center justify-between gap-3 rounded-xl border border-amber-300 bg-amber-100/70 px-4 py-3 text-sm font-bold text-amber-800">
              ...
              <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
                {fld("نوع عملیات *", (<div className="relative"><select value={form.type} onChange={e => { setField("type", e.target.value); if (e.target.value !== "customer_deposit" && e.target.value !== "customer_withdraw") { setField("customerId", ""); setField("customerName", ""); } }} className={`${uiInput} cursor-pointer appearance-none pl-9 ${!form.type ? "text-slate-400" : ""} ${errors.type ? errInput : ""}`}>{entryTypeOptions.map(o => <option key={o[0]} value={o[0]}>{o[1]}</option>)}</select><span className={chevPos}><Ic n="chevron" className="h-4 w-4" /></span></div>))}
              ...
              {isCustomerType && (
                <div className="rounded-xl border border-teal-200 bg-teal-50 p-4">
                  <div className="flex items-center gap-2 mb-3"><span className="grid h-8 w-8 place-items-center rounded-lg bg-teal-100 text-teal-600"><Ic n="user" className="h-4 w-4" /></span><b className="text-xs font-black text-teal-700">مشتری {form.type === "customer_deposit" ? "واریزکننده" : "برداشت‌کننده"}</b></div>
                  {fld("انتخاب مشتری *", (
                    <div className="relative" ref={customerListRef}>
                      ...
                      {showCustomerList && (
                        <div className="absolute left-0 top-full z-30 mt-1 w-full max-h-60 overflow-y-auto rounded-xl border border-slate-200 bg-white shadow-xl">
                          {filteredCustomerList.length === 0 ? (<div className="px-4 py-3 text-xs text-center text-slate-500">مشتری‌ای یافت نشد</div>) : (
                            filteredCustomerList.map((c) => {
                              const liveBal = c.balances[form.currency] || 0;
                              return (
                                <button key={c.id} type="button" onClick={() => { setField("customerId", c.id); setField("customerName", c.name); setCustomerFilter(""); setShowCustomerList(false); }} className="flex w-full items-center gap-2 px-3 py-2.5 text-right text-xs font-bold transition text-slate-700 hover:bg-teal-50 hover:text-teal-600">
                                  <span className="flex-1 truncate flex items-center gap-1.5">
                                    {c.name}
                                    {c.id === EXCHANGE_ACCOUNT_ID && " 💼"}
                                    {hasTelegram(c) && c.id !== EXCHANGE_ACCOUNT_ID && <span title="دارای چت آیدی تلگرام">📱</span>}
                                  </span>
                                  <span className={`text-[10px] tabular-nums font-bold ${currencyColors[form.currency]}`}>{fmt(liveBal)} {labels[form.currency]}</span>
                                </button>
                              );
                            })
                          )}
                  ...
                  {selectedCustomer && selectedCustomer.id !== CASH_BOX_ID && (
                    <div className="mt-4 rounded-xl border border-slate-200 bg-white p-4">
                      <div className="flex items-center gap-2 mb-3 pb-3 border-b border-dashed border-slate-200">
                        <span className="grid h-9 w-9 shrink-0 place-items-center rounded-lg bg-teal-100 text-teal-600"><Ic n="wallet" className="h-4 w-4" /></span>
                        <div className="flex-1 min-w-0"><b className="block text-sm font-black text-teal-700">💼 موجودی کامل {selectedCustomer.name}</b><span className="text-[10px] font-bold text-slate-500">تمام ارزهای نزد صرافی (محاسبه‌شده از دفتر کل)</span></div>
                        {selectedCustomer.phone && <span className="text-[10px] font-bold tabular-nums text-slate-500" dir="ltr">📞 {selectedCustomer.phone}</span>}
                      </div>
                      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-2">
                        {currencies.map(cur => {
                          ...
                          return (
                            <div key={cur} className={`relative rounded-xl px-3 py-2.5 text-center transition-all ${isSel ? "bg-teal-50 ring-2 ring-teal-400/40" : "bg-slate-50"}`}>
                              {isSel && <span className="absolute -top-1.5 -left-1.5 grid h-5 w-5 place-items-center rounded-full text-[9px] bg-teal-500 text-white">✓</span>}
                              <div className="text-[10px] font-bold mb-1 text-slate-500">{labels[cur]}</div>
                              <div className={`text-sm font-black tabular-nums ${isDebt ? "text-rose-500" : isCredit ? "text-emerald-700" : "text-slate-500"}`}>{fmt(bal)}</div>
                              <div className="min-h-[14px] mt-1">
                                {isDebt && <span className="text-[8px] font-black text-rose-500">🔴 قرض</span>}
                                {isCredit && <span className="text-[8px] font-black text-emerald-600">🟢 طلب</span>}
                                {bal === 0 && <span className="text-[8px] font-bold text-slate-500">⚪ صفر</span>}
                              </div>
                            </div>
                          );
                        })}
              ...
              {isCommissionType && (
                <div className="rounded-xl border border-purple-200 bg-purple-50 p-4">
                  <div className="flex items-center gap-2 mb-3"><span className="grid h-8 w-8 place-items-center rounded-lg bg-purple-100 text-purple-600"><Ic n="gem" className="h-4 w-4" /></span><b className="text-xs font-black text-purple-700">💎 برداشت کارمزد صرافی</b></div>
                  <p className="text-[11px] font-bold mb-3 text-slate-500">با این عملیات، کارمزد جمع‌آوری‌شده از صندوق خارج شده و به مالک پرداخت می‌شود.</p>
                  <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-2">
                    {currencies.map(cur => {
                      const avail = availableCommission[cur] || 0; const isSel = form.currency === cur;
                      return (
                        <div key={cur} className={`rounded-xl px-3 py-2.5 text-center ${isSel ? "bg-purple-100 ring-2 ring-purple-400/40" : "bg-white/70"}`}>
                          <div className="text-[10px] font-bold mb-1 text-slate-500">{labels[cur]}</div>
                          <div className={`text-sm font-black tabular-nums ${avail > 0 ? "text-purple-700" : "text-slate-500"}`}>{fmt(avail)}</div>
                          <div className="min-h-[14px] mt-1"><span className={`text-[8px] font-bold ${avail > 0 ? "text-emerald-600" : "text-slate-500"}`}>{avail > 0 ? "✅ قابل برداشت" : "⚪ موجود نیست"}</span></div>
                        </div>
                      );
                    })}
                  </div>
                  <div className="mt-3 flex items-center justify-between rounded-lg bg-white px-3 py-2">
                    <span className="text-xs font-bold text-slate-500">🎯 قابل برداشت ({labels[form.currency]}):</span>
                    <b className={`text-sm font-black tabular-nums ${(availableCommission[form.currency] || 0) > 0 ? "text-purple-700" : "text-rose-500"}`}>{fmt(availableCommission[form.currency] || 0)} {labels[form.currency]}</b>
                  </div>
                </div>
              )}
              ...
              {hasType && (
                <div className={`flex items-center gap-3 rounded-xl border p-4 ${isInType ? "border-emerald-200 bg-emerald-50" : "border-rose-200 bg-rose-50"}`}>
                  <span className={`grid h-10 w-10 place-items-center rounded-xl ${isInType ? "bg-emerald-100 text-emerald-600" : "bg-rose-100 text-rose-600"}`}><Ic n={isInType ? "arrowDown" : "arrowUp"} className="h-5 w-5" /></span>
                  <div>
                    <b className={`text-sm font-black ${isInType ? "text-emerald-700" : "text-rose-700"}`}>{isInType ? "افزایش موجودی صندوق" : "کاهش موجودی صندوق"}</b>
                    <p className="text-[11px] text-slate-500">{isInType ? "مبلغ به موجودی" : "مبلغ از موجودی"} {labels[form.currency]} {isInType ? "اضافه" : "کم"} می‌شود.{isCustomerType && form.customerId && form.customerId !== CASH_BOX_ID && form.customerId !== EXCHANGE_ACCOUNT_ID && (form.type === "customer_deposit" ? " موجودی حساب مشتری هم افزایش می‌یابد." : " موجودی حساب مشتری هم کاهش می‌یابد.")}{isCommissionType && " این مبلغ از کارمزد قابل برداشت کسر می‌شود."}</p>
                  </div>
                </div>
              )}

              {errorList.length > 0 && (<div className="space-y-2 rounded-xl border border-rose-500 bg-rose-50 p-4 text-rose-600"><b className="flex items-center gap-2 text-sm"><Ic n="alert" className="h-5 w-5 shrink-0" />لطفاً فیلدهای اجباری را تکمیل کنید:</b><ul className="list-disc pr-5 text-sm space-y-1">{errorList.map((msg, i) => (<li key={i}>{msg}</li>))}</ul></div>)}
              <button onClick={handleSubmitClick} className="group flex h-[50px] md:h-[52px] w-full cursor-pointer items-center justify-center gap-2 rounded-xl bg-gradient-to-l from-emerald-500 via-teal-500 to-cyan-500 text-base font-black text-white shadow-lg transition-all duration-300 hover:shadow-xl hover:brightness-110 active:scale-[0.985]">{editingEntryId ? "به‌روزرسانی" : "ثبت عملیات"}<Ic n="check" className="h-5 w-5" /></button>
            </section>
          )}
          ...
          {activeTab === "ledger" && (
            <section className={`cs-up overflow-hidden ${uiCard}`} style={{ animationDelay: "180ms" }}>
              <div className="flex flex-wrap items-center gap-3 p-4 md:p-5 pb-3 md:pb-4 md:px-7 md:pt-6">
                ...
                <div className="flex-1 min-w-0"><h2 className="cs-display text-xl md:text-2xl leading-none text-slate-900">روزنامچه صندوق</h2><p className="mt-1 text-[11px] font-bold text-slate-500">تمام دریافتی‌ها و پرداختی‌ها با جزئیات کامل</p></div>
                <div className="flex items-center gap-2">
                  <span className="rounded-full bg-emerald-100 px-3 py-1.5 text-[10px] font-black text-emerald-700">فعال {activeCount}</span>
                  <span className="rounded-full bg-rose-100 px-3 py-1.5 text-[10px] font-black text-rose-600">باطل {voidedCount}</span>
                </div>
              </div>
              <div className="px-4 md:px-7 pb-4 space-y-4">
                <div className="flex flex-wrap gap-3">
                  <div className="relative flex-1 min-w-[250px]"><input value={search} onChange={e => setSearch(e.target.value)} placeholder="جستجو: نام مشتری، کد پیگیری، شرح…" className={`${uiInput} pr-10`} /><span className="pointer-events-none absolute right-3 top-1/2 -translate-y-1/2 text-slate-400"><Ic n="search" className="h-4 w-4" /></span></div>
                  ...
                  {(search || filterType !== "all" || filterCurrency !== "all") && (<button onClick={() => { setSearch(""); setFilterType("all"); setFilterCurrency("all"); }} className="flex items-center gap-1.5 rounded-xl border border-slate-200 px-4 py-2.5 text-xs font-black transition-all active:scale-95 cursor-pointer text-slate-600 hover:bg-slate-50"><Ic n="x" className="h-3.5 w-3.5" />پاک کردن</button>)}
                </div>
                {filteredEntries.length === 0 ? (
                  <div className="flex flex-col items-center gap-3 px-6 py-16 text-slate-400"><span className="grid h-16 w-16 place-items-center rounded-2xl border border-dashed border-slate-300 bg-slate-50"><Ic n="inbox" className="h-7 w-7 opacity-70" /></span><p className="text-sm font-black text-center">{entries.length === 0 ? "هنوز عملیاتی در صندوق ثبت نشده است." : "هیچ عملیاتی با این فیلتر یافت نشد."}</p></div>
                ) : (
                  <div className="overflow-x-auto cs-scroll">
                    <table className="w-full min-w-[1200px] text-sm">
                      <thead><tr className="border-y border-slate-100 bg-slate-50">{["شماره", "کد پیگیری", "تاریخ", "نوع عملیات", "مشتری", "تماس", "تذکره", "شرح", "ارز", "دریافت", "پرداخت", "مانده", "عملیات"].map(h => (<th key={h} className="px-3 py-3 text-center text-[10px] font-black text-slate-400 whitespace-nowrap">{h}</th>))}</tr></thead>
                      <tbody className="divide-y divide-slate-100">
                        {filteredEntries.map((e, idx) => {
                          ...
                          let rowClass = `transition-colors hover:bg-emerald-50/70`;
                          if (isVoided) rowClass += " bg-rose-50";
                          const voidStrike = isVoided ? "line-through opacity-50" : "";
                          return (
                            <tr key={e.id} className={rowClass}>
                              <td className="px-3 py-3 text-center"><span className="inline-grid h-7 w-7 place-items-center rounded-lg text-[10px] font-black tabular-nums bg-slate-100 text-slate-500">{idx + 1}</span></td>
                              <td className="px-3 py-3 text-center">
                                <div className="flex flex-col items-center gap-1">
                                  <span className={`inline-flex items-center gap-1 rounded-lg border border-sky-300 bg-sky-50 px-2 py-1 text-[10px] font-black tabular-nums text-sky-700 ${voidStrike}`} dir="ltr"><Ic n="tag" className="h-2.5 w-2.5" />{e.trackingCode}</span>
                                  {isVoided && <span className="inline-flex rounded-full bg-rose-100 px-2 py-0.5 text-[8px] font-black text-rose-600">باطل شده</span>}
                                  {isDeleted && <span className="inline-flex rounded-full bg-slate-100 px-2 py-0.5 text-[8px] font-black text-slate-500">مشتری حذف‌شده</span>}
                                </div>
                              </td>
                              <td className={`whitespace-nowrap px-3 py-3 text-center text-[11px] tabular-nums text-slate-500 ${voidStrike}`}><div dir="ltr">{shortDateLabel(e.date)}</div><div dir="ltr" className="text-[9px] text-slate-500">{timeLabel(e.date)}</div></td>
                              <td className="px-3 py-3 text-center"><span className={`inline-flex rounded-full px-2 py-0.5 text-[9px] font-black whitespace-nowrap ${entryTypeColors[e.type]} ${voidStrike}`}>{entryTypeLabels[e.type]}</span></td>
                              <td className={`px-3 py-3 text-center text-[12px] font-bold whitespace-nowrap text-slate-700 ${voidStrike}`}>{isOwner ? <span className="text-amber-700">👤 مالک</span> : isAdjust ? <span className="text-slate-500">سیستم</span> : isFee ? <span className="text-slate-500">سیستم</span> : isCommW ? <span className="text-purple-700">💎 کارمزد</span> : (e.customerName || "—")}</td>
                              <td className={`px-3 py-3 text-center text-[11px] tabular-nums whitespace-nowrap text-slate-600 ${voidStrike}`} dir="ltr">{e.customerPhone || "—"}</td>
                              <td className={`px-3 py-3 text-center text-[11px] tabular-nums whitespace-nowrap text-slate-600 ${voidStrike}`} dir="ltr">{e.customerTazkira || "—"}</td>
                              <td className={`px-3 py-3 text-center text-[11px] max-w-[180px] truncate text-slate-600 ${voidStrike}`}>{e.reason || "—"}</td>
                              <td className={`px-3 py-3 text-center text-[11px] font-black whitespace-nowrap ${currencyColors[e.currency]} ${voidStrike}`}>{labels[e.currency]}</td>
                              ...
                              <td className={`px-3 py-3 text-center text-[12px] font-black tabular-nums whitespace-nowrap ${currencyColors[e.currency]} ${voidStrike}`}>{fmt(e.balanceAfter)}</td>
                              <td className="px-3 py-3 text-center">
                                <div className="relative action-dropdown flex justify-center">
                                  <button onClick={(ev) => { ev.stopPropagation(); setOpenActionId(isOpen ? null : e.id); }} className="grid h-8 w-8 place-items-center rounded-lg border border-slate-200 text-slate-500 transition-all duration-150 active:scale-90 cursor-pointer hover:bg-slate-100" title="عملیات"><Ic n="more" className="h-4 w-4" /></button>
                                  {isOpen && (
                                    <div className="absolute left-1/2 -translate-x-1/2 top-full z-50 mt-1.5 w-44 overflow-hidden rounded-xl border border-slate-200 bg-white shadow-xl">
                                      <button onClick={() => { setSelectedEntry(e); setOpenActionId(null); }} className="flex w-full items-center gap-2 px-3 py-2 text-right text-xs font-bold transition text-cyan-600 hover:bg-cyan-50"><Ic n="eye" className="h-3.5 w-3.5" /> مشاهده</button>
                                      {!isVoided && (<><button onClick={() => { editEntry(e); setOpenActionId(null); }} className="flex w-full items-center gap-2 px-3 py-2 text-right text-xs font-bold transition text-sky-600 hover:bg-sky-50"><Ic n="pencil" className="h-3.5 w-3.5" /> ویرایش</button><button onClick={() => { voidEntry(e); setOpenActionId(null); }} className="flex w-full items-center gap-2 px-3 py-2 text-right text-xs font-bold transition text-amber-600 hover:bg-amber-50"><Ic n="xCircle" className="h-3.5 w-3.5" /> ابطال</button></>)}
                                      <div className="my-1 h-px bg-slate-100" />
                                      <button onClick={() => { deleteEntry(e); setOpenActionId(null); }} className="flex w-full items-center gap-2 px-3 py-2 text-right text-xs font-bold transition text-rose-500 hover:bg-rose-50"><Ic n="trash" className="h-3.5 w-3.5" /> حذف</button>
                                    </div>
                                  )}
                                </div>
                              </td>
      ...
      {selectedEntry && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/60 p-3" onClick={() => setSelectedEntry(null)}>
          <div className="w-full max-w-lg rounded-xl border border-slate-200 bg-white shadow-2xl" onClick={ev => ev.stopPropagation()}>
            <div className="flex items-center justify-between border-b border-slate-100 px-4 py-3">
              <b className="text-sm text-slate-800">جزئیات سند صندوق</b>
            ...
            <div className="max-h-[70vh] overflow-y-auto px-4 py-2">
              {[["کد پیگیری", selectedEntry.trackingCode], ["تاریخ", `${shortDateLabel(selectedEntry.date)} - ${timeLabel(selectedEntry.date)}`], ["نوع عملیات", entryTypeLabels[selectedEntry.type]], ["مشتری", selectedEntry.customerName || (selectedEntry.type.includes("owner") ? "مالک" : selectedEntry.type === "commission_withdraw" ? "💎 برداشت کارمزد" : "سیستم")], ["ارز", labels[selectedEntry.currency]], ["مبلغ", fmt(selectedEntry.amount)], ["جهت", selectedEntry.direction === "in" ? "دریافت ➕" : "پرداخت ➖"], ["شرح", selectedEntry.reason || "-"], ["مانده بعد", fmt(selectedEntry.balanceAfter)], ["وضعیت", selectedEntry.status === "voided" ? "❌ باطل شده" : "✅ فعال"]].map(([l, v], i) => (
                <div key={i} className="flex items-start justify-between gap-4 border-b border-dashed border-slate-200 py-3 last:border-0">
                  <span className="shrink-0 text-[11px] font-black text-slate-400">{l}</span>
                  <span className={`text-left text-[13px] font-bold text-slate-700 ${l === "وضعیت" && selectedEntry.status === "voided" ? "text-rose-500" : ""}`}>{v}</span>
                </div>
              ))}
            </div>
          </div>
      ...
      {previewOpen && previewData && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/60 p-3 md:p-4 backdrop-blur-sm" onClick={() => { setPreviewOpen(false); setPreviewData(null); }}>
          <div className="cs-up w-full max-w-lg overflow-hidden rounded-xl md:rounded-2xl border border-slate-200 bg-white shadow-2xl" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between border-b border-slate-100 bg-slate-50 px-4 md:px-5 py-3 md:py-4">
              <b className="flex items-center gap-2 text-sm text-slate-800"><span className="grid h-8 w-8 place-items-center rounded-lg bg-emerald-100 text-emerald-600"><Ic n="doc" className="h-4 w-4" /></span>{editingEntryId ? "تأیید ویرایش سند" : isCommissionType ? "تأیید برداشت کارمزد" : "تأیید عملیات صندوق"}</b>
              <button onClick={() => { setPreviewOpen(false); setPreviewData(null); }} className="grid h-8 w-8 cursor-pointer place-items-center rounded-lg text-slate-400 transition-all duration-300 hover:bg-slate-100 hover:text-slate-700 hover:rotate-90"><Ic n="x" className="h-4 w-4" /></button>
            </div>
            <div className="px-4 md:px-5 py-4 space-y-4">
              <div className="flex items-center justify-between rounded-xl border border-sky-300 bg-sky-50 p-3">
                <b className="text-xs font-black text-sky-700">کد پیگیری</b>
                <span className="inline-flex items-center gap-1.5 rounded-lg border border-sky-300 bg-sky-50 px-3 py-1.5 text-[13px] font-black tabular-nums text-sky-700" dir="ltr"><Ic n="tag" className="h-3.5 w-3.5" />{previewData.trackingCode}</span>
              </div>
              <div className="rounded-xl border border-slate-200 bg-slate-50 p-4">
                <div className="grid grid-cols-2 gap-3 text-sm">
                  <div><span className="text-slate-500">نوع: </span><b>{entryTypeLabels[previewData.type]}</b></div>
                  <div><span className="text-slate-500">ارز: </span><b>{labels[previewData.currency]}</b></div>
                  <div><span className="text-slate-500">مبلغ: </span><b className={`tabular-nums ${previewData.direction === "in" ? "text-emerald-700" : "text-rose-500"}`}>{fmt(previewData.amount)}</b></div>
                  <div><span className="text-slate-500">جهت: </span><b>{previewData.direction === "in" ? "دریافت ➕" : "پرداخت ➖"}</b></div>
                  {previewData.customerName && (<div className="col-span-2"><span className="text-slate-500">مشتری: </span><b>{previewData.customerName}</b></div>)}
                  <div className="col-span-2"><span className="text-slate-500">شرح: </span><b>{previewData.reason}</b></div>
                </div>
              </div>
              <div className="flex flex-wrap gap-3 pt-2">
                <button onClick={confirmRegister} className="flex h-[48px] flex-1 min-w-[180px] cursor-pointer items-center justify-center gap-2 rounded-xl bg-gradient-to-l from-emerald-500 to-teal-500 text-sm font-black text-white shadow-lg transition-all hover:brightness-110 active:scale-[0.98]">{editingEntryId ? "ذخیره تغییرات" : "ثبت نهایی"}<Ic n="check" className="h-4 w-4" /></button>
                <button onClick={() => { setPreviewOpen(false); setPreviewData(null); }} className="flex h-[48px] px-6 cursor-pointer items-center justify-center rounded-xl border border-slate-200 text-sm font-bold text-slate-600 transition-all active:scale-95 hover:bg-slate-50">انصراف</button>
              </div>
            </div>
          </div>
        </div>
