...
export default function CustomersPage() {
  ...
  // ✅ نسخه نهایی و ضدگلوله‌ی محاسبه‌ی موجودی‌ها
  const allBalances = useMemo(() => {
    ...
    // ۱. محاسبه موجودی مشتریان عادی از روی دفتر کل
    ...
    for (const cur of currencies) {
      ...
      for (const tx of transactions) {
        ...
        if (tx.commission && tx.commission > 0 && tx.commissionCurrency === cur) {
          exchBalance += Number(tx.commission);
        }
      }
    ...
    return map;
  }, [customers, cashEntries, transactions, hawalas, ledger]);
