...
// ✅ خواندن هر دو منبع برای کد پیگیری مشترک
function getMaxTrackingNumberFromAllSources(): number {
  if (typeof window === "undefined") return 0;
  try {
    let maxNum = 0;
    const txs = safeGetItem(TRANSACTIONS_KEY);
    if (Array.isArray(txs)) {
      for (const tx of txs) {
        if (!tx) continue;
        const match = String(tx.trackingCode || "").match(/(\d+)$/);
        if (match) maxNum = Math.max(maxNum, Number(match[1]) || 0);
      }
    }
    const hws = safeGetItem(HAWALAS_KEY);
    if (Array.isArray(hws)) {
      for (const h of hws) {
        if (!h) continue;
        const match = String(h.number || "").match(/(\d+)$/);
        if (match) maxNum = Math.max(maxNum, Number(match[1]) || 0);
      }
    }
    return maxNum;
  ...
  try {
    ...
    if (Array.isArray(parsed) && typeof parsed[0] === "string") {
      const migrated = parsed.map((name: string, i: number): Customer => ({
        id: `cust-migrated-${i}`, name, phone: "", tazkira: "",
        balances: { AFN: 0, USD: 0, EUR: 0, IRR: 0, PKR: 0 },
      }));
      safeSetItem(CUSTOMERS_KEY, migrated);
      return migrated;
    }
...
const loadTransactions = (): Transaction[] => {
  ...
  try {
    ...
    return parsed.filter((t: any) => t && typeof t === "object" && t.id).map((t: any): Transaction => ({
      ...
      commissionPayer: t.commissionPayer === "receiver" ? "receiver" : t.commissionPayer === "sender" ? "sender" : undefined,
      description: t.description, status: t.status === "voided" ? "voided" : "active",
...
function CommissionBadge({ payer, txType, dk }: { payer?: CommissionPayer; txType: string; dk: boolean }) {
  ...
  if (txType === "convert") return (
    <span className={`inline-flex items-center gap-1 rounded-full px-2 py-0.5 text-[10px] font-black ring-1 ${dk ? "bg-violet-400/15 text-violet-300 ring-violet-400/25" : "bg-violet-100 text-violet-700 ring-violet-300/60"}`}>
      <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2} className="h-3 w-3"><path d="M15.75 6a3.75 3.75 0 1 1-7.5 0 3.75 3.75 0 0 1 7.5 0ZM4.501 20.118a7.5 7.5 0 0 1 14.998 0" /></svg>
      خود مشتری
    </span>
  );
  if (payer === "sender") return (
...
export default function CurrencyExchangePage() {
  ...
  const [nextCode, setNextCode] = useState("FX-0001");
  ...
  useEffect(() => {
    try {
      ...
      const maxNum = getMaxTrackingNumberFromAllSources();
  ...
  function resetTransferForm() { setSender(""); setReceiver(""); setSenderAmount(""); setReceiverAmount(""); setTransferRate(""); setCommission(""); setTransferCommissionPayer("sender"); setTransferCommissionCurrency("AFN"); setTransferDescription(""); setTransferErrors({}); }
  function resetConvertForm() { setConvertCustomer(""); setConvertFromCurrency("AFN"); setConvertToCurrency("USD"); setConvertAmount(""); setConvertRate(""); setConvertedAmount(""); setConvertCommission(""); setConvertCommissionCurrency("AFN"); setConvertDescription(""); setConvertErrors({}); }
  ...
  function submitExchange() {
    ...
    try {
      ...
      const newTx: Transaction = {
        id: generateId(), trackingCode: consumeSharedCode("FX"),
        type: "exchange", dealType: exchangeDealType as DealType, date: new Date().toISOString(),
        ...
        description, status: "active", profit: exchangeCommissionValue, profitCurrency: exchangeCommissionCurrency,
      };
      setTransactions(prev => [newTx, ...prev]);
      setCustomers(prev => applyBalanceChanges(prev, getBalanceChangesForTransaction(newTx)));
      resetExchangeForm();
      showToast("معامله با موفقیت ثبت شد و موجودی به‌روز شد.");
    } catch (err) {
  ...
  function submitTransfer() {
    ...
    try {
      ...
      const newTx: Transaction = {
        id: generateId(), trackingCode: consumeSharedCode("FX"),
        type: "transfer", date: new Date().toISOString(), senderId: sender, receiverId: receiver,
        ...
        description, status: "active", profit: commissionValue, profitCurrency: transferCommissionCurrency,
      };
      setTransactions(prev => [newTx, ...prev]);
      setCustomers(prev => applyBalanceChanges(prev, getBalanceChangesForTransaction(newTx)));
      resetTransferForm();
      showToast("انتقال با موفقیت ثبت شد و موجودی به‌روز شد.");
    } catch (err) {
      console.error("Submit error:", err);
      showToast("خطا در ثبت انتقال");
    }
  }
  ...
  function submitConvert() {
    ...
    try {
      ...
      const newTx: Transaction = {
        id: generateId(), trackingCode: consumeSharedCode("FX"),
        type: "convert", date: new Date().toISOString(),
        ...
        description, status: "active", profit: convertCommissionValue, profitCurrency: convertCommissionCurrency,
      };
      setTransactions(prev => [newTx, ...prev]);
      setCustomers(prev => applyBalanceChanges(prev, getBalanceChangesForTransaction(newTx)));
      resetConvertForm();
      showToast("تبدیل با موفقیت ثبت شد و موجودی به‌روز شد.");
    } catch (err) {
      console.error("Submit error:", err);
      showToast("خطا در ثبت تبدیل");
    }
  }
  ...
  function voidTransaction(tx: Transaction) {
    ...
    try {
      ...
      showToast("معامله لغو شد و موجودی به حالت قبل برگشت.");
    } catch (err) {
  ...
  function printReceipt(tx: Transaction) {
    try {
      ...
      if (!win) return;
      const html = `<!DOCTYPE html><html dir="rtl"><head><meta charset="utf-8"><title>رسید معامله</title><style>
        body{font-family:Tahoma,Arial,sans-serif;padding:24px;direction:rtl}
        h2{margin-bottom:16px}
        .tracking{background:#f0f9ff;border:2px solid #0ea5e9;border-radius:8px;padding:12px;text-align:center;font-size:24px;font-weight:bold;color:#0369a1;margin-bottom:16px}
        table{width:100%;border-collapse:collapse}
        td,th{border:1px solid #ccc;padding:8px;text-align:right}
      </style></head><body>
        <h2>رسید معامله</h2>
        <div class="tracking">${tx.trackingCode}</div>
        <table>
          <tr><th>کد پیگیری</th><td style="font-weight:bold;color:#0369a1">${tx.trackingCode}</td></tr>
          <tr><th>تاریخ</th><td>${dateLabel(tx.date)}</td></tr>
          ...
          <tr><th>پرداخت‌کننده کارمزد</th><td>${tx.commissionPayer === "sender" ? "فرستنده" : tx.commissionPayer === "receiver" ? "گیرنده" : "خود مشتری"}</td></tr>
  ...
  const filteredTransactions = useMemo(() => {
    try {
      ...
      return transactions.filter(tx => {
        const fields = [tx.trackingCode, transactionCustomerLabel(tx), labels[tx.fromCurrency], labels[tx.toCurrency], String(tx.fromAmount), String(tx.toAmount)];
    ...
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="text-center">
          <div className="inline-block h-8 w-8 animate-spin rounded-full border-4 border-slate-300 border-t-sky-500" />
  ...
  const actionButtonClass = `flex w-full cursor-pointer items-center gap-2 rounded-lg px-3 py-2 text-right text-xs font-bold transition-colors ${dk ? "text-slate-300 hover:bg-cyan-400/10 hover:text-cyan-300" : "text-slate-600 hover:bg-sky-50 hover:text-sky-700"}`;
  const dangerActionButtonClass = `flex w-full cursor-pointer items-center gap-2 rounded-lg px-3 py-2 text-right text-xs font-bold transition-colors ${dk ? "text-rose-300 hover:bg-rose-400/10" : "text-rose-500 hover:bg-rose-50"}`;
  ...
  const midBadgeMobile = (icon: IconName, cls: string) => (
    <div className="flex items-center justify-center lg:hidden py-1">
      <span className={`grid h-10 w-10 place-items-center rounded-full border shadow-md ${cls}`}><Ic n={icon} className="h-4 w-4" /></span>
    </div>
  );
  const secHead = (iconCls: string, icon: IconName, title: string, sub: string, chipCls: string, chipTxt: string) => (
    <div className="flex flex-wrap items-center gap-3">
      ...
      <div className="flex-1 min-w-0">
        <h2 className={`cu-display text-xl leading-tight md:text-2xl ${heading}`}>{title}</h2>
  ...
  return (
    <div dir="rtl" className={dk ? "dark" : ""}>
      <style>{`@import url("https://fonts.googleapis.com/css2?family=Lalezar&family=Vazirmatn:wght@300;400;500;600;700;800;900&display=swap");.cu-font{font-family:"Vazirmatn","Segoe UI",Tahoma,sans-serif}.cu-display{font-family:"Lalezar","Vazirmatn",Tahoma,sans-serif;letter-spacing:.01em}.dark{color-scheme:dark}.cu-grid{background-image:radial-gradient(circle at 1px 1px,rgba(2,132,199,.10) 1px,transparent 0);background-size:24px 24px;-webkit-mask-image:linear-gradient(to bottom,rgba(0,0,0,.9),rgba(0,0,0,.25) 60%,transparent);mask-image:linear-gradient(to bottom,rgba(0,0,0,.9),rgba(0,0,0,.25) 60%,transparent)}.dark .cu-grid{background-image:radial-gradient(circle at 1px 1px,rgba(148,163,184,.08) 1px,transparent 0)}@keyframes cuUp{from{opacity:0;transform:translateY(16px)}to{opacity:1;transform:translateY(0)}}@keyframes cuPop{from{opacity:0;transform:scale(.96) translateY(10px)}to{opacity:1;transform:scale(1) translateY(0)}}.cu-up{animation:cuUp .5s cubic-bezier(.22,.8,.35,1) both}.cu-pop{animation:cuPop .28s cubic-bezier(.22,.8,.35,1) both}details>summary{list-style:none}details>summary::-webkit-details-marker{display:none}.cu-scroll::-webkit-scrollbar{height:0;width:0}.cu-scroll{scrollbar-width:none;-ms-overflow-style:none}::selection{background:rgba(14,165,233,.25)}`}</style>
      <div className={`cu-font relative min-h-screen overflow-x-hidden antialiased transition-colors duration-500 ${dk ? "bg-[#0f172a] text-slate-100" : "bg-[#eef6fa] text-slate-800"}`}>
        ...
        <div className="relative z-10 mx-auto w-full max-w-7xl space-y-4 md:space-y-6 px-3 pb-16 pt-5 md:px-8 md:pt-9">
          <header className="cu-up flex flex-wrap items-center justify-between gap-3">
            <div className="flex items-center gap-2.5 md:gap-3.5 min-w-0">
              ...
              <div className="min-w-0">
                <h1 className={`cu-display text-2xl md:text-4xl leading-none ${heading}`}>معاملات ارزی</h1>
          ...
          <div className="cu-up space-y-2" style={{ animationDelay: "70ms" }}>
          ...
          <div className={`cu-up flex gap-1.5 md:gap-2 rounded-xl md:rounded-2xl border p-1.5 md:p-2 shadow-sm backdrop-blur ${glassChip}`} style={{ animationDelay: "140ms" }}>
            <button onClick={() => setTab("exchange")} className={`flex flex-1 cursor-pointer items-center justify-center gap-1.5 rounded-lg md:rounded-xl px-3 md:px-5 py-2.5 md:py-3 text-xs md:text-sm font-black transition-all duration-300 active:scale-[0.97] ${tab === "exchange" ? `bg-gradient-to-l shadow-lg ${dk ? "from-cyan-400 to-emerald-400 text-slate-950" : "from-sky-500 via-cyan-500 to-emerald-400 text-white"}` : dk ? "text-slate-400 hover:bg-slate-700/60 hover:text-slate-100" : "text-slate-500 hover:bg-sky-50 hover:text-slate-800"}`}>
              <Ic n="swap" className="h-4 w-4" /><span>تبادل ارز</span>
            </button>
            <button onClick={() => setTab("transfer")} className={`flex flex-1 cursor-pointer items-center justify-center gap-1.5 rounded-lg md:rounded-xl px-3 md:px-5 py-2.5 md:py-3 text-xs md:text-sm font-black transition-all duration-300 active:scale-[0.97] ${tab === "transfer" ? `bg-gradient-to-l shadow-lg ${dk ? "from-orange-400 to-amber-300 text-slate-950" : "from-orange-500 via-orange-400 to-amber-400 text-white"}` : dk ? "text-slate-400 hover:bg-slate-700/60 hover:text-slate-100" : "text-slate-500 hover:bg-sky-50 hover:text-slate-800"}`}>
              <Ic n="users" className="h-4 w-4" /><span>بین مشتریان</span>
            </button>
            <button onClick={() => setTab("convert")} className={`flex flex-1 cursor-pointer items-center justify-center gap-1.5 rounded-lg md:rounded-xl px-3 md:px-5 py-2.5 md:py-3 text-xs md:text-sm font-black transition-all duration-300 active:scale-[0.97] ${tab === "convert" ? `bg-gradient-to-l shadow-lg ${dk ? "from-violet-400 to-purple-400 text-slate-950" : "from-violet-500 via-purple-500 to-fuchsia-400 text-white"}` : dk ? "text-slate-400 hover:bg-slate-700/60 hover:text-slate-100" : "text-slate-500 hover:bg-violet-50 hover:text-slate-800"}`}>
          ...
          {tab === "exchange" && (
            <section className={`cu-up space-y-4 md:space-y-5 p-4 md:p-7 ${uiCard}`}>
              {secHead(identExIcon, "swap", "تبادل ارز صرافی با مشتری", "دریافت یک ارز از مشتری و پرداخت ارز دیگر", identExChip, "معاملهٔ جدید")}

              <div className="grid gap-3 md:gap-4 sm:grid-cols-2 xl:grid-cols-4">
                {fld("کد پیگیری", trackingField(nextCode, "bg-gradient-to-r from-cyan-500 to-sky-500"))}
                {fld("تاریخ و ساعت (شمسی)", (<input readOnly dir="ltr" value={currentDateTime} className={`${uiInput} ${roInput} pl-10 text-left tabular-nums`} />))}
              ...
              <div className="grid gap-3 md:gap-4 lg:grid-cols-[1fr_auto_1fr]">
                <div className={`space-y-4 rounded-2xl border p-4 transition-colors ${cEmerald.wrap}`}>
                  <div className="flex items-center gap-2.5"><span className={`grid h-9 w-9 place-items-center rounded-xl ${cEmerald.icon}`}><Ic n="down" className="h-4 w-4" /></span><b className={`text-sm font-black ${cEmerald.title}`}>دریافت از مشتری</b></div>
                ...
                <div className={`space-y-4 rounded-2xl border p-4 transition-colors ${cSky.wrap}`}>
                  <div className="flex items-center gap-2.5"><span className={`grid h-9 w-9 place-items-center rounded-xl ${cSky.icon}`}><Ic n="up" className="h-4 w-4" /></span><b className={`text-sm font-black ${cSky.title}`}>پرداخت به مشتری</b></div>
              ...
              {exchangeMode === "afn" && exchangeForeign && (
                <div>{rateBox(cSky, "نرخ دستی در برابر افغانی", (<div><label className={uiLabel}>نرخ</label><div className="flex flex-wrap items-center gap-2.5"><span className={rateChip}>{fmt(rateUnits[exchangeForeign])} {labels[exchangeForeign]} =</span><input type="text" inputMode="decimal" dir="ltr" value={rate} onChange={(e) => { setRate(toNumericText(e.target.value)); setExchangeErrors((p) => ({ ...p, rate: undefined, paidAmount: undefined })); }} placeholder="0" className={`h-12 w-32 md:w-44 px-3 text-left text-sm font-bold tabular-nums ${inputShell} ${exchangeErrors.rate ? errInput : ""}`} /><span className={rateChip}>{labels.AFN}</span></div></div>), (<>{pill(cSky.badge, exchangeRateValue > 0 ? `نرخ ثبت‌شده: ${afnRateLabel(exchangeForeign, exchangeRateValue)}` : "", true)}{pill(cEmerald.badge, paidAmount ? `نتیجه: ${paidAmount} ${labels[paidCurrency]}` : "")}</>))}</div>
              )}
              {exchangeMode === "direct" && (
                <div>{rateBox(cAmber, "نرخ مستقیم جفت‌ارز", (<div className="grid items-end gap-3 md:gap-4 md:grid-cols-2">{fld("مبنای نرخ", sel(exchangeDirectBaseValue, (v) => { setExchangeDirectBase(v as Currency); setExchangeErrors((p) => ({ ...p, rate: undefined, paidAmount: undefined })); }, [[receivedCurrency, labels[receivedCurrency]], [paidCurrency, labels[paidCurrency]]]))}<div><label className={uiLabel}>نرخ مستقیم</label><div className="flex flex-wrap items-center gap-2"><span className={rateChip}>{fmt(rateUnits[exchangeDirectBaseValue])} {labels[exchangeDirectBaseValue]} =</span><input type="text" inputMode="decimal" dir="ltr" value={rate} onChange={(e) => { setRate(toNumericText(e.target.value)); setExchangeErrors((p) => ({ ...p, rate: undefined, paidAmount: undefined })); }} placeholder="0" className={`h-12 w-28 md:w-40 px-3 text-left text-sm font-bold tabular-nums ${inputShell} ${exchangeErrors.rate ? errInput : ""}`} /><span className={rateChip}>{exchangeDirectCounter ? labels[exchangeDirectCounter] : ""}</span></div></div></div>), (<>{pill(cAmber.badge, exchangeRateValue > 0 && exchangeDirectCounter ? `نرخ ثبت‌شده: ${directRateLabel(exchangeDirectBaseValue, exchangeDirectCounter, exchangeRateValue)}` : "", true)}{pill(cEmerald.badge, paidAmount ? `نتیجه: ${paidAmount} ${labels[paidCurrency]}` : "")}</>))}</div>
              )}

              <div className="grid gap-3 md:gap-4 md:grid-cols-2">
                {fld("کارمزد", (<input type="text" inputMode="decimal" dir="ltr" value={exchangeCommission} onChange={(e) => { setExchangeCommission(toNumericText(e.target.value)); setExchangeErrors((p) => ({ ...p, exchangeCommission: undefined })); }} placeholder="0" className={`${uiInput} text-left tabular-nums ${exchangeErrors.exchangeCommission ? errInput : ""}`} />))}
              ...
              <button onClick={submitExchange} className={`flex h-[50px] md:h-[52px] w-full cursor-pointer items-center justify-center gap-2 rounded-xl bg-gradient-to-l text-base font-black shadow-lg transition-all duration-300 hover:shadow-xl hover:brightness-110 active:scale-[0.985] ${dk ? "from-cyan-400 to-emerald-400 text-slate-950" : "from-sky-500 via-cyan-500 to-emerald-400 text-white"}`}>
                ثبت معامله
          ...
          {tab === "transfer" && (
            <section className={`cu-up space-y-4 md:space-y-5 p-4 md:p-7 ${uiCard}`}>
              {secHead(identTrIcon, "users", "تبادل بین حساب مشتریان", "انتقال موجودی از حساب مشتری به مشتری دیگر", identTrChip, "انتقال جدید")}

              <div className="grid gap-3 md:gap-4 sm:grid-cols-2">
                {fld("کد پیگیری", trackingField(nextCode, "bg-gradient-to-r from-orange-500 to-amber-500"))}
                {fld("تاریخ و ساعت (شمسی)", (<input readOnly dir="ltr" value={currentDateTime} className={`${uiInput} ${roInput} pl-10 text-left tabular-nums`} />))}
              </div>

              <div className="grid gap-3 md:gap-4 lg:grid-cols-[1fr_auto_1fr]">
                <div className={`space-y-4 rounded-2xl border p-4 transition-colors ${cOrange.wrap}`}>
                  <div className="flex items-center gap-2.5"><span className={`grid h-9 w-9 place-items-center rounded-xl ${cOrange.icon}`}><Ic n="up" className="h-4 w-4" /></span><b className={`text-sm font-black ${cOrange.title}`}>فرستنده</b></div>
                  {fld("مشتری فرستنده", sel(sender, (v) => { setSender(v); setTransferErrors((p) => ({ ...p, sender: undefined })); }, customerOpts, transferErrors.sender ? errInput : ""))}
                  ...
                  {fld("مبلغ فرستنده", (<input type="text" inputMode="decimal" dir="ltr" value={senderAmount} onChange={(e) => { setSenderAmount(toNumericText(e.target.value)); setTransferErrors((p) => ({ ...p, senderAmount: undefined, receiverAmount: undefined })); }} placeholder="0.00" className={`${uiInput} text-left tabular-nums ${transferErrors.senderAmount ? errInput : ""}`} />))}
                </div>
                ...
                <div className={`space-y-4 rounded-2xl border p-4 transition-colors ${cEmerald.wrap}`}>
                  <div className="flex items-center gap-2.5"><span className={`grid h-9 w-9 place-items-center rounded-xl ${cEmerald.icon}`}><Ic n="down" className="h-4 w-4" /></span><b className={`text-sm font-black ${cEmerald.title}`}>گیرنده</b></div>
                  {fld("مشتری گیرنده", sel(receiver, (v) => { setReceiver(v); setTransferErrors((p) => ({ ...p, receiver: undefined })); }, customerOpts, transferErrors.receiver ? errInput : ""))}
              ...
              {transferMode === "afn" && transferForeign && (
                <div>{rateBox(cTeal, "نرخ دستی در برابر افغانی", (<div><label className={uiLabel}>نرخ</label><div className="flex flex-wrap items-center gap-2.5"><span className={rateChip}>{fmt(rateUnits[transferForeign])} {labels[transferForeign]} =</span><input type="text" inputMode="decimal" dir="ltr" value={transferRate} onChange={(e) => { setTransferRate(toNumericText(e.target.value)); setTransferErrors((p) => ({ ...p, transferRate: undefined, receiverAmount: undefined })); }} placeholder="0" className={`h-12 w-32 md:w-44 px-3 text-left text-sm font-bold tabular-nums ${inputShell} ${transferErrors.transferRate ? errInput : ""}`} /><span className={rateChip}>{labels.AFN}</span></div></div>), (<>{pill(cTeal.badge, transferRateValue > 0 ? `نرخ ثبت‌شده: ${afnRateLabel(transferForeign, transferRateValue)}` : "", true)}{pill(cEmerald.badge, receiverAmount ? `نتیجه: ${receiverAmount} ${labels[receiverCurrency]}` : "")}</>))}</div>
              )}
              {transferMode === "direct" && (
                <div>{rateBox(cOrange, "نرخ مستقیم جفت‌ارز", (<div className="grid items-end gap-3 md:gap-4 md:grid-cols-2">{fld("مبنای نرخ", sel(transferDirectBaseValue, (v) => { setTransferDirectBase(v as Currency); setTransferErrors((p) => ({ ...p, transferRate: undefined, receiverAmount: undefined })); }, [[senderCurrency, labels[senderCurrency]], [receiverCurrency, labels[receiverCurrency]]]))}<div><label className={uiLabel}>نرخ مستقیم</label><div className="flex flex-wrap items-center gap-2"><span className={rateChip}>{fmt(rateUnits[transferDirectBaseValue])} {labels[transferDirectBaseValue]} =</span><input type="text" inputMode="decimal" dir="ltr" value={transferRate} onChange={(e) => { setTransferRate(toNumericText(e.target.value)); setTransferErrors((p) => ({ ...p, transferRate: undefined, receiverAmount: undefined })); }} placeholder="0" className={`h-12 w-28 md:w-40 px-3 text-left text-sm font-bold tabular-nums ${inputShell} ${transferErrors.transferRate ? errInput : ""}`} /><span className={rateChip}>{transferDirectCounter ? labels[transferDirectCounter] : ""}</span></div></div></div>), (<>{pill(cOrange.badge, transferRateValue > 0 && transferDirectCounter ? `نرخ ثبت‌شده: ${directRateLabel(transferDirectBaseValue, transferDirectCounter, transferRateValue)}` : "", true)}{pill(cEmerald.badge, receiverAmount ? `نتیجه: ${receiverAmount} ${labels[receiverCurrency]}` : "")}</>))}</div>
              )}

              <div className="grid gap-3 md:gap-4 md:grid-cols-2">
                {fld("کارمزد", (<input type="text" inputMode="decimal" dir="ltr" value={commission} onChange={(e) => { setCommission(toNumericText(e.target.value)); setTransferErrors((p) => ({ ...p, commission: undefined })); }} placeholder="0" className={`${uiInput} text-left tabular-nums ${transferErrors.commission ? errInput : ""}`} />))}
              ...
              <button onClick={submitTransfer} className={`flex h-[50px] md:h-[52px] w-full cursor-pointer items-center justify-center gap-2 rounded-xl bg-gradient-to-l text-base font-black shadow-lg transition-all duration-300 hover:shadow-xl hover:brightness-110 active:scale-[0.985] ${dk ? "from-orange-400 to-amber-300 text-slate-950" : "from-orange-500 via-orange-400 to-amber-400 text-white"}`}>
                ثبت انتقال
                <Ic n="arrowLeft" className="h-5 w-5 transition-transform group-hover:-translate-x-1" />
              </button>
            </section>
          ...
          {tab === "convert" && (
            <section className={`cu-up space-y-4 md:space-y-5 p-4 md:p-7 ${uiCard}`}>
              {secHead(identCvIcon, "user", "تبدیل ارز مشتری", "تبدیل یک ارز به ارز دیگر در حساب خود مشتری", identCvChip, "تبدیل جدید")}

              <div className="grid gap-3 md:gap-4 sm:grid-cols-2">
                {fld("کد پیگیری", trackingField(nextCode, "bg-gradient-to-r from-violet-500 to-purple-500"))}
              ...
              <div className="grid gap-3 md:gap-4 lg:grid-cols-[1fr_auto_1fr]">
                <div className={`space-y-4 rounded-2xl border p-4 transition-colors ${cViolet.wrap}`}>
                  <div className="flex items-center gap-2.5"><span className={`grid h-9 w-9 place-items-center rounded-xl ${cViolet.icon}`}><Ic n="down" className="h-4 w-4" /></span><b className={`text-sm font-black ${cViolet.title}`}>از حساب مشتری</b></div>
                  ...
                  {fld("مبلغ مبدا", (<input type="text" inputMode="decimal" dir="ltr" value={convertAmount} onChange={(e) => { setConvertAmount(toNumericText(e.target.value)); setConvertErrors((p) => ({ ...p, amount: undefined, convertedAmount: undefined })); }} placeholder="0.00" className={`${uiInput} text-left tabular-nums ${convertErrors.amount ? errInput : ""}`} />))}
                </div>
                ...
                <div className={`space-y-4 rounded-2xl border p-4 transition-colors ${cEmerald.wrap}`}>
                  <div className="flex items-center gap-2.5"><span className={`grid h-9 w-9 place-items-center rounded-xl ${cEmerald.icon}`}><Ic n="up" className="h-4 w-4" /></span><b className={`text-sm font-black ${cEmerald.title}`}>به حساب مشتری</b></div>
              ...
              {convertMode === "afn" && convertForeign && (
                <div>{rateBox(cSky, "نرخ دستی در برابر افغانی", (<div><label className={uiLabel}>نرخ</label><div className="flex flex-wrap items-center gap-2.5"><span className={rateChip}>{fmt(rateUnits[convertForeign])} {labels[convertForeign]} =</span><input type="text" inputMode="decimal" dir="ltr" value={convertRate} onChange={(e) => { setConvertRate(toNumericText(e.target.value)); setConvertErrors((p) => ({ ...p, rate: undefined, convertedAmount: undefined })); }} placeholder="0" className={`h-12 w-32 md:w-44 px-3 text-left text-sm font-bold tabular-nums ${inputShell} ${convertErrors.rate ? errInput : ""}`} /><span className={rateChip}>{labels.AFN}</span></div></div>), (<>{pill(cSky.badge, convertRateValue > 0 ? `نرخ ثبت‌شده: ${afnRateLabel(convertForeign, convertRateValue)}` : "", true)}{pill(cEmerald.badge, convertedAmount ? `نتیجه: ${convertedAmount} ${labels[convertToCurrency]}` : "")}</>))}</div>
              )}
              {convertMode === "direct" && (
                <div>{rateBox(cAmber, "نرخ مستقیم جفت‌ارز", (<div className="grid items-end gap-3 md:gap-4 md:grid-cols-2">{fld("مبنای نرخ", sel(convertDirectBaseValue, (v) => { setConvertDirectBase(v as Currency); setConvertErrors((p) => ({ ...p, rate: undefined, convertedAmount: undefined })); }, [[convertFromCurrency, labels[convertFromCurrency]], [convertToCurrency, labels[convertToCurrency]]]))}<div><label className={uiLabel}>نرخ مستقیم</label><div className="flex flex-wrap items-center gap-2"><span className={rateChip}>{fmt(rateUnits[convertDirectBaseValue])} {labels[convertDirectBaseValue]} =</span><input type="text" inputMode="decimal" dir="ltr" value={convertRate} onChange={(e) => { setConvertRate(toNumericText(e.target.value)); setConvertErrors((p) => ({ ...p, rate: undefined, convertedAmount: undefined })); }} placeholder="0" className={`h-12 w-28 md:w-40 px-3 text-left text-sm font-bold tabular-nums ${inputShell} ${convertErrors.rate ? errInput : ""}`} /><span className={rateChip}>{convertDirectCounter ? labels[convertDirectCounter] : ""}</span></div></div></div>), (<>{pill(cAmber.badge, convertRateValue > 0 && convertDirectCounter ? `نرخ ثبت‌شده: ${directRateLabel(convertDirectBaseValue, convertDirectCounter, convertRateValue)}` : "", true)}{pill(cEmerald.badge, convertedAmount ? `نتیجه: ${convertedAmount} ${labels[convertToCurrency]}` : "")}</>))}</div>
              )}

              <div className="grid gap-3 md:gap-4 md:grid-cols-2">
                {fld("کارمزد", (<input type="text" inputMode="decimal" dir="ltr" value={convertCommission} onChange={(e) => { setConvertCommission(toNumericText(e.target.value)); setConvertErrors((p) => ({ ...p, commission: undefined })); }} placeholder="0" className={`${uiInput} text-left tabular-nums ${convertErrors.commission ? errInput : ""}`} />))}
              ...
              <button onClick={submitConvert} className={`flex h-[50px] md:h-[52px] w-full cursor-pointer items-center justify-center gap-2 rounded-xl bg-gradient-to-l text-base font-black shadow-lg transition-all duration-300 hover:shadow-xl hover:brightness-110 active:scale-[0.985] ${dk ? "from-violet-400 to-purple-400 text-slate-950" : "from-violet-500 via-purple-500 to-fuchsia-400 text-white"}`}>
                ثبت تبدیل
                <Ic n="arrowLeft" className="h-5 w-5 transition-transform group-hover:-translate-x-1" />
              </button>
            </section>
          ...
          <section className={`cu-up overflow-hidden ${uiCard}`} style={{ animationDelay: "160ms" }}>
            <div className="flex flex-wrap items-center gap-3 p-4 md:p-5 pb-3 md:pb-4 md:px-7 md:pt-6">
              ...
              <div className="flex-1 min-w-0">
                <h2 className={`cu-display text-xl md:text-2xl leading-none ${heading}`}>آخرین معاملات</h2>
                <p className={`mt-1 text-[11px] font-bold ${subText}`}>مشاهده، چاپ رسید و لغو معاملات</p>
              </div>
            </div>
            <div className="px-4 md:px-7 pb-4">
              <div className="relative mb-4">
                <input type="text" value={search} onChange={(e) => setSearch(e.target.value)} placeholder="جستجو بر اساس کد پیگیری، نام مشتری، مبلغ…" className={`${uiInput} pr-10`} />
                  ...
                  <table className="w-full min-w-[1100px] text-sm">
                    <thead>
                      <tr className={`border-y ${dk ? "border-slate-700 bg-slate-800/60" : "border-slate-100 bg-slate-50"}`}>
                        {["شماره", "کد پیگیری", "نام مشتری", "تاریخ (شمسی)", "نوع معامله", "دریافت", "پرداخت", "نرخ ارز", "کارمزد", "پرداخت‌کننده کارمزد", "عملیات"].map((h, i) => (
                      ...
                      {filteredTransactions.map((tx, index) => {
                        ...
                        return (
                          <tr key={tx.id} className={rowClass}>
                            ...
                            <td className={`whitespace-nowrap px-4 py-3.5 text-xs tabular-nums ${dk ? "text-slate-400" : "text-slate-500"}`} dir="ltr">{dateLabel(tx.date)}</td>
                            <td className="px-4 py-3.5">
                              <div className="flex flex-wrap items-center gap-1.5">
                                ...
                                {tx.status === "voided" && (<span className={`inline-flex rounded-full px-2.5 py-1 text-[10px] font-black ring-1 ${dk ? "bg-rose-400/10 text-rose-300 ring-rose-400/20" : "bg-rose-100 text-rose-600 ring-rose-300/60"}`}>لغو شده</span>)}
                              </div>
                            </td>
                            ...
                            <td className="px-4 py-3.5 md:px-7">
                              <details className="relative">
                                ...
                                <ul className={`cu-pop mt-2 w-44 space-y-1 rounded-xl border p-1.5 shadow-xl ${dk ? "border-slate-600 bg-slate-900 shadow-black/40" : "border-slate-200 bg-white shadow-slate-900/10"}`}>
      ...
      {selectedTransaction && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/60 p-3 md:p-4 backdrop-blur-sm" onClick={() => setSelectedTransaction(null)}>
          <div className={`cu-pop w-full max-w-lg overflow-hidden rounded-xl md:rounded-2xl border shadow-2xl ${dk ? "border-slate-600 bg-slate-900" : "border-slate-200 bg-white"}`} onClick={(e) => e.stopPropagation()}>
            ...
            <div className="max-h-[70vh] overflow-y-auto px-4 md:px-5 py-4 space-y-3">
              <div className={`flex items-center justify-between rounded-xl border p-3 ${dk ? "border-cyan-400/30 bg-cyan-400/10" : "border-sky-300 bg-sky-50"}`}>
                ...
                <span className={`inline-flex items-center gap-1.5 rounded-lg border px-3 py-1.5 text-[13px] font-black tabular-nums ${dk ? "border-cyan-400/30 bg-cyan-400/10 text-cyan-300" : "border-sky-300 bg-sky-50 text-sky-700"}`} dir="ltr">
                  <Ic n="tag" className="h-3.5 w-3.5" />
                  {selectedTransaction.trackingCode}
                </span>
              </div>
              <div className="grid grid-cols-2 gap-2 text-sm">
                <div><span className={subText}>تاریخ: </span><b dir="ltr">{dateLabel(selectedTransaction.date)}</b></div>
                <div><span className={subText}>نوع: </span><b>{transactionTypeLabel(selectedTransaction)}</b></div>
                <div className="col-span-2"><span className={subText}>مشتری: </span><b>{transactionCustomerLabel(selectedTransaction)}</b></div>
                <div><span className={subText}>دریافت: </span><b>{fmt(selectedTransaction.fromAmount)} {labels[selectedTransaction.fromCurrency]}</b></div>
                <div><span className={subText}>پرداخت: </span><b>{fmt(selectedTransaction.toAmount)} {labels[selectedTransaction.toCurrency]}</b></div>
                <div className="col-span-2"><span className={subText}>نرخ ارز: </span><b>{selectedTransaction.rateLabel}</b></div>
                <div><span className={subText}>کارمزد: </span><b>{transactionCommissionLabel(selectedTransaction)}</b></div>
                <div><span className={subText}>پرداخت‌کننده: </span><CommissionBadge payer={selectedTransaction.commissionPayer} txType={selectedTransaction.type} dk={dk} /></div>
                <div className="col-span-2"><span className={subText}>سود صرافی: </span><b className={dk ? "text-emerald-300" : "text-emerald-700"}>{transactionProfitLabel(selectedTransaction)}</b></div>
                <div className="col-span-2"><span className={subText}>وضعیت: </span>
                  <span className={selectedTransaction.status === "voided" ? dk ? "text-rose-300" : "text-rose-500" : dk ? "text-emerald-300" : "text-emerald-600"}>
                    {selectedTransaction.status === "voided" ? "لغو شده" : "فعال"}
                  </span>
                </div>
              </div>
            </div>
