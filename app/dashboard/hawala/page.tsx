...
export default function HawalaPage() {
  ...
  const confirmRegister = async () => {
    ...
    for (const cur of currencies) {
      const c = customers.find(x => x.id === hawala.senderId);
    ...
    setToast("حواله با موفقیت ثبت شد و رسید ارسال شد");
  };
  ...
  const settleHawala = async (h: Hawala) => {
    ...
    for (const cur of currencies) {
      const c = customers.find(x => x.id === h.receiverId);
  ...
  const cancelHawala = async (h: Hawala) => {
    ...
    const senderBalances: Record<string, number> = {};
    for (const cur of currencies) {
      const c = customers.find(x => x.id === h.senderId);
      senderBalances[cur] = c?.balances[cur] || 0;
    }
    await sendReceipt({
