...
function getMaxTrackingNumberFromAllSources(): number {
  ...
  try {
    let maxNum = 0;
    const txRaw = localStorage.getItem(TRANSACTIONS_KEY);
    if (txRaw) {
    ...
    const hwRaw = localStorage.getItem(HAWALAS_KEY);
    if (hwRaw) {
      const hws = JSON.parse(hwRaw);
...
export default function HawalaPage() {
  ...
  /* ✅ مقداردهی اولیه شمارنده مشترک از هر دو منبع */
  useEffect(() => {
                            ...
                            <td className="px-4 py-3.5 md:px-7">
                              <details className="relative">
                                ...
                                <ul className={`hw-up mt-2 w-44 space-y-1 rounded-xl border p-1.5 shadow-xl ${dk ? "border-slate-600 bg-slate-900 shadow-black/40" : "border-slate-200 bg-white shadow-slate-900/10"}`}>
                                  {item.status === "pending" && (
                                    <li>
                                      <button onClick={() => markAsSent(item)} className={actionButtonClass}>
                                        <Ic n="send" className="h-3.5 w-3.5" /> ارسال
                                      </button>
                                    </li>
                                  )}
                                  <li>
                                    <button onClick={() => openSettlement(item)} className={actionButtonClass}>
                                      <Ic n="check" className="h-3.5 w-3.5" /> تسویه
                                    </button>
                                  </li>
                                  <li>
                                    <button onClick={() => openCancel(item)} className={dangerActionButtonClass}>
