...
function getStoredCustomers(): Customer[] {
  ...
  try {
    const raw = localStorage.getItem(CUSTOMERS_KEY); if (!raw) return defaultCustomers;
    ...
    if (Array.isArray(parsed) && typeof parsed[0] === "string") {
      ...
      localStorage.setItem(CUSTOMERS_KEY, JSON.stringify(migrated)); return migrated;
    }
    return defaultCustomers;
  } catch { return defaultCustomers; }
}

const baseEmptyForm: FormState = { type: "", currencyFrom: "AFN", currencyTo: "USD", senderName: "", senderPhone: "", senderTelegram: "", amountFrom: "", rate: "", fee: "", feeCurrency: "AFN", feePayer: "sender", balance: "", province: "هرات", district: "گلران", receiverName: "", receiverTazkira: "", receiverPhone: "", receiverAddress: "", note: "" };
...
const normalizeDigits = (value: string) => { const pd = "۰۱۲۳۴۵۶۷۸۹"; const ad = "٠١٢٣٤٥٦٧٨٩"; return String(value || "").replace(/[۰-۹]/g, d => String(pd.indexOf(d))).replace(/[٠-٩]/g, d => String(ad.indexOf(d))); };
...
function shamsiParts(d: Date) { const parts = new Intl.DateTimeFormat("en-US-u-ca-persian-nu-latn", { year: "numeric", month: "2-digit", day: "2-digit" }).formatToParts(d); const get = (type: string) => parts.find((p) => p.type === type)?.value || "0"; return { year: get("year"), month: get("month"), day: get("day") }; }
...
function shamsiMonthLabel(d: Date) { const s = shamsiParts(d); const m = parseInt(s.month, 10); const day = parseInt(s.day, 10); if (!Number.isFinite(m) || m < 1 || m > 12 || !Number.isFinite(day)) return ""; return `${day} ${shamsiMonthNames[m - 1]} ${s.year}`; }
...
export default function HawalaPage() {
  ...
  const matchesAmountSearch = (item: Hawala, query: string) => {
    const raw = normalizeDigits(query).replace(/[,،\s]/g, ""); if (!raw) return true;
  ...
  const handleRegisterClick = () => { const newErrors = validateForm(); setErrors(newErrors); if (Object.keys(newErrors).length > 0) { showToast("لطفاً فیلدهای ضروری را خانه‌پری کنید."); return; } setPreviewOpen(true); };

  const confirmRegister = () => {
    const nowDate = new Date(); const senderName = form.senderName.trim(); const receiverName = form.receiverName.trim();
    if (senderName && !customers.some(c => c.name === senderName)) { setCustomers(prev => [...prev, { id: crypto.randomUUID(), name: senderName, phone: form.senderPhone, tazkira: "", balances: { AFN: 0, USD: 0, EUR: 0, IRR: 0, PKR: 0 } }]); }
    if (receiverName && !customers.some(c => c.name === receiverName)) { setCustomers(prev => [...prev, { id: crypto.randomUUID(), name: receiverName, phone: form.receiverPhone, tazkira: form.receiverTazkira, balances: { AFN: 0, USD: 0, EUR: 0, IRR: 0, PKR: 0 } }]); }
    let rateLabel = ""; const txRate = rateMode === "same" ? 1 : rateValue;
    ...
    setForm(createInitialForm()); setErrors({}); setPreviewOpen(false); setActiveTab("current");
  ...
  const confirmCancel = () => {
    ...
    setCancelTarget(null); showToast("حواله لغو شد و موجودی حساب به حالت قبل برگشت.");
  };
  ...
  const tabs = [{ id: "new" as const, label: "ثبت حواله جدید", icon: "plus" as IconName }, { id: "current" as const, label: "حواله‌های جاری", icon: "clock" as IconName }, { id: "history" as const, label: "تاریخچه حواله‌ها", icon: "doc" as IconName }];
          ...
          <div className={`hw-up flex gap-1.5 md:gap-2 rounded-xl md:rounded-2xl border p-1.5 md:p-2 shadow-sm backdrop-blur ${glassChip}`} style={{ animationDelay: "140ms" }}>
            {tabs.map(tab => (
              <button key={tab.id} onClick={() => setActiveTab(tab.id)} className={`flex flex-1 md:flex-initial cursor-pointer items-center justify-center md:justify-start gap-1.5 md:gap-2 rounded-lg md:rounded-xl px-3 md:px-5 py-2.5 md:py-3 text-xs md:text-sm font-black transition-all duration-300 active:scale-[0.97] ${activeTab === tab.id ? `bg-gradient-to-l shadow-lg ${dk ? "from-blue-400 to-cyan-400 text-slate-950 shadow-blue-400/25" : "from-blue-500 via-cyan-500 to-emerald-400 text-white shadow-blue-500/30"}` : dk ? "text-slate-400 hover:bg-slate-700/60 hover:text-slate-100" : "text-slate-500 hover:bg-blue-50 hover:text-slate-800"}`}>
                <Ic n={tab.icon} className="h-4 w-4" /><span className="hidden xs:inline md:inline">{tab.label}</span><span className="xs:hidden md:hidden inline">{tab.label.split(" ")[0]}</span>
              </button>
            ))}
          </div>
          ...
          {activeTab === "new" && (
            <section className={`hw-up space-y-4 md:space-y-5 p-4 md:p-7 ${uiCard}`}>
              ...
              {form.senderName && (() => { const sc = customers.find(c => c.name === form.senderName.trim()); if (!sc) return null; return (
                <div className={`rounded-xl border p-3 ${dk ? "border-blue-400/30 bg-blue-400/10" : "border-blue-200 bg-blue-50"}`}>
                  ...
                  <div className="grid grid-cols-3 md:grid-cols-5 gap-2 text-[10px] font-bold">
                    {currencies.map(c => (<div key={c} className={`rounded-lg px-2 py-1.5 ${dk ? "bg-slate-900/50" : "bg-white"}`}><div className={subText}>{labels[c]}</div><div className={`font-black tabular-nums ${dk ? "text-slate-100" : "text-slate-700"}`}>{fmt(sc.balances[c] || 0)}</div></div>))}
                  </div>
                </div>
              ); })()}
              ...
              <div className="flex flex-wrap gap-3">
                <button onClick={handleRegisterClick} className={`group flex h-[50px] md:h-[52px] flex-1 min-w-[200px] cursor-pointer items-center justify-center gap-2 rounded-xl bg-gradient-to-l text-base font-black shadow-lg transition-all duration-300 hover:shadow-xl hover:brightness-110 active:scale-[0.985] ${dk ? "from-blue-400 to-cyan-400 text-slate-950 shadow-blue-400/25" : "from-blue-500 via-cyan-500 to-emerald-400 text-white shadow-blue-500/30"}`}>ثبت حواله<Ic n="arrowLeft" className="h-5 w-5 transition-transform group-hover:-translate-x-1" /></button>
          ...
          {activeTab === "current" && (
            <section className={`hw-up overflow-hidden ${uiCard}`}>
              <div className="flex flex-wrap items-center gap-3 p-4 md:p-5 pb-3 md:pb-4 md:px-7 md:pt-6">
                ...
                <div className="flex-1 min-w-0"><h2 className={`hw-display text-xl md:text-2xl leading-none ${heading}`}>حواله‌های جاری</h2><p className={`mt-1 text-[11px] font-bold ${subText}`}>پیگیری، ارسال، تسویه و لغو حواله‌ها</p></div>
              </div>
