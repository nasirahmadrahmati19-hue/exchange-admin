...
const loadHawalas = (): Hawala[] => {
  ...
  try {
    ...
    return parsed.filter((h: any) => h && typeof h === "object" && h.id && h.number).map((h: any): Hawala => ({
      id: h.id, number: h.number, date: h.date || new Date().toISOString(), time: h.time || "",
      type: h.type === "receive" ? "receive" : "send",
      destinationCountry: h.destinationCountry || "افغانستان", province: h.province || "", district: h.district || "",
      destinationText: h.destinationText || "",
      currencyFrom: isCurrency(h.currencyFrom) ? h.currencyFrom : "AFN",
      currencyTo: isCurrency(h.currencyTo) ? h.currencyTo : "AFN",
      amountFrom: Number(h.amountFrom || 0) || 0, rate: Number(h.rate || 0) || 0, rateLabel: h.rateLabel || "",
      rateBase: h.rateBase, fee: Number(h.fee || 0) || 0,
...
const loadLastNames = (): LastNames => {
  if (typeof window === "undefined") return { senderName: "", receiverName: "" };
  try {
    const parsed = safeGetItem("hawalaLastNames");
...
export default function HawalaPage() {
  ...
  const [form, setForm] = useState<FormState>(baseEmptyForm);
  const [errors, setErrors] = useState<FormErrors>({});
  const [previewOpen, setPreviewOpen] = useState(false);
  const [theme, setTheme] = useState<"light" | "dark">("light");
  const [toast, setToast] = useState("");
  const [nextCode, setNextCode] = useState("HW-0001");
  ...
  useEffect(() => { if (mounted) safeSetItem("hawalaLastNames", lastNames); }, [lastNames, mounted]);
  useEffect(() => { if (mounted) safeSetItem(CUSTOMERS_KEY, customers); }, [customers, mounted]);
  useEffect(() => { if (mounted) safeSetItem(HAWALAS_KEY, hawalas); }, [hawalas, mounted]);
  useEffect(() => { if (mounted) setNextCode(getNextSharedCode("HW")); }, [mounted, hawalas.length]);
  ...
  const handleRegisterClick = () => {
    ...
    setErrors(newErrors);
    if (Object.keys(newErrors).length > 0) { showToast("لطفاً فیلدهای ضروری را خانه‌پری کنید."); return; }
    setPreviewOpen(true);
  };
  ...
  const confirmRegister = () => {
    try {
      ...
      if (senderName && !customers.some(c => c.name === senderName)) {
        setCustomers(prev => [...prev, { id: generateId(), name: senderName, phone: form.senderPhone, tazkira: "", balances: { AFN: 0, USD: 0, EUR: 0, IRR: 0, PKR: 0 } }]);
      }
      if (receiverName && !customers.some(c => c.name === receiverName)) {
        setCustomers(prev => [...prev, { id: generateId(), name: receiverName, phone: form.receiverPhone, tazkira: form.receiverTazkira, balances: { AFN: 0, USD: 0, EUR: 0, IRR: 0, PKR: 0 } }]);
      }
      let rateLabel = "";
  ...
  const resetForm = () => { setForm(baseEmptyForm); setErrors({}); showToast("فورم پاک شد."); };
  ...
  const actionButtonClass = `flex w-full cursor-pointer items-center gap-2 rounded-lg px-3 py-2 text-right text-xs font-bold transition-colors ${dk ? "text-slate-300 hover:bg-blue-400/10 hover:text-blue-300" : "text-slate-600 hover:bg-sky-50 hover:text-sky-700"}`;
    ...
    <div dir="rtl" className={dk ? "dark" : ""}>
      ...
      <div className={`hw-font relative min-h-screen overflow-x-hidden antialiased transition-colors duration-500 ${dk ? "bg-[#0f172a] text-slate-100" : "bg-[#eef6fa] text-slate-800"}`}>
        ...
        <div className="relative z-10 mx-auto w-full max-w-7xl space-y-4 md:space-y-6 px-3 pb-16 pt-5 md:px-8 md:pt-9">
          ...
          <header className="hw-up flex flex-wrap items-center justify-between gap-3">
            ...
            <div className="flex items-center gap-1.5 md:gap-2.5">
              <div className={`hidden sm:flex items-center gap-2 rounded-xl border px-3 py-2 shadow-sm backdrop-blur ${glassChip}`}>
                ...
                {now && (<><span className={`h-4 w-px ${dk ? "bg-slate-600" : "bg-slate-200"}`} /><span className={`whitespace-nowrap text-[10px] font-black ${dk ? "text-cyan-300" : "text-blue-600"}`}>{shamsiMonthLabel(now)}</span></>)}
              </div>
          ...
          <div className={`hw-up flex gap-1.5 md:gap-2 rounded-xl md:rounded-2xl border p-1.5 md:p-2 shadow-sm backdrop-blur ${glassChip}`} style={{ animationDelay: "140ms" }}>
            {tabs.map(tab => (
              <button key={tab.id} onClick={() => setActiveTab(tab.id)} className={`flex flex-1 cursor-pointer items-center justify-center gap-1.5 md:gap-2 rounded-lg md:rounded-xl px-3 md:px-5 py-2.5 md:py-3 text-xs md:text-sm font-black transition-all duration-300 active:scale-[0.97] ${activeTab === tab.id ? `bg-gradient-to-l shadow-lg ${dk ? "from-blue-400 to-cyan-400 text-slate-950" : "from-blue-500 via-cyan-500 to-emerald-400 text-white"}` : dk ? "text-slate-400 hover:bg-slate-700/60 hover:text-slate-100" : "text-slate-500 hover:bg-blue-50 hover:text-slate-800"}`}>
            ...
            <section className={`hw-up space-y-4 md:space-y-5 p-4 md:p-7 ${uiCard}`}>
              ...
              <div className={`rounded-2xl border p-4 ${dk ? "border-slate-600 bg-slate-900/50" : "border-slate-200 bg-slate-50"}`}>
                ...
                <div className="grid gap-3 md:gap-4 sm:grid-cols-2 lg:grid-cols-3">
                  {fld("کد پیگیری", (
                    <div className="relative">
                      <input readOnly dir="ltr" value={nextCode} className={`${uiInput} ${roInput} pl-14 text-left tabular-nums font-black text-[15px]`} />
              ...
              <div className={`rounded-2xl border p-4 ${dk ? "border-slate-600 bg-slate-900/50" : "border-slate-200 bg-slate-50"}`}>
                ...
                {rateMode === "afn" && afnForeign && (
                  <div className="mt-4">{rateBox(cBlue, "نرخ دستی در برابر افغانی", (
                ...
                {rateMode === "direct" && (
                  <div className="mt-4">{rateBox(cAmber, "نرخ مستقیم جفت‌ارز", (
              ...
              <div className="flex flex-wrap gap-3">
                <button onClick={handleRegisterClick} className={`flex h-[50px] md:h-[52px] flex-1 min-w-[200px] cursor-pointer items-center justify-center gap-2 rounded-xl bg-gradient-to-l text-base font-black shadow-lg transition-all duration-300 hover:shadow-xl hover:brightness-110 active:scale-[0.985] ${dk ? "from-blue-400 to-cyan-400 text-slate-950" : "from-blue-500 via-cyan-500 to-emerald-400 text-white"}`}>
                  ثبت حواله
            ...
            <section className={`hw-up overflow-hidden ${uiCard}`}>
              ...
              <div className="px-4 md:px-7 pb-4 space-y-4">
                ...
                {currentHawalas.length === 0 ? (
                  ...
                  <div className="overflow-x-auto hw-scroll">
                          ...
                          <tr key={item.id} className={`transition-colors ${dk ? "hover:bg-slate-700/30" : "hover:bg-blue-50/70"}`}>
                            ...
                            <td className="px-4 py-3.5 md:px-7">
                              <details className="relative">
                                ...
                                <ul className={`hw-pop mt-2 w-44 space-y-1 rounded-xl border p-1.5 shadow-xl ${dk ? "border-slate-600 bg-slate-900 shadow-black/40" : "border-slate-200 bg-white shadow-slate-900/10"}`}>
                                  {item.status === "pending" && (
                                    <li><button onClick={() => markAsSent(item)} className={actionButtonClass}><Ic n="send" className="h-3.5 w-3.5" /> ارسال</button></li>
                                  )}
                                  <li><button onClick={() => openSettlement(item)} className={actionButtonClass}><Ic n="check" className="h-3.5 w-3.5" /> تسویه</button></li>
                                  <li><button onClick={() => openCancel(item)} className={dangerActionButtonClass}><Ic n="xCircle" className="h-3.5 w-3.5" /> لغو</button></li>
                                </ul>
                              </details>
                            </td>
                          </tr>
      ...
      {previewOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/60 p-3 md:p-4 backdrop-blur-sm" onClick={() => setPreviewOpen(false)}>
          <div className={`hw-pop w-full max-w-2xl overflow-hidden rounded-xl md:rounded-2xl border shadow-2xl ${dk ? "border-slate-600 bg-slate-900" : "border-slate-200 bg-white"}`} onClick={(e) => e.stopPropagation()}>
      ...
      {settleTarget && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/60 p-3 md:p-4 backdrop-blur-sm" onClick={() => setSettleTarget(null)}>
          <div className={`hw-pop w-full max-w-lg overflow-hidden rounded-xl md:rounded-2xl border shadow-2xl ${dk ? "border-slate-600 bg-slate-900" : "border-slate-200 bg-white"}`} onClick={(e) => e.stopPropagation()}>
            <div className={`flex items-center justify-between border-b px-4 md:px-5 py-3 md:py-4 ${dk ? "border-slate-700 bg-slate-800/60" : "border-slate-100 bg-slate-50"}`}>
