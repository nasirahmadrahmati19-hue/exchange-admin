...
function getSharedCounter(): number {
  ...
  try {
    const v = localStorage.getItem(SHARED_COUNTER_KEY);
...
// ✅ خواندن هر دو منبع برای محاسبه ماکزیمم
...
const loadHawalas = (): Hawala[] => {
  ...
  try {
    ...
    return parsed.filter((h: any) => h && typeof h === "object" && h.id && h.number).map((h: any): Hawala => ({
      id: h.id, number: h.number, date: h.date || new Date().toISOString(), time: h.time || "", type: h.type || "send",
      destinationCountry: h.destinationCountry || "افغانستان", province: h.province || "", district: h.district || "",
      destinationText: h.destinationText || "", currencyFrom: h.currencyFrom || "AFN", currencyTo: h.currencyTo || "AFN",
      amountFrom: Number(h.amountFrom || 0) || 0, rate: Number(h.rate || 0) || 0, rateLabel: h.rateLabel || "",
      rateBase: h.rateBase, fee: Number(h.fee || 0) || 0, feeCurrency: h.feeCurrency || "AFN",
      feePayer: h.feePayer === "receiver" ? "receiver" : "sender", finalAmount: Number(h.finalAmount || 0) || 0,
      balance: h.balance || "", note: h.note || "", profit: Number(h.profit || 0) || 0, profitCurrency: h.profitCurrency || "AFN",
      senderName: h.senderName || "", senderPhone: h.senderPhone || "", senderTelegram: h.senderTelegram || "",
      receiverName: h.receiverName || "", receiverTazkira: h.receiverTazkira || "", receiverPhone: h.receiverPhone || "",
      receiverAddress: h.receiverAddress || "", status: h.status || "pending", paidAt: h.paidAt, paidBy: h.paidBy,
      paidAmount: h.paidAmount, cancelReason: h.cancelReason,
    }));
  } catch { return []; }
};
...
export default function HawalaPage() {
  ...
  useEffect(() => {
    if (mounted) setNextCode(getNextSharedCode("HW"));
  }, [mounted, hawalas.length]);
  ...
  const actionButtonClass = `flex w-full cursor-pointer items-center gap-2 rounded-lg px-3 py-2 text-right text-xs font-bold transition-colors disabled:cursor-not-allowed disabled:opacity-35 ${dk ? "text-slate-300 hover:bg-blue-400/10 hover:text-blue-300" : "text-slate-600 hover:bg-sky-50 hover:text-sky-700"}`;
                          ...
                          <tr key={item.id} className={`transition-colors ${dk ? "hover:bg-slate-700/30" : "hover:bg-blue-50/70"}`}>
                            ...
                            <td className="px-4 py-3.5 md:px-7">
                              <details className="relative">
                                ...
                                <ul className={`hw-up mt-2 w-44 space-y-1 rounded-xl border p-1.5 shadow-xl ${dk ? "border-slate-600 bg-slate-900 shadow-black/40" : "border-slate-200 bg-white shadow-slate-900/10"}`}>
                                  {item.status === "pending" && (
