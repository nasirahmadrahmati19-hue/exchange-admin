...
// ✅ تابع کمکی برای دسترسی امن به labels با cast
function getCurrencyLabel(cur: any): string {
  if (!cur) return "—";
  return labels[cur as Currency] || String(cur);
}
...
// ✅ Build ledger - با استفاده از getCurrencyLabel برای جلوگیری از خطای TS
  ...
  for (const tx of transactions) {
    ...
    if (tx.type === "exchange" && tx.customerId) {
      entries.push({
        ...
        currency: fromCur, amount: tx.fromAmount, direction: "out",
      ...
      entries.push({
        ...
        currency: toCur, amount: tx.toAmount, direction: "in",
        balanceAfter: 0, referenceId: tx.id, referenceNumber: refNum,
      });
      if (tx.commission && tx.commission > 0 && commCur) {
        entries.push({
          id: `${tx.id}-fee`, date, customerId: tx.customerId, type: "fee",
          description: `کارمزد معامله`,
          currency: commCur, amount: tx.commission, direction: "out",
          balanceAfter: 0, referenceId: tx.id, referenceNumber: refNum,
        });
      }
    }
    ...
    if (tx.type === "transfer") {
      if (tx.senderId) {
        ...
        if (tx.commissionPayer === "sender" && tx.commission && tx.commission > 0 && commCur) {
          entries.push({
      ...
      if (tx.receiverId) {
        ...
        if (tx.commissionPayer === "receiver" && tx.commission && tx.commission > 0 && commCur) {
          entries.push({
  ...
  for (const h of hawalas) {
    ...
    if (sender) {
      entries.push({
        ...
        currency: hFromCur, amount: h.amountFrom, direction: "out",
      ...
      if (h.feePayer === "sender" && h.fee > 0) {
        entries.push({
          ...
          currency: hFeeCur, amount: h.fee, direction: "out",
          balanceAfter: 0, referenceId: h.id, referenceNumber: refNum,
        });
      }
    }
    if (receiver && h.status === "paid") {
      entries.push({
        ...
        currency: hToCur, amount: h.finalAmount, direction: "in",
        balanceAfter: 0, referenceId: h.id, referenceNumber: refNum,
      });
...
export default function CustomersPage() {
  ...
  const [transactions, setTransactions] = useState<any[]>(getStoredTransactions);
  const [hawalas, setHawalas] = useState<any[]>(getStoredHawalas);
