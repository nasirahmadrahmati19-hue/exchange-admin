...
function buildLedger(customers: Customer[], transactions: any[], hawalas: any[], cashEntries: any[]): LedgerEntry[] {
  ...
  for (const tx of transactions) {
    ...
    if (tx.type === "exchange") {
      ...
      if (cid && cid !== CASH_BOX_ID && isCurrency(fromCur) && isCurrency(toCur)) {
    ...
    if (tx.type === "transfer") {
      ...
      if (sId && sId !== CASH_BOX_ID && isCurrency(fromCur)) {
      ...
      if (rId && rId !== CASH_BOX_ID && isCurrency(toCur)) {
  ...
  for (const h of hawalas) {
    ...
    if (sender && sender.id !== CASH_BOX_ID && isCurrency(hFromCur)) {
    ...
    if (receiver && receiver.id !== CASH_BOX_ID && h.status === "paid" && isCurrency(hToCur)) {
...
export default function CustomersPage() {
  ...
  const printStatement = () => {
    ...
    try {
      ...
      const title = isCashBox ? "صورت‌حساب صندوق صرافی" : `صورت‌حساب ${selectedCustomer.name}`;
      const customerInfo = isCashBox ? "موجودی صندوق صرافی" : `تلفن: ${selectedCustomer.phone || "-"} | تذکره: ${selectedCustomer.tazkira || "-"} | تلگرام: ${selectedCustomer.telegram || "-"}`;
                          ...
                          <div key={c.id} className={`rounded-2xl border p-4 ${glassCard} ${isCashBoxRow ? (dk ? "border-emerald-400/30" : "border-emerald-200") : ""}`}>
                            <div className="flex items-start gap-3">
                              ...
                              <div className="flex-1 min-w-0">
                                ...
                                {isCashBoxRow && <span className={`mr-2 text-[9px] font-black ${dk ? "text-emerald-300" : "text-emerald-600"}`}>موجودی صندوق صرافی</span>}
                                <div className={`text-[11px] ${subText} mt-1 space-y-0.5`}>
                                  ...
                                  {isCashBoxRow && <div>💰 موجودی صندوق صرافی</div>}
                                </div>
                              </div>
                            </div>
                            ...
                            return (
                              <tr key={c.id} className={`transition-colors ${dk ? "hover:bg-slate-700/30" : "hover:bg-emerald-50/70"} ${isCashBoxRow ? (dk ? "bg-emerald-400/[0.03]" : "bg-emerald-50/30") : ""}`}>
                                ...
                                <td className="px-4 py-3.5 text-center align-middle">
                                  <div className={`text-[13px] font-black ${dk ? "text-slate-100" : "text-slate-800"}`}>{isCashBoxRow && <span className="ml-1">💰</span>}{c.name}{isCashBoxRow && <span className={`mr-2 text-[9px] font-black ${dk ? "text-emerald-300" : "text-emerald-600"}`}>صندوق صرافی</span>}</div>
                                  {!isCashBoxRow && c.address && <div className={`text-[10px] mt-1 ${subText}`}>📍 {c.address}</div>}
                                  {isCashBoxRow && <div className={`text-[10px] mt-1 ${subText}`}>موجودی صندوق صرافی</div>}
                                </td>
                                <td className="px-4 py-3.5 text-center align-middle">
            ...
            <section className="cu-up space-y-4 md:space-y-5">
              <div className={`relative overflow-hidden rounded-2xl border p-5 md:p-7 ${uiCard}`}>
                <div className="relative">
                  ...
                  <div className="flex flex-wrap items-start gap-4 md:gap-6">
                    ...
                    <div className="flex-1 min-w-0">
                      <h2 className={`cu-display text-2xl md:text-3xl leading-none ${heading}`}>{selectedCustomer.name}{isCashBox && <span className={`mr-2 text-sm font-black ${dk ? "text-emerald-300" : "text-emerald-600"}`}>(موجودی صندوق صرافی)</span>}</h2>
                      <div className={`grid grid-cols-2 md:grid-cols-4 gap-x-4 gap-y-1 text-xs mt-2 ${subText}`}>
                        ...
                        {isCashBox && <div className="md:col-span-3"><b>توضیحات:</b> <span className="font-black">موجودی صندوق صرافی - شامل تمام دریافتی‌ها و پرداختی‌های نقدی</span></div>}
                      </div>
                    </div>
              ...
              <div className={`rounded-2xl border p-4 md:p-5 ${uiCard}`}>
                <div className="flex items-center gap-2 mb-3">
                  ...
                  <b className={`text-sm font-black ${heading}`}>{isCashBox ? "موجودی صندوق" : "موجودی فعلی"}</b>
                  ...
                  {isCashBox ? (
                    <div className={`rounded-xl border p-4 ${dk ? "border-emerald-400/25 bg-emerald-400/[0.07]" : "border-emerald-200 bg-emerald-50"}`}>
                      <div className="flex items-center gap-3 mb-3">
                        ...
                        <div><b className={`block text-sm font-black ${dk ? "text-emerald-300" : "text-emerald-700"}`}>💰 صندوق صرافی</b><span className={`text-[10px] font-bold ${subText}`}>این بخش قابل ویرایش نیست</span></div>
                      </div>
                      <p className={`text-sm leading-6 ${dk ? "text-slate-300" : "text-slate-600"}`}>صندوق نشان‌دهنده موجودی نقدی صرافی است. تمام دریافتی‌ها و پرداختی‌های نقدی در این بخش ثبت می‌شوند. برای مشاهده جزئیات، به تب‌های <b>موجودی</b>، <b>روزنامچه</b> و <b>صورت‌حساب</b> مراجعه کنید.</p>
                    </div>
                  ) : (
                    <>
                      <div className="grid gap-4 md:grid-cols-2">
                  ...
                  <div className="grid gap-3 md:grid-cols-2 mb-4">
                    <div className={`rounded-xl border p-4 ${dk ? "border-slate-700 bg-slate-800/60" : "border-slate-200 bg-white"}`}>
                      ...
                      <div className={`space-y-1 text-xs mt-2 ${dk ? "text-slate-300" : "text-slate-600"}`}>
                        ...
                        {isCashBox && <div><b>توضیحات:</b> موجودی صندوق صرافی</div>}
                      </div>
                    </div>
                    <div className={`rounded-xl border p-4 ${dk ? "border-slate-700 bg-slate-800/60" : "border-slate-200 bg-white"}`}>
