...
// ✅ اصلاح قطعی مسیر: بازگشت به ریشه پروژه و سپس ورود به پوشه lib
...
function syncCashEntriesForExchange(action: "add" | "remove" | "replace", tx: Transaction | null, oldTxId?: string) {
  ...
  if ((action === "add" || action === "replace") && tx) {
    ...
    newEntries.push({
      id: newId(), trackingCode: `${tx.trackingCode}-EX-IN`, date: dateStr, type: "exchange_account_in",
      currency: tx.fromCurrency, amount: tx.fromAmount, direction: "in", reason: "تبادل ارز - دریافت ارز توسط حساب صرافی",
      balanceAfter: 0, customerId: EXCHANGE_ACCOUNT_ID, customerName: EXCHANGE_ACCOUNT_NAME, linkedExchangeId: tx.id, status: "active"
    });
    newEntries.push({
      id: newId(), trackingCode: `${tx.trackingCode}-EX-OUT`, date: dateStr, type: "exchange_account_out",
      currency: tx.toCurrency, amount: tx.toAmount, direction: "out", reason: "تبادل ارز - پرداخت ارز توسط حساب صرافی",
      balanceAfter: 0, customerId: EXCHANGE_ACCOUNT_ID, customerName: EXCHANGE_ACCOUNT_NAME, linkedExchangeId: tx.id, status: "active"
    });
    if (tx.commission && tx.commission > 0 && tx.commissionCurrency) {
      newEntries.push({
        id: newId(), trackingCode: `${tx.trackingCode}-FEE`, date: dateStr, type: "exchange_account_in",
        currency: tx.commissionCurrency, amount: tx.commission, direction: "in", reason: "تبادل ارز - کارمزد دریافتی صرافی",
        balanceAfter: 0, customerId: EXCHANGE_ACCOUNT_ID, customerName: EXCHANGE_ACCOUNT_NAME, linkedExchangeId: tx.id, status: "active"
      });
    }
...
export default function CurrencyExchangePage() {
  ...
  const selectedCustomer = useMemo(() => { if (isCustomerExchangeAccount) return EXCHANGE_ACCOUNT_CUSTOMER; return customers.find(c => c.name === customer) || null; }, [customers, customer, isCustomerExchangeAccount]);
  const selectedSender = useMemo(() => { if (isSenderExchangeAccount) return EXCHANGE_ACCOUNT_CUSTOMER; return customers.find(c => c.name === sender) || null; }, [customers, sender, isSenderExchangeAccount]);
  const selectedReceiver = useMemo(() => { if (isReceiverExchangeAccount) return EXCHANGE_ACCOUNT_CUSTOMER; return customers.find(c => c.name === receiver) || null; }, [customers, receiver, isReceiverExchangeAccount]);
  const selectedConvertCustomer = useMemo(() => { if (isConvertCustomerExchangeAccount) return EXCHANGE_ACCOUNT_CUSTOMER; return customers.find(c => c.name === convertCustomer) || null; }, [customers, convertCustomer, isConvertCustomerExchangeAccount]);
