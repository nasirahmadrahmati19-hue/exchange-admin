...
const EXCHANGE_ACCOUNT_CUSTOMER: Customer = { 
  id: EXCHANGE_ACCOUNT_ID, name: EXCHANGE_ACCOUNT_NAME, phone: "", telegram: "", telegramChatId: "", 
  balances: { AFN: 0, USD: 0, EUR: 0, IRR: 0, PKR: 0 } 
};

const normalizeDigits = (s: string) => 
  String(s || "").replace(/[۰-۹]/g, d => String("۰۱۲۳۴۵۶۷۸۹".indexOf(d))).replace(/[٠-٩]/g, d => String("٠١٢٣٤٥٦٧٨٩".indexOf(d)));
...
export default function CurrencyExchangePage() {
        ...
        if (settings.enabled && settings.botToken) {
          ...
          for (const custId of recipientIds) {
            ...
            if (tx.type === "exchange" && tx.customerId === custId) {
              ...
              if (tx.senderId === custId) { changeAmount = -tx.fromAmount; changeCurrency = tx.fromCurrency; } 
              else if (tx.receiverId === custId) { changeAmount = tx.toAmount; changeCurrency = tx.toCurrency; }
  ...
  const displayTransactions = useMemo(() => transactions.filter(t => t.status !== "voided"), [transactions]);
