...
type SettlementType = "cash" | "account"; // ✅ جدید: نوع تسویه
...
type Transaction = {
  ...
  settlementType?: SettlementType; // ✅ جدید
};
...
function settlementLabel(s?: SettlementType) { return s === "cash" ? "💵 نقدی" : s === "account" ? "🏦 از حساب" : "-"; }
...
// ✅ اصلاح شده: پشتیبانی از معاملات نقدی (بدون تغییر در حساب مشتری)
function getBalanceChangesForTransaction(tx: Transaction, action: "register" | "reverse"): BalanceChange[] {
  ...
  if (tx.type === "exchange" && tx.customerId) {
    // ✅ فقط اگر تسویه از "حساب" باشد، موجودی مشتری تغییر می‌کند
    if (tx.settlementType === "account") {
    ...
    // اگر settlementType === "cash" باشد، هیچ تغییری در بالانس مشتری ایجاد نمی‌شود
  }
...
// ✅ اصلاح شده: همگام‌سازی با صندوق برای معاملات نقدی
function syncCashEntriesForExchange(action: "add" | "remove" | "replace", tx: Transaction | null, oldTxId?: string) {
  ...
  if ((action === "add" || action === "replace") && tx) {
    ...
    // ✅ اگر تسویه نقدی باشد، یا مشتری خودش صندوق باشد، عملیات فیزیکی است
    const isPhysicalCash = tx.settlementType === "cash";
    const isCashBoxCustomer = tx.customerId === CASH_BOX_ID || isCashBoxName(tx.customerName);
    const isCashTransaction = isPhysicalCash || isCashBoxCustomer;

    if (isCashTransaction) {
      newEntries.push({
        id: newId(), trackingCode: `${tcBase}-IN`, date: dateStr,
        type: isCashBoxCustomer ? "owner_deposit" : "customer_deposit",
        currency: tx.fromCurrency, amount: tx.fromAmount, direction: "in",
        reason: isCashBoxCustomer ? `تبادل ارز - دریافت اسکناس از صندوق` : `تبادل ارز نقدی - دریافت از ${tx.customerName || "مشتری"}`,
        balanceAfter: 0, customerId: isCashBoxCustomer ? undefined : tx.customerId,
        customerName: isCashBoxCustomer ? undefined : tx.customerName,
        customerPhone: isCashBoxCustomer ? undefined : tx.customerPhone,
        linkedExchangeId: tx.id,
      });
      newEntries.push({
        id: newId(), trackingCode: `${tcBase}-OUT`, date: dateStr,
        type: isCashBoxCustomer ? "owner_withdraw" : "customer_withdraw",
        currency: tx.toCurrency, amount: tx.toAmount, direction: "out",
        reason: isCashBoxCustomer ? `تبادل ارز - پرداخت اسکناس از صندوق` : `تبادل ارز نقدی - پرداخت به ${tx.customerName || "مشتری"}`,
        balanceAfter: 0, customerId: isCashBoxCustomer ? undefined : tx.customerId,
        customerName: isCashBoxCustomer ? undefined : tx.customerName,
        customerPhone: isCashBoxCustomer ? undefined : tx.customerPhone,
        linkedExchangeId: tx.id,
      ...
      if (tx.commission && tx.commission > 0 && tx.commissionCurrency) {
        newEntries.push({
          id: newId(), trackingCode: `${tcBase}-FEE`, date: dateStr, type: "fee",
          currency: tx.commissionCurrency, amount: tx.commission, direction: "in",
          reason: `تبادل ارز نقدی - کارمزد دریافتی از ${tx.customerName || "مشتری"}`, balanceAfter: 0,
          customerId: isCashBoxCustomer ? undefined : tx.customerId,
          customerName: isCashBoxCustomer ? undefined : tx.customerName,
          linkedExchangeId: tx.id,
        });
      }
  ...
  if ((action === "add" || action === "replace") && tx) {
    ...
    if (tx.commission && tx.commission > 0 && tx.commissionCurrency) {
      newEntries.push({
        id: newId(), trackingCode: `${tcBase}-FEE`, date: dateStr, type: "fee",
        currency: tx.commissionCurrency, amount: tx.commission, direction: "in",
        reason: `انتقال - کارمزد صرافی`, balanceAfter: 0,
        linkedTransferId: tx.id,
      });
    }
    entries = [...entries, ...newEntries];
  ...
  if ((action === "add" || action === "replace") && tx) {
    ...
    if (tx.commission && tx.commission > 0 && tx.commissionCurrency) {
      newEntries.push({
        id: newId(), trackingCode: `${tcBase}-FEE`, date: dateStr, type: "fee",
        currency: tx.commissionCurrency, amount: tx.commission, direction: "in",
        reason: `تبدیل ارز - کارمزد صرافی`, balanceAfter: 0,
        linkedConvertId: tx.id,
      });
    }
    entries = [...entries, ...newEntries];
...
const iconPaths = {
  ...
  cash: "M2.25 18 9 11.25l4.306 4.306a11.95 11.95 0 0 1 5.814-5.518l2.74-1.22m0 0-5.94-2.281m5.94 2.28-2.28 5.941",
...
export default function CurrencyExchangePage() {
  ...
  const [settlementType, setSettlementType] = useState<SettlementType>("cash"); // ✅ جدید
  ...
  // Auto-calculate receiver amount
  useEffect(() => {
  ...
  const validateExchange = useCallback((): ExchangeFormErrors => {
    ...
    // ✅ اگر تسویه نقدی است، مشتری اختیاری است (شخص بیرونی)
    if (settlementType === "account" && !customer) e.customer = "مشتری خالی است.";
    const a = parseAmount(receivedAmount);
    ...
    return e;
  }, [exchangeDealType, customer, receivedAmount, exchangeMode, rate, exchangeDirectCounter, paidAmount, exchangeCommission, settlementType]);
  ...
  const submitExchange = useCallback(() => {
    ...
    const tx: Transaction = {
      ...
      customerId: selC?.id, customerName: customer || (settlementType === "cash" ? "مشتری بیرونی" : ""),
    ...
    setPreviewData(tx);
    setPreviewOpen(true);
  }, [validateExchange, exchangeFromAmount, exchangeToAmount, exchangeMode, exchangeRateValue, exchangeForeign, exchangeDirectCounter, exchangeDirectBaseValue, exchangeDescription, exchangeCommissionValue, exchangeCommissionCurrency, editingExchangeId, transactions, exchangeDealType, customer, receivedCurrency, paidCurrency, exchangeCommissionPayer, customerPhone, customerTelegram, customers, settlementType]);
  ...
  const editTransaction = useCallback((tx: Transaction) => {
    ...
    if (tx.type === "exchange") {
      ...
      setSettlementType(tx.settlementType || "cash"); // ✅ جدید
        ...
        <div className="relative z-10 mx-auto w-full max-w-7xl space-y-4 md:space-y-6 px-3 pb-16 pt-5 md:px-8 md:pt-9">
          ...
          {tab === "exchange" && (
            <section className={`space-y-4 p-4 md:p-7 ${uiCard}`}>
              ...
              {selectedCustomer && !isCustomerCashBox && settlementType === "account" && <CustomerBalanceCard customer={selectedCustomer} color="cyan" isCashBox={false} />}
              ...
              <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
                ...
                {/* ✅ جدید: انتخاب نوع تسویه */}
                {fld("نوع تسویه *", (
                  <div className={`flex rounded-xl border p-1 ${dk ? "border-slate-600 bg-slate-900" : "border-slate-200 bg-slate-50"}`}>
                    <button
                      type="button"
                      onClick={() => { setSettlementType("cash"); setExchangeErrors(p => ({ ...p, customer: undefined })); }}
                      className={`flex-1 rounded-lg px-3 py-2.5 text-xs font-black transition-all ${settlementType === "cash" ? (dk ? "bg-emerald-400 text-slate-950" : "bg-emerald-500 text-white") : (dk ? "text-slate-400 hover:text-slate-200" : "text-slate-500 hover:text-slate-700")}`}
                    >
                      💵 نقدی
                    </button>
                    <button
                      type="button"
                      onClick={() => { setSettlementType("account"); setExchangeErrors(p => ({ ...p, customer: undefined })); }}
                      className={`flex-1 rounded-lg px-3 py-2.5 text-xs font-black transition-all ${settlementType === "account" ? (dk ? "bg-sky-400 text-slate-950" : "bg-sky-500 text-white") : (dk ? "text-slate-400 hover:text-slate-200" : "text-slate-500 hover:text-slate-700")}`}
                    >
                      🏦 از حساب
                    </button>
                  </div>
              ...
              {/* ✅ هشدار هوشمند برای حالت نقدی */}
              {settlementType === "cash" && (
                <div className={`flex items-start gap-3 rounded-xl border p-3 text-xs font-bold ${dk ? "border-amber-400/30 bg-amber-400/10 text-amber-300" : "border-amber-300 bg-amber-50 text-amber-800"}`}>
                  <Ic n="info" className="h-4 w-4 shrink-0 mt-0.5" />
                  <div>
                    <b>حالت نقدی (فیزیکی) فعال است.</b>
                    <p className={`mt-1 text-[11px] font-normal ${subText}`}>
                      در این حالت، اسکناس فیزیکی دریافت و پرداخت می‌شود. موجودی حساب مشتری تغییری نمی‌کند و مبلغ مستقیماً به موجودی فیزیکی صندوق صرافی اضافه/کم می‌شود. نام مشتری اختیاری است (می‌تواند شخص بیرونی باشد).
                    </p>
                  </div>
                </div>
              )}
              ...
              {/* ✅ فیلد مشتری - در حالت نقدی اختیاری */}
              <div className="grid gap-3 md:grid-cols-2">
                {fld(`مشتری ${settlementType === "cash" ? "(اختیاری)" : "*"}`, (
                  <div className="relative" ref={customerListRef}>
                    ...
                    }} placeholder={settlementType === "cash" ? "نام مشتری یا شخص بیرونی (اختیاری)…" : "انتخاب از لیست یا نوشتن نام جدید…"} className={`${uiInput} pl-12 ${exchangeErrors.customer ? errInput : ""}`} autoComplete="off" />
          ...
          {tab === "transfer" && (
            <section className={`space-y-4 p-4 md:p-7 ${uiCard}`}>
              ...
              <div className="grid gap-3 md:grid-cols-2">
                <CustomerBalanceCard customer={selectedSender} color="orange" isCashBox={isSenderCashBox} />
                <CustomerBalanceCard customer={selectedReceiver} color="cyan" isCashBox={isReceiverCashBox} />
              </div>
              <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
          ...
          {tab === "convert" && (
            <section className={`space-y-4 p-4 md:p-7 ${uiCard}`}>
              ...
              <CustomerBalanceCard customer={selectedConvertCustomer} color="violet" isCashBox={isConvertCustomerCashBox} />
              <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/60 p-3" onClick={() => setSelectedTransaction(null)}>
          <div className={`w-full max-w-lg rounded-xl border shadow-2xl ${dk ? "border-slate-600 bg-slate-900" : "border-slate-200 bg-white"}`} onClick={e => e.stopPropagation()}>
            ...
            <div className="max-h-[70vh] overflow-y-auto px-4 py-2">
              ...
              {selectedTransaction.type === "exchange" && <DetailRow dark={dk} label="نوع تسویه" value={settlementLabel(selectedTransaction.settlementType)} />}
              ...
              {previewData.type === "exchange" && (
                <div className={`rounded-xl border p-4 ${dk ? "border-emerald-400/20 bg-emerald-400/5" : "border-emerald-200 bg-emerald-50/50"}`}>
                  <div className="flex items-center gap-2 mb-3">
                    <Ic n={previewData.settlementType === "cash" ? "cash" : "bank"} className={`h-4 w-4 ${dk ? "text-emerald-300" : "text-emerald-600"}`} />
                    <b className={`text-sm font-black ${dk ? "text-emerald-300" : "text-emerald-700"}`}>
                      نوع تسویه: {settlementLabel(previewData.settlementType)}
                    </b>
                  </div>
                  <p className={`text-xs ${subText}`}>
                    {previewData.settlementType === "cash"
                      ? "💵 معامله نقدی (فیزیکی) است. موجودی حساب مشتری تغییر نمی‌کند و اسکناس مستقیماً به صندوق صرافی وارد/خارج می‌شود."
                      : "🏦 معامله از حساب مشتری است. موجودی حساب مشتری به‌روزرسانی می‌شود."}
                  </p>
                </div>
              )}
                ...
                <div className={`rounded-xl border p-4 ${dk ? "border-cyan-400/20 bg-cyan-400/5" : "border-cyan-200 bg-cyan-50/50"}`}>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-sm">
                    <div>
                      ...
                      <b className={dk ? "text-slate-100" : "text-slate-800"}>{previewData.customerName || "مشتری بیرونی"}</b>
                      ...
                      {!previewData.customerId && previewData.settlementType === "cash" && <span className={`mr-2 text-[10px] font-black ${dk ? "text-amber-300" : "text-amber-600"}`}>⚠️ شخص بیرونی (نقدی)</span>}
                    </div>
