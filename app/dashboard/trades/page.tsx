...
// ✅ اصلاح: تشخیص صندوق و استفاده از نوع درست
function syncCashEntriesForExchange(action: "add" | "remove" | "replace", tx: Transaction | null, oldTxId?: string) {
  ...
  if ((action === "add" || action === "replace") && tx) {
    ...
    newEntries.push({
      ...
      reason: isCashBox ? `تبادل ارز - دریافت اسکناس از صندوق` : `تبادل ارز - دریافت اسکناس از مشتری`,
      balanceAfter: 0, customerId: isCashBox ? undefined : tx.customerId,
      customerName: isCashBox ? undefined : tx.customerName,
      customerPhone: isCashBox ? undefined : tx.customerPhone,
    ...
    newEntries.push({
      ...
      reason: isCashBox ? `تبادل ارز - پرداخت اسکناس از صندوق` : `تبادل ارز - پرداخت اسکناس به مشتری`,
      balanceAfter: 0, customerId: isCashBox ? undefined : tx.customerId,
      customerName: isCashBox ? undefined : tx.customerName,
      customerPhone: isCashBox ? undefined : tx.customerPhone,
      linkedExchangeId: tx.id,
    ...
    if (tx.commission && tx.commission > 0 && tx.commissionCurrency) {
      newEntries.push({
        ...
        reason: `تبادل ارز - کارمزد صرافی`, balanceAfter: 0,
...
// ✅ توابع جدید برای Transfer و Convert
...
export default function CurrencyExchangePage() {
  ...
  // ✅ اصلاح: confirmRegister با sync کامل همه انواع معاملات
  const confirmRegister = useCallback(() => {
    ...
    // Reload cash entries to update balances
    setCashEntries(loadCashEntriesShared());
  ...
  // ✅ اصلاح: CustomerBalanceCard با نمایش موجودی صندوق
        ...
        <div className="relative z-10 mx-auto w-full max-w-7xl space-y-4 md:space-y-6 px-3 pb-16 pt-5 md:px-8 md:pt-9">
          ...
          {tab === "exchange" && (
            <section className={`space-y-4 p-4 md:p-7 ${uiCard}`}>
              ...
              <CustomerBalanceCard customer={selectedCustomer} color="cyan" isCashBox={isCustomerCashBox} />
