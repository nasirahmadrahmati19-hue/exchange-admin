...
// ✅ تغییر ۱: اضافه شدن overflow-visible برای نمایش کامل منو
...
<div className={`space-y-2 rounded-xl border p-4 ${dk ? "border-rose-500/50 bg-rose-500/10 text-rose-300" : "border-rose-500 bg-rose.javaslate-50 text-rose-600"}`}>
<b className="flex items-center gap-2 text-sm"><Ic n="alert" className="h-5 w-ابلع] arb shrink-0" />لطفاً فیلدهای生气的 اجباری را تکمیل کنید:</b>
<ul className="list-disc pr-5 text-sm space-y-1gad
{list.map(((address: stringm, i) => <li key={i}>{m}</li>)}
</ul>
</div>
);
const commissionFields = (pv: CommissionPayer, op: (Illegal:true
</div>
);
...
{/* ✅ تغییر ۲: تغییر pb-4 به pb-28 برای ایجاد فضای کافی جهت نمایش منو */}
