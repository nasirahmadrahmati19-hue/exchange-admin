...
// حساب‌های مجازی فقط از Ledger صندوق محاسبه می‌شوند؛ در آرایه مشتریان ذخیره نمی‌شوند.
...
function applyBalanceChanges(customers: Customer[], changes: BalanceChange[]): Customer[] {
  return customers.map(c => {
    // صندوق و حساب صرافی virtual هستند و موجودی آنها از CASH_KEY خوانده می‌شود.
...
function syncCashEntriesForTransfer(action: "add" | "remove" | "replace", tx: Transaction | null, oldTxId?: string) {
  ...
  if ((action === "add" || action === "replace") && tx) {
    ...
    // انتقال مشتری به مشتری، انتقال داخلی حساب‌هاست و صندوق فیزیکی نباید تغییر کند.
    // وقتی یک طرف «حساب صرافی» است، فقط یک سند صندوق ثبت می‌شود تا موجودی فیزیکی دوبار تغییر نکند.
    if (senderAccount) {
    ...
    if (tx.commission && tx.commission > 0 && tx.commissionCurrency) {
      newEntries.push({ id: newId(), trackingCode: `${tx.trackingCode}-FEE`, date: dateStr, type: "fee", currency: tx.commissionCurrency, amount: tx.commission, direction: "in", reason: `انتقال - کارمزد صرافی`, balanceAfter: 0, customerId: EXCHANGE_ACCOUNT_ID, customerName: EXCHANGE_ACCOUNT_NAME, linkedTransferId: tx.id, status: "active" });
    }
    entries = [...entries, ...newEntries];
  }
  entries = recomputeCashBalances(entries);
...
// ✅ اصلاح: تشخیص "موجودی صرافی"
const isAccount = customer === EXCHANGE_ACCOUNT_NAME;
const selC = isAccount ? EXCHANGE_ACCOUNT_CUSTOMER : customers.find(c => c.name === customer);
...
// ✅ اصلاح: تشخیص "موجودی صرافی"
const isAccount = convertCustomer === EXCHANGE_ACCOUNT_NAME;
const selC = isAccount ? EXCHANGE_ACCOUNT_CUSTOMER : customers.find(c => c.name === convertCustomer);
const tx: Transaction = {
