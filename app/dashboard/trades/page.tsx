...
const currencyBadge: Record<Currency, string> = { AFN: "from-emerald-500 to-teal-400", USD: "from-sky-500 to-cyan-400", EUR: "from-blue-600 to-blue-400", IRR: "from-amber-500 to-orange-400", PKR: "from-rose-500 to-pink-400" };
...
export default function CurrencyExchangePage() {
  ...
  useEffect(() => { try { initSharedCounterFromAllSources(); } catch {} }, []);
  ...
  const resetConvertForm = useCallback(() => { setConvertCustomer(""); setConvertFromCurrency("AFN"); setConvertToCurrency("USD"); setConvertAmount(""); setConvertRate(""); setConvertedAmount(""); setConvertCommission(""); setConvertCommissionCurrency("AFN"); setConvertDescription(""); setConvertErrors({}); setEditingConvertId(null); }, []);
  ...
  const submitExchange = useCallback(() => { const errs = validateExchange(); setExchangeErrors(errs); if (Object.values(errs).some(Boolean)) return; let rl = ""; const tr = exchangeMode === "same" ? 1 : exchangeRateValue; if (exchangeMode === "same") rl = "بدون تبدیل"; if (exchangeMode === "afn" && exchangeForeign) rl = afnRateLabel(exchangeForeign, tr); if (exchangeMode === "direct" && exchangeDirectCounter) rl = directRateLabel(exchangeDirectBaseValue, exchangeDirectCounter, tr); const tx: Transaction = { id: editingExchangeId || newId(), trackingCode: editingExchangeId ? (transactions.find(t => t.id === editingExchangeId)?.trackingCode || getNextSharedCode("FX")) : getNextSharedCode("FX"), type: "exchange", dealType: exchangeDealType as DealType, date: editingExchangeId ? (transactions.find(t => t.id === editingExchangeId)?.date || new Date().toISOString()) : new Date().toISOString(), customerId: customer, fromCurrency: receivedCurrency, fromAmount: exchangeFromAmount, toCurrency: paidCurrency, toAmount: exchangeToAmount, rate: tr, rateLabel: rl, rateBase: exchangeMode === "direct" ? exchangeDirectBaseValue : undefined, commission: exchangeCommissionValue, commissionCurrency: exchangeCommissionCurrency, commissionPayer: exchangeCommissionPayer, description: exchangeDescription.trim() || undefined, status: "active", profit: exchangeCommissionValue, profitCurrency: exchangeCommissionCurrency, customerPhone, customerTelegram }; setPreviewData(tx); setPreviewOpen(true); }, [validateExchange, exchangeFromAmount, exchangeToAmount, exchangeMode, exchangeRateValue, exchangeForeign, exchangeDirectCounter, exchangeDirectBaseValue, exchangeDescription, exchangeCommissionValue, exchangeCommissionCurrency, editingExchangeId, transactions, exchangeDealType, customer, receivedCurrency, paidCurrency, exchangeCommissionPayer, customerPhone, customerTelegram]);
  ...
  const transactionProfitLabel = useCallback((tx: Transaction) => tx.profit === undefined ? "-" : `${fmt(tx.profit)} ${tx.profitCurrency ? labels[tx.profitCurrency] : ""}`, []);
  ...
  const nextTrackingCode = useMemo(() => getNextSharedCode("FX"), []);

  const editTransaction = useCallback((tx: Transaction) => { if (tx.status === "voided") return; if (tx.type === "exchange") { setTab("exchange"); setEditingTransferId(null); setEditingConvertId(null); setEditingExchangeId(tx.id); setCustomer(tx.customerId || ""); setExchangeDealType(tx.dealType || ""); setCustomerPhone(tx.customerPhone || ""); setCustomerTelegram(tx.customerTelegram || ""); setReceivedCurrency(tx.fromCurrency); setPaidCurrency(tx.toCurrency); setReceivedAmount(String(tx.fromAmount)); setExchangeCommission(tx.commission ? String(tx.commission) : "0"); setExchangeCommissionPayer(tx.commissionPayer || "sender"); setExchangeCommissionCurrency(tx.commissionCurrency || "AFN"); setExchangeDescription(tx.description || ""); setRate(String(tx.rate)); if (getRateMode(tx.fromCurrency, tx.toCurrency) === "direct") setExchangeDirectBase(tx.rateBase || preferredDirectBase(tx.fromCurrency, tx.toCurrency)); setExchangeErrors({}); } if (tx.type === "transfer") { setTab("transfer"); setEditingExchangeId(null); setEditingConvertId(null); setEditingTransferId(tx.id); setSender(tx.senderId || ""); setReceiver(tx.receiverId || ""); setSenderCurrency(tx.fromCurrency); setReceiverCurrency(tx.toCurrency); setSenderAmount(String(tx.fromAmount)); setCommission(tx.commission ? String(tx.commission) : "0"); setTransferCommissionPayer(tx.commissionPayer || "sender"); setTransferCommissionCurrency(tx.commissionCurrency || "AFN"); setTransferDescription(tx.description || ""); setTransferRate(String(tx.rate)); if (getRateMode(tx.fromCurrency, tx.toCurrency) === "direct") setTransferDirectBase(tx.rateBase || preferredDirectBase(tx.fromCurrency, tx.toCurrency)); setTransferErrors({}); } if (tx.type === "convert") { setTab("convert"); setEditingExchangeId(null); setEditingTransferId(null); setEditingConvertId(tx.id); setConvertCustomer(tx.customerId || ""); setConvertFromCurrency(tx.fromCurrency); setConvertToCurrency(tx.toCurrency); setConvertAmount(String(tx.fromAmount)); setConvertCommission(tx.commission ? String(tx.commission) : "0"); setConvertCommissionCurrency(tx.commissionCurrency || "AFN"); setConvertDescription(tx.description || ""); setConvertRate(String(tx.rate)); if (getRateMode(tx.fromCurrency, tx.toCurrency) === "direct") setConvertDirectBase(tx.rateBase || preferredDirectBase(tx.fromCurrency, tx.toCurrency)); setConvertErrors({}); } }, []);
  const viewTransaction = useCallback((tx: Transaction) => setSelectedTransaction(tx), []);
  const voidTransaction = useCallback((tx: Transaction) => { if (tx.status === "voided") return; if (!window.confirm("لغو شود؟")) return; setTransactions(p => p.map(t => t.id === tx.id ? { ...t, status: "voided" } : t)); setEditingExchangeId(null); setEditingTransferId(null); setEditingConvertId(null); setOpenMenuId(null); }, []);
  const deleteTransaction = useCallback((tx: Transaction) => { if (!window.confirm(`حذف ${tx.trackingCode}؟`)) return; setTransactions(p => p.filter(t => t.id !== tx.id)); setOpenMenuId(null); }, []);
  ...
  // ✅✅✅ CustomerDropdown بدون onFocus - فقط تایپ و دکمه ✅✅✅
  const CustomerDropdown = memo(({ value, onInputChange, showList, onToggleList, filter, onFilterChange, listRef, filteredList, err }: {
    ...
    <div className="relative" ref={listRef}>
      ...
      <button type="button" onMouseDown={e => { e.preventDefault(); e.stopPropagation(); onToggleList(); }} className={`absolute left-2 top-1/2 -translate-y-1/2 grid h-8 w-8 place-items-center rounded-lg ${dk ? "text-slate-400 hover:bg-slate-700" : "text-slate-400 hover:bg-slate-100"}`}>
      ...
      {showList && (
        <div className={`absolute left-0 top-full z-30 mt-1 w-full max-h-60 overflow-y-auto rounded-xl border shadow-xl ${dk ? "border-slate-600 bg-slate-800" : "border-slate-200 bg-white"}`}>
          {filteredList.length === 0 ? <div className={`px-4 py-3 text-xs text-center ${subText}`}>یافت نشد</div> : filteredList.map((c, idx) => (
            <button key={c.id} type="button" onMouseDown={e => { e.preventDefault(); e.stopPropagation(); }} onClick={() => { onInputChange(c.name); onFilterChange(""); onToggleList(); }} className={`flex w-full items-center gap-2 px-3 py-2.5 text-right text-xs font-bold ${dk ? "text-slate-200 hover:bg-cyan-400/15" : "text-slate-700 hover:bg-cyan-50"}`}>
          ...
          <div className={`px-3 py-2 text-[10px] text-center ${subText}`}>یا نام جدید بنویسید</div>
        </div>
      )}
    </div>
  ));

  // ✅✅✅ ActionMenu بدون حلقه ✅✅✅
  const ActionMenu = memo(({ tx }: { tx: Transaction }) => {
    const isOpen = openMenuId === tx.id;
    const isVoided = tx.status === "voided";
    return (
      <div className="relative" ref={isOpen ? menuRef : null}>
        <button onMouseDown={e => { e.preventDefault(); e.stopPropagation(); toggleMenu(tx.id); }} className={`inline-flex cursor-pointer items-center gap-1.5 rounded-lg border px-3 py-2 text-[11px] font-black ${isOpen ? dk ? "border-blue-400/50 bg-blue-400/20 text-blue-300" : "border-blue-400 bg-blue-50 text-blue-600" : dk ? "border-slate-600 bg-slate-900 text-cyan-300" : "border-slate-200 bg-white text-sky-600"}`}>
        ...
        {isOpen && (
          <ul className={`absolute left-0 top-full z-20 mt-1.5 w-44 space-y-1 rounded-xl border p-1.5 shadow-xl ${dk ? "border-slate-600 bg-slate-900" : "border-slate-200 bg-white"}`}>
            <li><button onClick={() => { setOpenMenuId(null); viewTransaction(tx); }} className={`flex w-full items-center gap-2 rounded-lg px-3 py-2 text-right text-xs font-bold ${dk ? "text-slate-300 hover:bg-cyan-400/10" : "text-slate-600 hover:bg-sky-50"}`}><Ic n="eye" className="h-3.5 w-3.5" />مشاهده</button></li>
            <li><button onClick={() => { setOpenMenuId(null); editTransaction(tx); }} disabled={isVoided} className={`flex w-full items-center gap-2 rounded-lg px-3 py-2 text-right text-xs font-bold ${isVoided ? "opacity-40 cursor-not-allowed" : dk ? "text-slate-300 hover:bg-cyan-400/10" : "text-slate-600 hover:bg-sky-50"}`}><Ic n="pencil" className="h-3.5 w-3.5" />ویرایش</button></li>
            <li><button onClick={() => { setOpenMenuId(null); voidTransaction(tx); }} disabled={isVoided} className={`flex w-full items-center gap-2 rounded-lg px-3 py-2 text-right text-xs font-bold ${isVoided ? "opacity-40 cursor-not-allowed" : dk ? "text-rose-300 hover:bg-rose-400/10" : "text-rose-500 hover:bg-rose-50"}`}><Ic n="xCircle" className="h-3.5 w-3.5" />لغو</button></li>
            <li className={`h-px ${dk ? "bg-slate-700" : "bg-slate-100"}`} />
            <li><button onClick={() => { setOpenMenuId(null); deleteTransaction(tx); }} className={`flex w-full items-center gap-2 rounded-lg px-3 py-2 text-right text-xs font-bold ${dk ? "text-rose-300 hover:bg-rose-400/10" : "text-rose-500 hover:bg-rose-50"}`}><Ic n="trash" className="h-3.5 w-3.5" />حذف</button></li>
          </ul>
        )}
      </div>
    );
  ...
  const TransactionRow = memo(({ tx, isSearching }: { tx: Transaction; isSearching: boolean }) => {
    ...
    return (<tr className={rc}>
      <td className="px-4 py-3.5 md:px-7"><span className={`inline-flex items-center gap-1 rounded-lg border px-2.5 py-1.5 text-[11px] font-black ${dk ? "border-cyan-400/30 bg-cyan-400/10 text-cyan-300" : "border-cyan-300 bg-cyan-50 text-cyan-700"}`} dir="ltr"><Ic n="tag" className="h-3 w-3" />{tx.trackingCode}</span></td>
  ...
  return (
    <div dir="rtl" className={dk ? "dark" : ""}>
        ...
        <div className="relative z-10 mx-auto w-full max-w-7xl space-y-4 md:space-y-6 px-3 pb-16 pt-5 md:px-8 md:pt-9">
          <header className="flex flex-wrap items-center justify-between gap-3">
            <div className="flex items-center gap-2.5 min-w-0">
              <div className="relative grid h-11 w-11 md:h-14 md:w-14 place-items-center rounded-xl md:rounded-2xl bg-gradient-to-br from-sky-500 via-cyan-500 to-emerald-400 text-white shadow-lg"><Ic n="swap" className="h-5 w-5 md:h-6 md:w-6" /></div>
          ...
          {tab === "exchange" && (
            <section className={`space-y-4 p-4 md:p-7 ${uiCard}`}>
              ...
              <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
                ...
                {fld("کد پیگیری", (<input readOnly dir="ltr" value={editingExchangeId ? (editingExchangeTransaction?.trackingCode || "-") : nextTrackingCode} className={`${uiInput} ${roInput} text-left font-black`} />))}
                {fld("مشتری", (<CustomerDropdown value={customer} onInputChange={n => { setCustomer(n); const c = customers.find(x => x.name === n); setCustomerPhone(c?.phone || ""); setCustomerTelegram(c?.telegram || ""); setExchangeErrors(p => ({ ...p, customer: undefined })); }} showList={showCustomerList} onToggleList={() => setShowCustomerList(!showCustomerList)} filter={customerFilter} onFilterChange={setCustomerFilter} listRef={customerListRef} filteredList={filteredCustomerList} err={!!exchangeErrors.customer} />))}
              </div>
          ...
          {tab === "transfer" && (
            <section className={`space-y-4 p-4 md:p-7 ${uiCard}`}>
              ...
              <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
                ...
                {fld("کد", (<input readOnly dir="ltr" value={editingTransferId ? (editingTransferTransaction?.trackingCode || "-") : nextTrackingCode} className={`${uiInput} ${roInput} text-left font-black`} />))}
                {fld("جستجو", (<input value={search} onChange={e => setSearch(e.target.value)} placeholder="جستجو…" className={uiInput} />))}<div></div>
              </div>
              <div className="grid gap-3 lg:grid-cols-[1fr_auto_1fr]">
                {panel(cOrange, "up", "فرستنده", (<>{fld("مشتری", (<CustomerDropdown value={sender} onInputChange={n => { setSender(n); setTransferErrors(p => ({ ...p, sender: undefined })); }} showList={showSenderList} onToggleList={() => setShowSenderList(!showSenderList)} filter={senderFilter} onFilterChange={setSenderFilter} listRef={senderListRef} filteredList={filteredSenderList} err={!!transferErrors.sender} />))}{fld("ارز", currencySelect(senderCurrency, v => { setSenderCurrency(v); setTransferErrors(p => ({ ...p, transferRate: undefined, receiverAmount: undefined })); }))}{fld("مبلغ", (<input type="text" inputMode="decimal" dir="ltr" value={senderAmount} onChange={e => { setSenderAmount(toNumericText(e.target.value)); setTransferErrors(p => ({ ...p, senderAmount: undefined, receiverAmount: undefined })); }} placeholder="0" className={`${uiInput} text-left tabular-nums ${transferErrors.senderAmount ? errInput : ""}`} />))}</>))}
                {midBadge("arrowLeft", dk ? "border-slate-600 bg-slate-900 text-orange-300" : "border-slate-200 bg-white text-orange-500")}
                {panel(cEmerald, "down", "گیرنده", (<>{fld("مشتری", (<CustomerDropdown value={receiver} onInputChange={n => { setReceiver(n); setTransferErrors(p => ({ ...p, receiver: undefined })); }} showList={showReceiverList} onToggleList={() => setShowReceiverList(!showReceiverList)} filter={receiverFilter} onFilterChange={setReceiverFilter} listRef={receiverListRef} filteredList={filteredReceiverList} err={!!transferErrors.receiver} />))}{fld("ارز", currencySelect(receiverCurrency, v => { setReceiverCurrency(v); setTransferErrors(p => ({ ...p, transferRate: undefined, receiverAmount: undefined })); }))}{fld("مبلغ", (<input readOnly dir="ltr" value={receiverAmount} className={`${uiInput} ${roInput} text-left tabular-nums`} />))}</>))}
              </div>
          ...
          {tab === "convert" && (
            <section className={`space-y-4 p-4 md:p-7 ${uiCard}`}>
              ...
              <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
                ...
                {fld("کد", (<input readOnly dir="ltr" value={editingConvertId ? (editingConvertTransaction?.trackingCode || "-") : nextTrackingCode} className={`${uiInput} ${roInput} text-left font-black`} />))}
                {fld("مشتری", (<CustomerDropdown value={convertCustomer} onInputChange={n => { setConvertCustomer(n); setConvertErrors(p => ({ ...p, customer: undefined })); }} showList={showConvertList} onToggleList={() => setShowConvertList(!showConvertList)} filter={convertFilter} onFilterChange={setConvertFilter} listRef={convertListRef} filteredList={filteredConvertList} err={!!convertErrors.customer} />))}<div></div>
              </div>
              <div className="grid gap-3 lg:grid-cols-[1fr_auto_1fr]">
          ...
          <section className={`overflow-hidden ${uiCard}`}>
            ...
            <div className="hidden md:block overflow-x-auto">
              <table className="w-full min-w-[1100px] text-sm">
                <thead><tr className={`border-y ${dk ? "border-slate-700 bg-slate-800/60" : "border-slate-100 bg-slate-50"}`}>{["کد","مشتری","تاریخ","نوع","دریافت","پرداخت","نرخ","کارمزد","پرداخت‌کننده","عملیات"].map(h => <th key={h} className="px-4 py-3 text-right text-[11px] font-black text-slate-400">{h}</th>)}</tr></thead>
      ...
      {previewOpen && previewData && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/60 p-3" onClick={() => { setPreviewOpen(false); setPreviewData(null); }}>
          <div className={`w-full max-w-2xl rounded-xl border shadow-2xl ${dk ? "border-slate-600 bg-slate-900" : "border-slate-200 bg-white"}`} onClick={e => e.stopPropagation()}>
            <div className={`flex items-center justify-between border-b px-4 py-3 ${dk ? "border-slate-700" : "border-slate-100"}`}>
              <b className={`text-sm ${dk ? "text-slate-100" : "text-slate-800"}`}>پیش‌نمایش قبل از ثبت</b>
              <button onClick={() => { setPreviewOpen(false); setPreviewData(null); }} className="grid h-8 w-8 place-items-center rounded-lg text-slate-400"><Ic n="x" className="h-4 w-4" /></button>
            </div>
            <div className="max-h-[70vh] overflow-y-auto px-4 py-4 space-y-4">
              <div className={`flex items-center justify-between rounded-xl border p-3 ${dk ? "border-cyan-400/30 bg-cyan-400/10" : "border-cyan-300 bg-cyan-50"}`}>
                <b className={`text-xs ${dk ? "text-cyan-300" : "text-cyan-700"}`}>کد پیگیری</b>
                <span className={`text-[13px] font-black ${dk ? "text-cyan-300" : "text-cyan-700"}`} dir="ltr">{previewData.trackingCode}</span>
              </div>
