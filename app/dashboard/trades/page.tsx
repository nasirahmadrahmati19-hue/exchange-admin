...
interface TransferTransaction extends BaseTransaction {
  ...
  rate: number; // 1 baseUnit(senderCurrency) = rate receiverCurrency (new definition)
  commission: number;
...
export default function CurrencyExchangePage() {
  ...
  // ---------- تبادل صرافی-مشتری: paidAmount = (receivedAmount * rate) / baseUnit(received) ----------
  const computeExchangePaid = () => {
    ...
    const paid = (received * rate) / baseReceived;
    setExPaidAmount(formatNumber(paid));
  };

  useMemo(() => computeExchangePaid(), [exReceivedAmount, exRate, exReceivedCurrency]);

  // ---------- تبادل بین مشتریان: receiverAmount = (senderAmount * rate) / baseUnit(sender) ----------
  const computeTransferReceiver = () => {
    ...
    const baseSender = baseUnits[trSenderCurrency] || 1;
    const receiver = (senderAmt * rate) / baseSender;
    setTrReceiverAmount(formatNumber(receiver));
  };

  useMemo(() => computeTransferReceiver(), [trSenderAmount, trRate, trSenderCurrency]);
  ...
  const submitTransfer = () => {
    ...
    const receiverAmountNum = (senderAmountNum * rateNum) / baseSender;
  ...
  const printReceipt = (tx: Transaction) => {
    ...
    if (tx.type === "صرافی-مشتری") {
      ...
      const bu = baseUnits[tx.senderCurrency] || 1;
      content += `<p><strong>نرخ:</strong> ${bu} ${currencyLabels[tx.senderCurrency]} = ${tx.rate} ${currencyLabels[tx.receiverCurrency]}</p>`;
      if (tx.commission > 0) {
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
          ...
          <div className="mb-6">
            ...
            <input
              ...
              placeholder={`${baseUnits[trSenderCurrency]} ${currencyLabels[trSenderCurrency]} = ? ${currencyLabels[trReceiverCurrency]}`}
              className="h-14 rounded-[14px] w-full px-4 py-2 border border-gray-200 bg-white text-gray-800 text-sm focus:outline-none focus:ring-2 focus:ring-[#092F3A]/20"
            />
          </div>
                ...
                <tr key={tx.id} className={`hover:bg-gray-50 ${isVoided ? "opacity-60 line-through" : ""}`}>
                  ...
                  <td className="py-3 px-2 text-xs">
                    {tx.type === "صرافی-مشتری"
                      ...
                      : `${baseUnits[tx.senderCurrency]} ${currencyLabels[tx.senderCurrency]} = ${tx.rate} ${currencyLabels[tx.receiverCurrency]}`
                    }
                  </td>
            ...
            <div className="space-y-2 text-sm">
              ...
              {viewTx.type === "بین-مشتریان" && (
                <>
                  ...
                  <p><strong>نرخ:</strong> {baseUnits[viewTx.senderCurrency]} {currencyLabels[viewTx.senderCurrency]} = {viewTx.rate} {currencyLabels[viewTx.receiverCurrency]}</p>
