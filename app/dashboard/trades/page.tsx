...
const iconPaths = {
  ...
  convert: "M7.5 21 3 16.5m0 0L7.5 12M3 16.5h13.5m0-13.5L21 7.5m0 0L16.5 12M21 7.5H7.5",
...
export default function CurrencyExchangePage() {
  ...
  const exchangeDirectBaseValue =
    exchangeMode === "direct" ? getSafeDirectBase(exchangeDirectBase, receivedCurrency, paidCurrency) : receivedCurrency;
  const exchangeDirectCounter =
    exchangeMode === "direct" ? getDirectCounter(exchangeDirectBaseValue, receivedCurrency, paidCurrency) : null;
  ...
  const transferDirectBaseValue =
    transferMode === "direct" ? getSafeDirectBase(transferDirectBase, senderCurrency, receiverCurrency) : senderCurrency;
  const transferDirectCounter =
    transferMode === "direct" ? getDirectCounter(transferDirectBaseValue, senderCurrency, receiverCurrency) : null;
  ...
  const convertDirectBaseValue =
    convertMode === "direct" ? getSafeDirectBase(convertDirectBase, convertFromCurrency, convertToCurrency) : convertFromCurrency;
  const convertDirectCounter =
    convertMode === "direct" ? getDirectCounter(convertDirectBaseValue, convertFromCurrency, convertToCurrency) : null;
  ...
  useEffect(() => {
    setConvertRate("");
  }, [convertMode, convertForeign, convertDirectBaseValue, convertDirectCounter]);
  ...
  function resetExchangeForm() {
    setCustomer(""); setExchangeDealType(""); setReceivedAmount(""); setPaidAmount(""); setRate("");
  ...
  function submitExchange() {
    ...
    if (editingExchangeId) {
      setTransactions((prev) => prev.map((t) => t.id === editingExchangeId ? {
        ...
        description, profit: calculatedProfit, profitCurrency: calculatedProfitCurrency,
      } : t));
    } else {
      const tx: Transaction = {
        id: newId(), type: "exchange", dealType: exchangeDealType as DealType, date: new Date().toISOString(),
  ...
  function submitTransfer() {
    ...
    if (editingTransferId) {
      ...
      const tx: Transaction = {
        id: newId(), type: "transfer", date: new Date().toISOString(), senderId: sender, receiverId: receiver,
  ...
  function submitConvert() {
    ...
    if (editingConvertId) {
      setTransactions((prev) => prev.map((t) => t.id === editingConvertId ? {
        ...t, type: "convert", customerId: convertCustomer,
        fromCurrency: convertFromCurrency, fromAmount, toCurrency: convertToCurrency, toAmount,
        rate: txRate, rateLabel, rateBase: convertRateBase,
        commission: convertCommissionValue, commissionCurrency: convertCommissionCurrency,
        commissionPayer: "sender" as CommissionPayer,
        description, profit: calculatedProfit, profitCurrency: calculatedProfitCurrency,
      } : t));
    } else {
      const tx: Transaction = {
        id: newId(), type: "convert", date: new Date().toISOString(),
  ...
  /* ✅ فیلد انتخاب کارمزد - قابل استفاده در هر سه تب */
    ...
    <div className="grid gap-3 md:gap-4 sm:grid-cols-2">
      {showPayer && fld("کارمزد از حساب", (
        <div className={`flex rounded-xl border p-1 ${dk ? "border-slate-600 bg-slate-900" : "border-slate-200 bg-slate-50"}`}>
          ...
          <button
            type="button"
            onClick={() => onPayerChange("receiver")}
            className={`flex-1 rounded-lg px-3 py-2 text-xs font-black transition-all ${
              payerValue === "receiver"
                ? dk ? "bg-cyan-400 text-slate-950 shadow" : "bg-sky-500 text-white shadow"
                : dk ? "text-slate-400 hover:text-slate-200" : "text-slate-500 hover:text-slate-700"
    ...
    <div dir="rtl" className={dk ? "dark" : ""}>
        ...
        <div className="relative z-10 mx-auto w-full max-w-7xl space-y-4 md:space-y-6 px-3 pb-16 pt-5 md:px-8 md:pt-9">
              ...
              <div className={`hidden sm:flex items-center gap-2 rounded-xl border px-3 py-2 shadow-sm backdrop-blur ${glassChip}`}>
                ...
                {now && (
                  <>
                    ...
                    <span className={`whitespace-nowrap text-[10px] font-black ${dk ? "text-cyan-300" : "text-sky-600"}`}>
          ...
          <div className="fx-up space-y-2" style={{ animationDelay: "70ms" }}>
            <div className="fx-scroll flex gap-2 overflow-x-auto -mx-3 px-3 pb-1 md:mx-0 md:px-0 md:pb-0">
              {currencies.map((c) => (
                <span
                  key={c}
                  className={`flex shrink-0 cursor-default items-center gap-1.5 rounded-full border py-1 pl-3 pr-1.5 md:py-1.5 md:pl-3.5 text-xs font-bold shadow-sm backdrop-blur transition-all duration-300 hover:-translate-y-0.5 hover:shadow-md ${glassChip} ${
                    dk ? "text-slate-300 hover:border-cyan-400/50" : "text-slate-600 hover:border-sky-400/60"
                  }`}
                >
          ...
          <div className={`fx-up flex gap-1.5 md:gap-2 rounded-xl md:rounded-2xl border p-1.5 md:p-2 shadow-sm backdrop-blur ${glassChip}`} style={{ animationDelay: "140ms" }}>
            <button
              ...
              className={`flex flex-1 md:flex-initial cursor-pointer items-center justify-center md:justify-start gap-1.5 md:gap-2 rounded-lg md:rounded-xl px-3 md:px-5 py-2.5 md:py-3 text-xs md:text-sm font-black transition-all duration-300 active:scale-[0.97] ${
                tab === "exchange"
                  ? `bg-gradient-to-l shadow-lg ${dk ? "from-cyan-400 to-emerald-400 text-slate-950 shadow-cyan-400/25" : "from-sky-500 via-cyan-500 to-emerald-400 text-white shadow-sky-500/30"}`
                  : dk
            ...
            >
              ...
              <span className="hidden xs:inline md:inline">تبادل ارز</span>
              <span className="xs:hidden md:hidden inline">تبادل</span>
            </button>
            ...
            <button
              ...
              className={`flex flex-1 md:flex-initial cursor-pointer items-center justify-center md:justify-start gap-1.5 md:gap-2 rounded-lg md:rounded-xl px-3 md:px-5 py-2.5 md:py-3 text-xs md:text-sm font-black transition-all duration-300 active:scale-[0.97] ${
                tab === "transfer"
                  ? `bg-gradient-to-l shadow-lg ${dk ? "from-orange-400 to-amber-300 text-slate-950 shadow-orange-400/25" : "from-orange-500 via-orange-400 to-amber-400 text-white shadow-orange-500/30"}`
                  : dk
                  ? "text-slate-400 hover:bg-slate-700/60 hover:text-slate-100"
                  : "text-slate-500 hover:bg-sky-50 hover:text-slate-800"
              }`}
            >
              ...
              <span className="hidden xs:inline md:inline">بین مشتریان</span>
              <span className="xs:hidden md:hidden inline">مشتریان</span>
            </button>
            ...
            <button
              ...
              className={`flex flex-1 md:flex-initial cursor-pointer items-center justify-center md:justify-start gap-1.5 md:gap-2 rounded-lg md:rounded-xl px-3 md:px-5 py-2.5 md:py-3 text-xs md:text-sm font-black transition-all duration-300 active:scale-[0.97] ${
                tab === "convert"
                  ? `bg-gradient-to-l shadow-lg ${dk ? "from-violet-400 to-purple-400 text-slate-950 shadow-violet-400/25" : "from-violet-500 via-purple-500 to-fuchsia-400 text-white shadow-violet-500/30"}`
                  : dk
                  ? "text-slate-400 hover:bg-slate-700/60 hover:text-slate-100"
            ...
            >
              ...
              <span className="hidden xs:inline md:inline">تبدیل ارز مشتری</span>
              <span className="xs:hidden md:hidden inline">تبدیل</span>
            </button>
          </div>
          ...
          {tab === "exchange" && (
            <section className={`fx-up space-y-4 md:space-y-5 p-4 md:p-7 ${uiCard}`}>
              {secHead(
                identExIcon, "swap", "تبادل ارز صرافی با مشتری", "دریافت یک ارز از مشتری و پرداخت ارز دیگر", identExChip,
                editingExchangeId ? `ویرایش ${shortId(editingExchangeId)}` : "معاملهٔ جدید"
              )}
              ...
              <div className="grid gap-3 md:gap-4 sm:grid-cols-2 xl:grid-cols-4">
                ...
                {fld("نوع معامله", sel(exchangeDealType, (v) => {
                  setExchangeDealType(v as DealType | "");
                  setExchangeErrors((p) => ({ ...p, dealType: undefined }));
                }, [["", "انتخاب نوع معامله"], ["buy", "خرید"], ["sell", "فروش"]], exchangeErrors.dealType ? errInput : ""))}
                {fld("مشتری", sel(customer, (v) => {
                  setCustomer(v);
                  setExchangeErrors((p) => ({ ...p, customer: undefined }));
                }, customerOpts, exchangeErrors.customer ? errInput : ""))}
              ...
              <div className="grid gap-3 md:gap-4 lg:grid-cols-[1fr_auto_1fr]">
                {panel(cEmerald, "down", "دریافت از مشتری", (
                  <>
                    {fld("ارز دریافتی", currencySelect(receivedCurrency, (v) => {
                      setReceivedCurrency(v);
                      setExchangeErrors((p) => ({ ...p, rate: undefined, paidAmount: undefined }));
                    }))}
                    {fld("مبلغ دریافتی", (
                      <input type="text" inputMode="decimal" dir="ltr" value={receivedAmount}
                        onChange={(e) => {
                          setReceivedAmount(toNumericText(e.target.value));
                          setExchangeErrors((p) => ({ ...p, receivedAmount: undefined, paidAmount: undefined }));
                        }}
                        placeholder="0.00"
                        className={`${uiInput} text-left tabular-nums ${exchangeErrors.receivedAmount ? errInput : ""}`} />
                    ))}
                  </>
                ))}
                ...
                {panel(cSky, "up", "پرداخت به مشتری", (
                  <>
                    {fld("ارز پرداختی", currencySelect(paidCurrency, (v) => {
                      setPaidCurrency(v);
                      setExchangeErrors((p) => ({ ...p, rate: undefined, paidAmount: undefined }));
                    }))}
                    {fld("مبلغ پرداختی", (
                      <input readOnly dir="ltr" value={paidAmount} className={`${uiInput} ${roInput} text-left tabular-nums ${exchangeErrors.paidAmount ? errInput : ""}`} />
                    ))}
                  </>
                ))}
              </div>
              ...
              {exchangeMode === "afn" && exchangeForeign &&
                rateBox(cSky, "نرخ دستی در برابر افغانی", (
                  <div>
                    ...
                    <div className="flex flex-wrap items-center gap-2.5">
                      ...
                      {rateInput(rate, (s) => {
                        setRate(s);
                        setExchangeErrors((p) => ({ ...p, rate: undefined, paidAmount: undefined }));
                      }, !!exchangeErrors.rate, "w-32 md:w-44")}
              ...
              {exchangeMode === "direct" &&
                rateBox(cAmber, "نرخ مستقیم جفت‌ارز", (
                  <div className="grid items-end gap-3 md:gap-4 md:grid-cols-2">
                    {fld("مبنای نرخ", sel(exchangeDirectBaseValue, (v) => {
                      setExchangeDirectBase(v as Currency);
                      setExchangeErrors((p) => ({ ...p, rate: undefined, paidAmount: undefined }));
                    }, [[receivedCurrency, labels[receivedCurrency]], [paidCurrency, labels[paidCurrency]]]))}
                    <div>
                      ...
                      <div className="flex flex-wrap items-center gap-2">
                        ...
                        {rateInput(rate, (s) => {
                          setRate(s);
                          setExchangeErrors((p) => ({ ...p, rate: undefined, paidAmount: undefined }));
                        }, !!exchangeErrors.rate, "w-28 md:w-40")}
                  ...
                  <>
                    {pill(
                      ...
                      exchangeRateValue > 0 && exchangeDirectCounter
                        ? `نرخ ثبت‌شده: ${directRateLabel(exchangeDirectBaseValue, exchangeDirectCounter, exchangeRateValue)}`
                        : "",
                      true
                    )}
                    {pill(cEmerald.badge, paidAmount ? `نتیجه: ${paidAmount} ${labels[paidCurrency]}` : "")}
              ...
              <div className="grid gap-3 md:gap-4 md:grid-cols-2">
                {fld("کارمزد", moneyField(exchangeCommission, (s) => {
                  setExchangeCommission(s);
                  setExchangeErrors((p) => ({ ...p, exchangeCommission: undefined }));
                }, !!exchangeErrors.exchangeCommission, labels[exchangeCommissionCurrency], dk ? "bg-cyan-400/15 text-cyan-300" : "bg-sky-100 text-sky-700"))}

                {fld("توضیحات", (
                  <input type="text" value={exchangeDescription} onChange={(e) => setExchangeDescription(e.target.value)} placeholder="توضیحات اختیاری…" className={uiInput} />
                ))}
              </div>

              {/* ✅ فیلدهای کارمزد */}
              {commissionFields(
                exchangeCommissionPayer,
                setExchangeCommissionPayer,
                exchangeCommissionCurrency,
                setExchangeCommissionCurrency,
                true
              )}
              ...
              <button
                ...
                className={`group flex h-[50px] md:h-[52px] w-full cursor-pointer items-center justify-center gap-2 rounded-xl bg-gradient-to-l text-base font-black shadow-lg transition-all duration-300 hover:shadow-xl hover:brightness-110 active:scale-[0.985] ${
                  dk ? "from-cyan-400 to-emerald-400 text-slate-950 shadow-cyan-400/25" : "from-sky-500 via-cyan-500 to-emerald-400 text-white shadow-sky-500/30"
                }`}
              >
          ...
          {tab === "transfer" && (
            <section className={`fx-up space-y-4 md:space-y-5 p-4 md:p-7 ${uiCard}`}>
              {secHead(
                identTrIcon, "users", "تبادل بین حساب مشتریان", "انتقال موجودی از حساب مشتری به مشتری دیگر", identTrChip,
                editingTransferId ? `ویرایش ${shortId(editingTransferId)}` : "انتقال جدید"
              )}
              ...
              <div className="grid gap-3 md:gap-4 lg:grid-cols-[1fr_auto_1fr]">
                {panel(cOrange, "up", "فرستنده", (
                  <>
                    {fld("مشتری فرستنده", sel(sender, (v) => {
                      setSender(v);
                      setTransferErrors((p) => ({ ...p, sender: undefined }));
                    }, customerOpts, transferErrors.sender ? errInput : ""))}
                    {fld("ارز فرستنده", currencySelect(senderCurrency, (v) => {
                      setSenderCurrency(v);
                      setTransferErrors((p) => ({ ...p, transferRate: undefined, receiverAmount: undefined }));
                    }))}
                    {fld("مبلغ فرستنده", (
                      <input type="text" inputMode="decimal" dir="ltr" value={senderAmount}
                        onChange={(e) => {
                          setSenderAmount(toNumericText(e.target.value));
                          setTransferErrors((p) => ({ ...p, senderAmount: undefined, receiverAmount: undefined }));
                        }}
                        placeholder="0.00"
                        className={`${uiInput} text-left tabular-nums ${transferErrors.senderAmount ? errInput : ""}`} />
                    ))}
                  </>
                ))}
                ...
                {panel(cEmerald, "down", "گیرنده", (
                  <>
                    {fld("مشتری گیرنده", sel(receiver, (v) => {
                      setReceiver(v);
                      setTransferErrors((p) => ({ ...p, receiver: undefined }));
                    }, customerOpts, transferErrors.receiver ? errInput : ""))}
                    {fld("ارز گیرنده", currencySelect(receiverCurrency, (v) => {
                      setReceiverCurrency(v);
                      setTransferErrors((p) => ({ ...p, transferRate: undefined, receiverAmount: undefined }));
                    }))}
                    {fld("مبلغ گیرنده", (
                      <input readOnly dir="ltr" value={receiverAmount} className={`${uiInput} ${roInput} text-left tabular-nums ${transferErrors.receiverAmount ? errInput : ""}`} />
                    ))}
                  </>
                ))}
              </div>
              ...
              {transferMode === "afn" && transferForeign &&
                rateBox(cTeal, "نرخ دستی در برابر افغانی", (
                  <div>
                    <label className={uiLabel}>نرخ</label>
                    <div className="flex flex-wrap items-center gap-2.5">
                      <span className={rateChip}>{fmt(rateUnits[transferForeign])} {labels[transferForeign]} =</span>
                      {rateInput(transferRate, (s) => {
                        setTransferRate(s);
                        setTransferErrors((p) => ({ ...p, transferRate: undefined, receiverAmount: undefined }));
                      }, !!transferErrors.transferRate, "w-32 md:w-44")}
                      <span className={rateChip}>{labels.AFN}</span>
                    </div>
                  </div>
                ), (
              ...
              {transferMode === "direct" &&
                rateBox(cOrange, "نرخ مستقیم جفت‌ارز", (
                  <div className="grid items-end gap-3 md:gap-4 md:grid-cols-2">
                    {fld("مبنای نرخ", sel(transferDirectBaseValue, (v) => {
                      setTransferDirectBase(v as Currency);
                      setTransferErrors((p) => ({ ...p, transferRate: undefined, receiverAmount: undefined }));
                    }, [[senderCurrency, labels[senderCurrency]], [receiverCurrency, labels[receiverCurrency]]]))}
                    <div>
                      <label className={uiLabel}>نرخ مستقیم</label>
                      <div className="flex flex-wrap items-center gap-2">
                        ...
                        {rateInput(transferRate, (s) => {
                          setTransferRate(s);
                          setTransferErrors((p) => ({ ...p, transferRate: undefined, receiverAmount: undefined }));
                        }, !!transferErrors.transferRate, "w-28 md:w-40")}
                  ...
                  <>
                    {pill(
                      ...
                      transferRateValue > 0 && transferDirectCounter
                        ? `نرخ ثبت‌شده: ${directRateLabel(transferDirectBaseValue, transferDirectCounter, transferRateValue)}`
                        : "",
                      true
                    )}
                    {pill(cEmerald.badge, receiverAmount ? `نتیجه: ${receiverAmount} ${labels[receiverCurrency]}` : "")}
              ...
              <div className="grid gap-3 md:gap-4 md:grid-cols-2">
                {fld("کارمزد", moneyField(commission, (s) => {
                  setCommission(s);
                  setTransferErrors((p) => ({ ...p, commission: undefined }));
                }, !!transferErrors.commission, labels[transferCommissionCurrency], dk ? "bg-orange-400/15 text-orange-300" : "bg-orange-100 text-orange-700"))}

                {fld("توضیحات", (
                  <input type="text" value={transferDescription} onChange={(e) => setTransferDescription(e.target.value)} placeholder="توضیحات اختیاری…" className={uiInput} />
                ))}
              </div>
              ...
              {commissionFields(
                ...
                setTransferCommissionPayer,
                transferCommissionCurrency,
                setTransferCommissionCurrency,
                true
              )}
              ...
              <button
                ...
                className={`group flex h-[50px] md:h-[52px] w-full cursor-pointer items-center justify-center gap-2 rounded-xl bg-gradient-to-l text-base font-black shadow-lg transition-all duration-300 hover:shadow-xl hover:brightness-110 active:scale-[0.985] ${
                  dk ? "from-orange-400 to-amber-300 text-slate-950 shadow-orange-400/25" : "from-orange-500 via-orange-400 to-amber-400 text-white shadow-orange-500/30"
                }`}
              >
          ...
          {tab === "convert" && (
            <section className={`fx-up space-y-4 md:space-y-5 p-4 md:p-7 ${uiCard}`}>
              {secHead(
                identCvIcon, "user", "تبدیل ارز مشتری", "تبدیل یک ارز به ارز دیگر در حساب خود مشتری", identCvChip,
                editingConvertId ? `ویرایش ${shortId(editingConvertId)}` : "تبدیل جدید"
              )}

              {editingConvertId &&
                editBanner(
                  <>در حال ویرایش تبدیل {shortId(editingConvertId)}. تاریخ اصلی حفظ می‌شود.</>,
                  resetConvertForm
                )}

              <div className="grid gap-3 md:gap-4 sm:grid-cols-2">
                {fld(`تاریخ و ساعت (شمسی) ${editingConvertId ? "(اصل)" : "(خودکار)"}`, dateField(convertDateDisplay))}
                {fld("مشتری", sel(convertCustomer, (v) => {
                  setConvertCustomer(v);
                  setConvertErrors((p) => ({ ...p, customer: undefined }));
                }, customerOpts, convertErrors.customer ? errInput : ""))}
              </div>

              <div className="grid gap-3 md:gap-4 lg:grid-cols-[1fr_auto_1fr]">
                {panel(cViolet, "down", "از حساب مشتری", (
                  <>
                    {fld("ارز مبدا", currencySelect(convertFromCurrency, (v) => {
                      setConvertFromCurrency(v);
                      setConvertErrors((p) => ({ ...p, rate: undefined, convertedAmount: undefined }));
                    }))}
                    {fld("مبلغ مبدا", (
                      <input type="text" inputMode="decimal" dir="ltr" value={convertAmount}
                        onChange={(e) => {
                          setConvertAmount(toNumericText(e.target.value));
                          setConvertErrors((p) => ({ ...p, amount: undefined, convertedAmount: undefined }));
                        }}
                        placeholder="0.00"
                        className={`${uiInput} text-left tabular-nums ${convertErrors.amount ? errInput : ""}`} />
                    ))}
                  </>
                ))}
                ...
                {panel(cEmerald, "up", "به حساب مشتری", (
                  <>
                    {fld("ارز مقصد", currencySelect(convertToCurrency, (v) => {
                      setConvertToCurrency(v);
                      setConvertErrors((p) => ({ ...p, rate: undefined, convertedAmount: undefined }));
                    }))}
                    {fld("مبلغ مقصد", (
                      <input readOnly dir="ltr" value={convertedAmount} className={`${uiInput} ${roInput} text-left tabular-nums ${convertErrors.convertedAmount ? errInput : ""}`} />
                    ))}
                  </>
                ))}
              </div>
                ...
                rateBox(cSky, "نرخ دستی در برابر افغانی", (
                  <div>
                    ...
                    <div className="flex flex-wrap items-center gap-2.5">
                      ...
                      {rateInput(convertRate, (s) => {
                        setConvertRate(s);
                        setConvertErrors((p) => ({ ...p, rate: undefined, convertedAmount: undefined }));
                      }, !!convertErrors.rate, "w-32 md:w-44")}
                      <span className={rateChip}>{labels.AFN}</span>
                    </div>
                  </div>
                ), (
              ...
              {convertMode === "direct" &&
                rateBox(cAmber, "نرخ مستقیم جفت‌ارز", (
                  <div className="grid items-end gap-3 md:gap-4 md:grid-cols-2">
                    {fld("مبنای نرخ", sel(convertDirectBaseValue, (v) => {
                      setConvertDirectBase(v as Currency);
                      setConvertErrors((p) => ({ ...p, rate: undefined, convertedAmount: undefined }));
                    }, [[convertFromCurrency, labels[convertFromCurrency]], [convertToCurrency, labels[convertToCurrency]]]))}
                    <div>
                      <label className={uiLabel}>نرخ مستقیم</label>
                      <div className="flex flex-wrap items-center gap-2">
                        ...
                        {rateInput(convertRate, (s) => {
                          setConvertRate(s);
                          setConvertErrors((p) => ({ ...p, rate: undefined, convertedAmount: undefined }));
                        }, !!convertErrors.rate, "w-28 md:w-40")}
                  ...
                  <>
                    {pill(
                      cAmber.badge,
                      convertRateValue > 0 && convertDirectCounter
                        ? `نرخ ثبت‌شده: ${directRateLabel(convertDirectBaseValue, convertDirectCounter, convertRateValue)}`
                        : "",
                      true
                    )}
                    {pill(cEmerald.badge, convertedAmount ? `نتیجه: ${convertedAmount} ${labels[convertToCurrency]}` : "")}
              ...
              <div className="grid gap-3 md:gap-4 md:grid-cols-2">
                {fld("کارمزد", moneyField(convertCommission, (s) => {
                  setConvertCommission(s);
                  setConvertErrors((p) => ({ ...p, commission: undefined }));
                }, !!convertErrors.commission, labels[convertCommissionCurrency], dk ? "bg-violet-400/15 text-violet-300" : "bg-violet-100 text-violet-700"))}

                {fld("توضیحات", (
                  <input type="text" value={convertDescription} onChange={(e) => setConvertDescription(e.target.value)} placeholder="توضیحات اختیاری…" className={uiInput} />
                ))}
              </div>

              {/* ✅ فیلد ارز کارمزد (بدون انتخاب پرداخت‌کننده چون یک نفر است) */}
              {commissionFields(
                "sender",
                () => {},
                convertCommissionCurrency,
                setConvertCommissionCurrency,
                false
              )}
              ...
              <button
                ...
                className={`group flex h-[50px] md:h-[52px] w-full cursor-pointer items-center justify-center gap-2 rounded-xl bg-gradient-to-l text-base font-black shadow-lg transition-all duration-300 hover:shadow-xl hover:brightness-110 active:scale-[0.985] ${
                  dk ? "from-violet-400 to-purple-400 text-slate-950 shadow-violet-400/25" : "from-violet-500 via-purple-500 to-fuchsia-400 text-white shadow-violet-500/30"
                }`}
              >
              ...
              {isSearching && (
                <button
                  ...
                  className={`flex cursor-pointer items-center gap-1.5 rounded-full px-3 py-1.5 text-[10px] font-black ring-1 transition ${
                    dk ? "bg-amber-400/10 text-amber-300 ring-amber-400/25 hover:bg-amber-400/20" : "bg-amber-100 text-amber-700 ring-amber-300/60 hover:bg-amber-200/70"
                  }`}
                >
                  نتایج جستجو
            ...
            <div className="md:hidden px-3 md:px-5 pb-4 space-y-2">
              {transactions.length === 0 ? (
                <div className={`flex flex-col items-center gap-3 px-6 py-12 ${dk ? "text-slate-500" : "text-slate-400"}`}>
                  ...
                  <p className="text-sm font-black text-center">هنوز معامله‌ای ثبت نشده است</p>
                  <p className="text-xs font-medium text-center">اولین معامله را از فرم بالا ثبت کنید؛ جزئیات آن اینجا نمایش داده می‌شود.</p>
                </div>
              ) : (
                transactions.map((tx, index) => {
                  ...
                  if (isSearching) {
                    if (matchesSearch) {
                      cardClass += dk ? " bg-amber-400/10 border-amber-400/40" : " bg-amber-50 border-amber-300";
                    } else {
                  ...
                  if (isVoided) {
                    cardClass += dk ? " opacity-70" : " opacity-75";
                  }

                  const fromColor = dk ? "text-emerald-300" : "text-emerald-700";
                  const toColor = dk ? "text-sky-300" : "text-sky-700";
                  ...
                  return (
                    <details key={tx.id} className={`${cardClass} group`}>
                      <summary className={`cursor-pointer select-none p-3 flex items-center gap-2.5 ${dk ? "hover:bg-slate-700/30" : "hover:bg-slate-50"}`}>
                        <span className={`grid h-8 w-8 shrink-0 place-items-center rounded-lg text-[10px] font-black tabular-nums ${dk ? "bg-slate-700 text-slate-300" : "bg-slate-100 text-slate-600"}`}>
                          {transactions.length - index}
                        </span>
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-1.5 mb-1 flex-wrap">
                            <span className={`inline-flex rounded-full px-2 py-0.5 text-[9px] font-black ${typeChipClass(tx)}`}>
                              {transactionTypeLabel(tx)}
                            </span>
                            {isVoided && (
                              <span className={`inline-flex rounded-full px-2 py-0.5 text-[9px] font-black ring-1 ${dk ? "bg-rose-400/10 text-rose-300 ring-rose-400/20" : "bg-rose-100 text-rose-600 ring-rose-300/60"}`}>
                                لغو شده
                              </span>
                            )}
                          </div>
                          <div className={`text-[12px] font-bold truncate ${dk ? "text-slate-100" : "text-slate-800"}`}>
                      ...
                      <div className={`px-3 pb-3 pt-1 space-y-2.5 border-t ${dk ? "border-slate-700" : "border-slate-100"}`}>
                        <div className={`text-[11px] tabular-nums ${subText}`} dir="ltr">
                          {dateLabel(tx.date)}
                        </div>

                        <div className="grid grid-cols-2 gap-2">
                          <div className={`rounded-lg p-2 ${dk ? "bg-emerald-400/10" : "bg-emerald-50"}`}>
                            <div className={`text-[10px] font-bold ${subText} mb-0.5`}>دریافت</div>
                            <div className={`text-[13px] font-black tabular-nums ${fromColor}`}>{fmt(tx.fromAmount)}</div>
                            <div className={`text-[10px] font-bold ${subText}`}>{labels[tx.fromCurrency]}</div>
                          </div>
                          <div className={`rounded-lg p-2 ${dk ? "bg-sky-400/10" : "bg-sky-50"}`}>
                            <div className={`text-[10px] font-bold ${subText} mb-0.5`}>پرداخت</div>
                            <div className={`text-[13px] font-black tabular-nums ${toColor}`}>{fmt(tx.toAmount)}</div>
                            <div className={`text-[10px] font-bold ${subText}`}>{labels[tx.toCurrency]}</div>
                          </div>
                        </div>

                        <div className={`text-[11px] font-bold rounded-lg px-2.5 py-2 ${dk ? "bg-slate-900/60 text-slate-300" : "bg-slate-50 text-slate-600"}`}>
                          <span className={`block text-[9px] font-black ${subText} mb-0.5`}>نرخ ارز</span>
                          {tx.rateLabel}
                        </div>

                        <div className="flex items-center justify-between text-[11px]">
                          <span className={`font-bold ${subText}`}>کارمزد</span>
                          <span className={`font-black tabular-nums ${dk ? "text-slate-200" : "text-slate-700"}`}>
                            {transactionCommissionLabel(tx)}
                          </span>
                        </div>

                        <div className="flex items-center justify-between text-[11px]">
                          <span className={`font-bold ${subText}`}>پرداخت‌کننده کارمزد</span>
                          <span className={`font-black ${dk ? "text-slate-200" : "text-slate-700"}`}>
                            {commissionPayerLabel(tx)}
                          </span>
                        </div>
                        ...
                        <div className="grid grid-cols-2 gap-1.5 pt-1">
                          <button
                            ...
                            disabled={isVoided}
                            className={`flex items-center justify-center gap-1 rounded-lg border px-2 py-2 text-[11px] font-bold transition active:scale-95 ${
                              dk
                                ? "border-slate-600 text-slate-300 disabled:opacity-40 active:bg-slate-700"
                                : "border-slate-200 text-slate-600 disabled:opacity-40 active:bg-slate-50"
                            }`}
                          >
                          ...
                          <button
                            ...
                            disabled={isVoided}
                            className={`flex items-center justify-center gap-1 rounded-lg border px-2 py-2 text-[11px] font-bold transition active:scale-95 disabled:opacity-40 ${
                              dk
                                ? "border-rose-400/30 text-rose-300 active:bg-rose-400/10"
                                : "border-rose-300 text-rose-600 active:bg-rose-50"
                            }`}
                          >
              ...
              <table className="w-full min-w-[1100px] text-sm">
                <thead>
                  <tr className={`border-y ${dk ? "border-slate-700 bg-slate-800/60" : "border-slate-100 bg-slate-50"}`}>
                    {["شماره", "نام مشتری", "تاریخ (شمسی)", "نوع معامله", "دریافت", "پرداخت", "نرخ ارز", "کارمزد", "پرداخت‌کننده کارمزد", "عملیات"].map((h, i) => (
                      <th key={h} className={`px-4 py-3 text-right text-[11px] font-black text-slate-400 ${i === 0 ? "md:px-7" : ""} ${i === 9 ? "md:px-7" : ""}`}>
                        {h}
                      </th>
                    ))}
                  </tr>
                    ...
                    <tr>
                      <td colSpan={10}>
                        <div className={`flex flex-col items-center gap-3 px-6 py-14 ${dk ? "text-slate-500" : "text-slate-400"}`}>
                          ...
                          <p className="text-xs font-medium">اولین معامله را از فرم بالا ثبت کنید؛ جزئیات آن اینجا نمایش داده می‌شود.</p>
                        </div>
                      </td>
                    </tr>
                  ) : (
                    transactions.map((tx, index) => {
                      ...
                      if (isSearching) {
                        rowClass += matchesSearch
                          ...
                          : " opacity-30";
                        if (tx.status === "voided") rowClass += dk ? " text-slate-500" : " text-slate-400";
                      } else if (tx.status === "voided") {
                        rowClass += dk ? " bg-rose-400/[0.05] text-slate-500" : " bg-rose-50 text-slate-400";
                      }
                        ...
                        <tr key={tx.id} className={rowClass}>
                          ...
                          <td className="px-4 py-3.5">
                            <div className="flex flex-wrap items-center gap-1.5">
                              <span className={`inline-flex rounded-full px-2.5 py-1 text-[10px] font-black ${typeChipClass(tx)}`}>
                                {transactionTypeLabel(tx)}
                              </span>
                              {tx.status === "voided" && (
                                <span className={`inline-flex rounded-full px-2.5 py-1 text-[10px] font-black ring-1 ${dk ? "bg-rose-400/10 text-rose-300 ring-rose-400/20" : "bg-rose-100 text-rose-600 ring-rose-300/60"}`}>
                                  لغو شده
                                </span>
                              )}
                            </div>
                          ...
                          <td className="px-4 py-3.5 md:px-7">
                            <details className="relative">
                              ...
                              <ul className={`fx-pop mt-2 w-44 space-y-1 rounded-xl border p-1.5 shadow-xl ${dk ? "border-slate-600 bg-slate-900 shadow-black/40" : "border-slate-200 bg-white shadow-slate-900/10"}`}>
                                <li>
                                  <button onClick={() => editTransaction(tx)} disabled={tx.status === "voided"} className={actionButtonClass}>
                                    <Ic n="pencil" className="h-3.5 w-3.5" /> ویرایش
                                  </button>
                                </li>
                                <li>
                                  <button onClick={() => printReceipt(tx)} className={actionButtonClass}>
                                    <Ic n="printer" className="h-3.5 w-3.5" /> چاپ رسید
                                  </button>
                                </li>
                                <li>
                                  <button onClick={() => viewTransaction(tx)} className={actionButtonClass}>
                                    <Ic n="eye" className="h-3.5 w-3.5" /> مشاهده
                                  </button>
                                </li>
                                <li>
                                  <button onClick={() => voidTransaction(tx)} disabled={tx.status === "voided"} className={dangerActionButtonClass}>
      ...
      {selectedTransaction && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/60 p-3 md:p-4 backdrop-blur-sm" onClick={() => setSelectedTransaction(null)}>
          <div
            className={`fx-pop w-full max-w-lg overflow-hidden rounded-xl md:rounded-2xl border shadow-2xl ${dk ? "border-slate-600 bg-slate-900" : "border-slate-200 bg-white"}`}
          ...
          >
            <div className={`flex items-center justify-between border-b px-4 md:px-5 py-3 md:py-4 ${dk ? "border-slate-700 bg-slate-800/60" : "border-slate-100 bg-slate-50"}`}>
              <b className={`flex items-center gap-2 text-sm ${dk ? "text-slate-100" : "text-slate-800"}`}>
                <span className={`grid h-8 w-8 place-items-center rounded-lg ${dk ? "bg-cyan-400/10 text-cyan-300" : "bg-sky-100 text-sky-600"}`}>
              ...
              <button
                ...
                className={`grid h-8 w-8 cursor-pointer place-items-center rounded-lg text-slate-400 transition-all duration-300 hover:rotate-90 ${dk ? "hover:bg-slate-700 hover:text-white" : "hover:bg-slate-100 hover:text-slate-700"}`}
              >
            ...
            <div className="max-h-[70vh] overflow-y-auto px-4 md:px-5 py-2">
              ...
              <DetailRow
                dark={dk}
                label="وضعیت"
                value={selectedTransaction.status === "voided" ? "لغو شده" : "فعال"}
                valueClass={
                  selectedTransaction.status === "voided"
                    ? dk ? "text-rose-300" : "text-rose-500"
