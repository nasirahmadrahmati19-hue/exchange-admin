...
type Customer = { id: string | number; name: string; phone?: string; telegram?: string; telegramChatId?: string; balances: Record<Currency, number>; };
type Transaction = {
  id: string; trackingCode: string; type: "exchange" | "transfer" | "convert"; dealType?: DealType; date: string;
  customerId?: string | number; customerName?: string; senderId?: string | number; senderName?: string; receiverId?: string | number; receiverName?: string;
...
type BalanceChange = { customerId?: string | number; customerName: string; currency: Currency; amount: number; };
...
function getTelegramSettings() {
  try {
    const r = typeof window !== "undefined" ? localStorage.getItem("fx-settings") : null;
...
function applyBalanceChanges(customers: Customer[], changes: BalanceChange[]): Customer[] {
  return customers.map(c => {
    ...
    const cc = changes.filter(ch => String(ch.customerId) === String(c.id));
...
function getBalanceChangesForTransaction(tx: Transaction, action: "register" | "reverse"): BalanceChange[] {
  ...
  if (tx.type === "exchange" && tx.customerId && String(tx.customerId) !== String(CASH_BOX_ID) && String(tx.customerId) !== String(EXCHANGE_ACCOUNT_ID)) {
    if (tx.dealType === "sell") {
  ...
  if (tx.type === "transfer") {
    if (tx.senderId && String(tx.senderId) !== String(CASH_BOX_ID) && String(tx.senderId) !== String(EXCHANGE_ACCOUNT_ID)) {
    ...
    if (tx.receiverId && String(tx.receiverId) !== String(CASH_BOX_ID) && String(tx.receiverId) !== String(EXCHANGE_ACCOUNT_ID)) {
  ...
  if (tx.type === "convert" && tx.customerId && String(tx.customerId) !== String(CASH_BOX_ID) && String(tx.customerId) !== String(EXCHANGE_ACCOUNT_ID)) {
    changes.push({ customerId: tx.customerId, customerName: tx.customerName || "", currency: tx.fromCurrency, amount: -tx.fromAmount * sign });
    changes.push({ customerId: tx.customerId, customerName: tx.customerName || "", currency: tx.toCurrency, amount: tx.toAmount * sign });
    if (tx.commission && tx.commission > 0 && tx.commissionCurrency) {
      changes.push({ customerId: tx.customerId, customerName: tx.customerName || "", currency: tx.commissionCurrency, amount: -tx.commission * sign });
...
function syncCashEntriesForTransfer(action: "add" | "remove" | "replace", tx: Transaction | null, oldTxId: string | undefined, currentEntries: any[]): any[] {
  ...
  if ((action === "add" || action === "replace") && tx) {
    const dateStr = tx.date || new Date().toISOString();
    const newEntries: any[] = [];
    if (String(tx.senderId) === String(EXCHANGE_ACCOUNT_ID)) {
    ...
    } else if (String(tx.receiverId) === String(EXCHANGE_ACCOUNT_ID)) {
...
function syncCashEntriesForConvert(action: "add" | "remove" | "replace", tx: Transaction | null, oldTxId: string | undefined, currentEntries: any[]): any[] {
  ...
  if ((action === "add" || action === "replace") && tx) {
    const dateStr = tx.date || new Date().toISOString();
    const newEntries: any[] = [];
    if (String(tx.customerId) === String(EXCHANGE_ACCOUNT_ID)) {
...
const iconPaths = {
  ...
  briefcase: "M20 7h-4V5a2 2 0 0 0-2-2h-4a2 2 0 0 0-2 2v2H4a2 2 0 0 0-2 2v10a2 2 0 0 0 2 2h16a2 2 0 0 0 2-2V9a2 2 0 0 0-2-2Zm-6 0h-4V5h4v2Z"
};
...
const ActionButtons = memo(function ActionButtons({
  ...
  const btnBase = isDark ? "text-slate-200 hover:bg-slate-700/60" : "text-slate-700 hover:bg-slate-100";
  return (
    <div className="relative action-dropdown flex justify-center">
      <button onClick={(e) => { e.stopPropagation(); setOpenActionId(isOpen ? null : tx.id); }} className={`grid h-8 w-8 place-items-center rounded-lg border transition-all duration-150 active:scale-90 cursor-pointer ${isDark ? "border-slate-600 text-slate-300 hover:bg-slate-700" : "border-slate-200 text-slate-500 hover:bg-slate-100"}`} title="عملیات">
        <Ic n="more" className="h-4 w-4" />
      </button>
      {isOpen && (
        <div className={`absolute left-1/2 -translate-x-1/2 top-full z-50 mt-1.5 w-44 overflow-hidden rounded-xl border shadow-xl ${isDark ? "border-slate-600 bg-slate-800" : "border-slate-200 bg-white"}`}>
          <button onClick={() => { viewTransaction(tx); setOpenActionId(null); }} className={`${btn} ${btnBase} ${isDark ? "text-cyan-300 hover:bg-cyan-400/15" : "text-cyan-600 hover:bg-cyan-50"}`}>
            <Ic n="eye" className="h-3.5 w-3.5" /> مشاهده
          </button>
          {!isVoided && (
            <>
              <button onClick={() => { editTransaction(tx); setOpenActionId(null); }} className={`${btn} ${btnBase} ${isDark ? "text-sky-300 hover:bg-sky-400/15" : "text-sky-600 hover:bg-sky-50"}`}>
                <Ic n="pencil" className="h-3.5 w-3.5" /> ویرایش
              </button>
              <button onClick={() => { voidTransaction(tx); setOpenActionId(null); }} className={`${btn} ${btnBase} ${isDark ? "text-amber-300 hover:bg-amber-400/15" : "text-amber-600 hover:bg-amber-50"}`}>
                <Ic n="xCircle" className="h-3.5 w-3.5" /> لغو
              </button>
            </>
          )}
          <button onClick={() => { printReceipt(tx); setOpenActionId(null); }} className={`${btn} ${btnBase} ${isDark ? "text-emerald-300 hover:bg-emerald-400/15" : "text-emerald-600 hover:bg-emerald-50"}`}>
            <Ic n="printer" className="h-3.5 w-3.5" /> چاپ رسید
          </button>
          <div className={`my-1 h-px ${isDark ? "bg-slate-700" : "bg-slate-100"}`} />
          <button onClick={() => { deleteTransaction(tx); setOpenActionId(null); }} className={`${btn} ${isDark ? "text-rose-300 hover:bg-rose-400/15" : "text-rose-500 hover:bg-rose-50"}`}>
...
const TransactionRow = memo(({ tx, index, isSearching, isDark, transactionCustomerLabel, openActionId, setOpenActionId, viewTransaction, editTransaction, voidTransaction, printReceipt, deleteTransaction }: {
  ...
  return (
    <tr className={rc}>
      <td className={cellClass}><span className={`inline-grid h-7 w-7 place-items-center rounded-lg text-[11px] font-black tabular-nums ${isDark ? "bg-slate-700 text-slate-300" : "bg-slate-100 text-slate-600"}`}>{index + 1}</span></td>
...
const MobileTransactionCard = memo(({ tx, index, isDark, transactionCustomerLabel, openActionId, setOpenActionId, viewTransaction, editTransaction, voidTransaction, printReceipt, deleteTransaction }: {
  ...
  const subTextLocal = isDark ? "text-slate-500" : "text-slate-400";
  return (
    <div className={`rounded-xl border p-4 mb-3 overflow-visible ${isDark ? "border-slate-700 bg-slate-800/50" : "border-slate-200 bg-white"}`}>
...
export default function CurrencyExchangePage() {
  // ✅ تغییر ۳: استفاده از هوک جدید و نام‌های کالکشن استاندارد (بدون [] داخل <>)
  const [customers, setCustomers] = useSafeSyncedState<Customer>("customers", []);
  const [transactions, setTransactions] = useSafeSyncedState<Transaction>("transactions", []);
  const [cashEntries, setCashEntries] = useSafeSyncedState<any>("cash_entries", []);
  const [hawalas] = useSafeSyncedState<any>("hawalas", []);

  const [isSubmitting, setIsSubmitting] = useState(false);

  useEffect(() => {
    if (!customers.find(c => String(c.id) === String(EXCHANGE_ACCOUNT_ID))) {
  ...
  const filteredCustomerList = useMemo(() => {
    ...
    const normal = customers.filter(c => String(c.id) !== String(CASH_BOX_ID) && String(c.id) !== String(EXCHANGE_ACCOUNT_ID) && (!q || c.name.toLowerCase().includes(q) || (c.phone && normalizeDigits(c.phone).includes(q))));
    const matchExchange = !q || EXCHANGE_ACCOUNT_NAME.toLowerCase().includes(q) || "حساب".includes(q) || "صرافی".includes(q);
  ...
  const selectedCustomer = useMemo(() => isCustomerExchangeAccount ? EXCHANGE_ACCOUNT_CUSTOMER : customers.find(c => c.name === customer) || null, [customers, customer, isCustomerExchangeAccount]);
  const selectedSender = useMemo(() => isSenderExchangeAccount ? EXCHANGE_ACCOUNT_CUSTOMER : customers.find(c => c.name === sender) || null, [customers, sender, isSenderExchangeAccount]);
  const selectedReceiver = useMemo(() => isReceiverExchangeAccount ? EXCHANGE_ACCOUNT_CUSTOMER : customers.find(c => c.name === receiver) || null, [customers, receiver, isReceiverExchangeAccount]);
  const selectedConvertCustomer = useMemo(() => isConvertCustomerExchangeAccount ? EXCHANGE_ACCOUNT_CUSTOMER : customers.find(c => c.name === convertCustomer) || null, [customers, convertCustomer, isConvertCustomerExchangeAccount]);
  ...
  const submitExchange = useCallback(() => {
    ...
    const tx: Transaction = {
      ...
      trackingCode: editingExchangeId ? (transactions.find(t => t.id === editingExchangeId)?.trackingCode || "PENDING") : "PENDING",
      ...
      customerId: selC?.id, customerName: selC?.name || customer,
  ...
  const submitTransfer = useCallback(() => {
    ...
    const tx: Transaction = {
      ...
      trackingCode: editingTransferId ? (transactions.find(t => t.id === editingTransferId)?.trackingCode || "PENDING") : "PENDING",
      type: "transfer",
      date: editingTransferId ? (transactions.find(t => t.id === editingTransferId)?.date || new Date().toISOString()) : new Date().toISOString(),
      senderId: selS?.id, senderName: selS?.name || sender,
      receiverId: selR?.id, receiverName: selR?.name || receiver,
  ...
  const submitConvert = useCallback(() => {
    ...
    const tx: Transaction = {
      ...
      trackingCode: editingConvertId ? (transactions.find(t => t.id === editingConvertId)?.trackingCode || "PENDING") : "PENDING",
      type: "convert",
      date: editingConvertId ? (transactions.find(t => t.id === editingConvertId)?.date || new Date().toISOString()) : new Date().toISOString(),
      customerId: selC?.id, customerName: selC?.name || convertCustomer,
  ...
  const confirmRegister = useCallback(async () => {
    ...
    try {
      let finalTrackingCode = previewData.trackingCode;
      if (!editingExchangeId && !editingTransferId && !editingConvertId) {
        try {
          finalTrackingCode = await consumeTrackingCode();
        } catch (error) {
          console.error("خطا در تولید کد پیگیری:", error);
          finalTrackingCode = `TR-ERROR-${Date.now().toString().slice(-5)}`;
        }
      }
      const tx = { ...previewData, trackingCode: finalTrackingCode };
      ...
      if (editingExchangeId) {
        ...
        setCustomers(prev => {
          ...
          if (oldTx && oldTx.status !== "voided") {
            updated = applyBalanceChanges(updated, getBalanceChangesForTransaction(oldTx, "reverse"));
          }
          updated = applyBalanceChanges(updated, getBalanceChangesForTransaction(tx, "register"));
          return updated;
        });
        setTransactions(p => p.map(t => t.id === editingExchangeId ? { ...tx, id: editingExchangeId, trackingCode: t.trackingCode, date: t.date } : t));
        setCashEntries(prev => {
          let e = syncCashEntriesForExchange("remove", null, editingExchangeId, prev);
          return syncCashEntriesForExchange("add", tx, undefined, e);
        });
        ...
        setCashEntries(prev => {
          let e = syncCashEntriesForTransfer("remove", null, editingTransferId, prev);
          return syncCashEntriesForTransfer("add", tx, undefined, e);
        });
        ...
        setCashEntries(prev => {
          let e = syncCashEntriesForConvert("remove", null, editingConvertId, prev);
          return syncCashEntriesForConvert("add", tx, undefined, e);
        });
      } else {
        setTransactions(x => [...x, tx]);
        if (tx.type === "exchange") setCashEntries(prev => syncCashEntriesForExchange("add", tx, undefined, prev));
        else if (tx.type === "transfer") setCashEntries(prev => syncCashEntriesForTransfer("add", tx, undefined, prev));
        else if (tx.type === "convert") setCashEntries(prev => syncCashEntriesForConvert("add", tx, undefined, prev));
      ...
      try {
        ...
        if (settings.enabled && settings.botToken) {
          const recipientIds = new Set<string | number>();
          if (tx.type === "exchange" || tx.type === "convert") {
            if (tx.customerId && String(tx.customerId) !== String(CASH_BOX_ID) && String(tx.customerId) !== String(EXCHANGE_ACCOUNT_ID)) recipientIds.add(tx.customerId);
          } else if (tx.type === "transfer") {
            if (tx.senderId && String(tx.senderId) !== String(CASH_BOX_ID) && String(tx.senderId) !== String(EXCHANGE_ACCOUNT_ID)) recipientIds.add(tx.senderId);
            if (tx.receiverId && String(tx.receiverId) !== String(CASH_BOX_ID) && String(tx.receiverId) !== String(EXCHANGE_ACCOUNT_ID)) recipientIds.add(tx.receiverId);
          }
          ...
          for (const custId of recipientIds) {
            const cust = updatedCustomers.find(c => String(c.id) === String(custId));
            if (!cust) continue;
            ...
            if (tx.type === "exchange" && String(tx.customerId) === String(custId)) {
              if (tx.dealType === "sell") { changeAmount = -tx.fromAmount; changeCurrency = tx.fromCurrency; }
              else if (tx.dealType === "buy") { changeAmount = tx.toAmount; changeCurrency = tx.toCurrency; }
            } else if (tx.type === "transfer") {
              if (String(tx.senderId) === String(custId)) { changeAmount = -tx.fromAmount; changeCurrency = tx.fromCurrency; }
              else if (String(tx.receiverId) === String(custId)) { changeAmount = tx.toAmount; changeCurrency = tx.toCurrency; }
            } else if (tx.type === "convert" && String(tx.customerId) === String(custId)) {
              changeAmount = tx.toAmount; changeCurrency = tx.toCurrency;
            }
            ...
            if (tx.type === "exchange") {
              ...
              const isSender = String(tx.senderId) === String(custId);
            ...
            for (const cur of currencies) {
              ...
              if (b > 0) text += `${label}: ${fmt(b)} طلب\n`;
              else if (b < 0) text += `${label}: ${fmt(Math.abs(b))} قرض\n`;
              else text += `${label}: 0\n`;
            }
  ...
  }, [previewData, isSubmitting, editingExchangeId, editingTransferId, editingConvertId, transactions, customers, resetExchangeForm, resetTransferForm, resetConvertForm, setCustomers, setTransactions, setCashEntries]);

  const customerName = useCallback((id?: string | number) => {
    if (String(id) === String(EXCHANGE_ACCOUNT_ID)) return EXCHANGE_ACCOUNT_NAME;
    return customers.find(c => String(c.id) === String(id))?.name || String(id || "-");
  }, [customers]);
  ...
  const editTransaction = useCallback((tx: Transaction) => {
    ...
    if (tx.type === "exchange") {
      ...
      setCustomer(tx.customerName || ""); setExchangeDealType(tx.dealType || "");
    ...
    if (tx.type === "transfer") {
      ...
      setSender(tx.senderName || ""); setReceiver(tx.receiverName || "");
    ...
    if (tx.type === "convert") {
      ...
      setConvertCustomer(tx.customerName || ""); setConvertFromCurrency(tx.fromCurrency); setConvertToCurrency(tx.toCurrency);
  ...
  const voidTransaction = useCallback((tx: Transaction) => {
    ...
    if (tx.type === "exchange") setCashEntries(prev => syncCashEntriesForExchange("remove", null, tx.id, prev));
    else if (tx.type === "transfer") setCashEntries(prev => syncCashEntriesForTransfer("remove", null, tx.id, prev));
    else if (tx.type === "convert") setCashEntries(prev => syncCashEntriesForConvert("remove", null, tx.id, prev));
    setEditingExchangeId(null); setEditingTransferId(null); setEditingConvertId(null);
  }, [setCustomers, setTransactions, setCashEntries]);
  ...
  const printReceipt = useCallback((tx: Transaction) => {
    ...
    if (!w) return;
    w.document.write(`<html dir="rtl"><head><meta charset="utf-8"/><title>رسید</title><style>body{font-family:Tahoma;padding:24px;direction:rtl}table{width:100%;border-collapse:collapse;font-size:12px;margin:12px 0}th,td{border:1px solid #ccc;padding:6px 8px;text-align:right}th{background:#f0f9ff}</style></head><body><h2>رسید</h2><table><tr><th>کد</th><td>${tx.trackingCode}</td></tr><tr><th>تاریخ</th><td>${dateLabel(tx.date)}</td></tr><tr><th>مشتری</th><td>${transactionCustomerLabel(tx)}</td></tr><tr><th>دریافت</th><td>${fmt(tx.fromAmount)} ${labels[tx.fromCurrency]}</td></tr><tr><th>پرداخت</th><td>${fmt(tx.toAmount)} ${labels[tx.toCurrency]}</td></tr><tr><th>وضعیت</th><td>${tx.status === "voided" ? "لغو" : "فعال"}</td></tr></table></body></html>`);
    w.document.close(); w.focus(); w.print();
  }, [transactionCustomerLabel]);

  const getSharedCustomerBalances = useCallback((customerId: string | number): Record<Currency, number> => {
    const balances: Record<Currency, number> = { AFN: 0, USD: 0, EUR: 0, IRR: 0, PKR: 0 };
    if (!customerId || String(customerId) === String(CASH_BOX_ID) || String(customerId) === String(EXCHANGE_ACCOUNT_ID)) return balances;
    ...
    for (const tx of transactions) {
      ...
      if (tx.type === "exchange" && String(tx.customerId) === String(customerId)) {
        if (tx.dealType === "sell") {
    ...
    for (const ce of cashEntries) {
      if (!ce || ce.status === "voided" || String(ce.customerId) !== String(customerId)) continue;
  ...
  const CustomerBalanceCard = memo(({ customer, color, isCashBox }: { customer: Customer | null; color: "cyan" | "orange" | "violet"; isCashBox?: boolean }) => {
    ...
    const balances = String(customer.id) === String(EXCHANGE_ACCOUNT_ID) ? computeExchangeBalances(cashEntries) : getSharedCustomerBalances(customer.id);
    const title = String(customer.id) === String(EXCHANGE_ACCOUNT_ID) ? "💼 موجودی حساب صرافی" : `موجودی حساب ${customer.name}`;
    const colors = {
            ...
            return (
              <div key={cur} className={`rounded-lg px-2 py-1.5 ${dk ? "bg-slate-900/50" : "bg-white"}`}>
                ...
                <div className="min-h-[12px] mt-0.5">
                  {String(customer.id) === String(EXCHANGE_ACCOUNT_ID) ? (
                    <>
    ...
    return (
      <button key={String(c.id)} type="button" onClick={() => onSelect(c)} className={`flex w-full items-center gap-2 px-3 py-2.5 text-right text-xs font-bold transition ${
        ...
        <span className="flex-1 truncate">
          {c.name}
          {isExchangeAcc && <span className="mr-1 text-[9px] font-black opacity-70">(حساب داخلی)</span>}
        </span>
  ...
  const secHead = (ic: string, icon: IconName, t: string, s: string, cc: string, ct: string) => (
    <div className="flex flex-wrap items-center gap-3">
      ...
      <div className="flex-1 min-w-0">
        <h2 className={`text-xl md:text-2xl ${heading}`}>{t}</h2>
  ...
  return (
    <div dir="rtl" className={dk ? "dark" : ""}>
      <style>{`@import url("https://fonts.googleapis.com/css2?family=Lalezar&family=Vazirmatn:wght@300;400;500;600;700;800;900&display=swap");.cs-font{font-family:"Vazirmatn","Segoe UI",Tahoma,sans-serif}.cs-display{font-family:"Lalezar","Vazirmatn",Tahoma,sans-serif}.dark{color-scheme:dark}`}</style>
      <div className={`cs-font relative min-h-screen ${dk ? "bg-[#0f172a] text-slate-100" : "bg-[#eef6fa] text-slate-800"}`}>
        ...
        <div className="relative z-10 mx-auto w-full max-w-7xl space-y-4 md:space-y-6 px-3 pb-16 pt-5 md:px-8 md:pt-9">
          <header className="flex flex-wrap items-center justify-between gap-3">
            <div className="flex items-center gap-2.5 min-w-0">
              ...
              <div className="min-w-0">
                <h1 className={`text-2xl md:text-4xl ${heading}`}>معاملات ارزی</h1>
          ...
          {tab === "exchange" && (
            <section className={`space-y-4 p-4 md:p-7 ${uiCard}`}>
                    ...
                    {showCustomerList && (
                      <div className={`absolute left-0 top-full z-30 mt-1 w-full max-h-60 overflow-y-auto rounded-xl border shadow-xl ${dk ? "border-slate-600 bg-slate-800" : "border-slate-200 bg-white"}`}>
                        {filteredCustomerList.length === 0 ? (
                          ...
                          filteredCustomerList.map((c, idx) => renderCustomerItem(c, idx, (sel) => {
                            setCustomer(sel.name); setCustomerPhone(sel.phone || ""); setCustomerTelegram(sel.telegram || "");
                            setCustomerFilter(""); setShowCustomerList(false); setExchangeErrors(p => ({ ...p, customer: undefined }));
                          }, dk ? "text-slate-200 hover:bg-cyan-400/15 hover:text-cyan-300" : "text-slate-700 hover:bg-cyan-50 hover:text-cyan-600", "bg-gradient-to-br from-cyan-500 to-sky-500"))
                        )}
              ...
              <button onClick={submitExchange} disabled={isSubmitting} className={`flex h-[50px] w-full cursor-pointer items-center justify-center gap-2 rounded-xl bg-gradient-to-l text-base font-black shadow-lg ${isSubmitting ? 'opacity-70 cursor-not-allowed' : ''} ${dk ? "from-cyan-400 to-emerald-400 text-slate-950" : "from-sky-500 to-emerald-400 text-white"}`}>
                {isSubmitting ? 'در حال ثبت...' : (editingExchangeId ? "به‌روزرسانی" : "ثبت")}
          ...
          {tab === "transfer" && (
            <section className={`space-y-4 p-4 md:p-7 ${uiCard}`}>
                          ...
                          <div className={`absolute left-0 top-full z-30 mt-1 w-full max-h-60 overflow-y-auto rounded-xl border shadow-xl ${dk ? "border-slate-600 bg-slate-800" : "border-slate-200 bg-white"}`}>
                            {filteredSenderList.length === 0 ? (
                              ...
                              filteredSenderList.map((c, idx) => renderCustomerItem(c, idx, (sel) => {
                                setSender(sel.name); setSenderPhone(sel.phone || ""); setSenderTelegram(sel.telegram || "");
                                setSenderFilter(""); setShowSenderList(false); setTransferErrors(p => ({ ...p, sender: undefined }));
                              }, dk ? "text-slate-200 hover:bg-orange-400/15 hover:text-orange-300" : "text-slate-700 hover:bg-orange-50 hover:text-orange-600", "bg-gradient-to-br from-orange-500 to-amber-500"))
                            )}
                            <div className={`h-px ${dk ? "bg-slate-700" : "bg-slate-100"}`} />
                            <div className={`px-3 py-2 text-[10px] text-center ${subText}`}>💼 حساب صرافی همیشه در دسترس است</div>
                          </div>
                          ...
                          <div className={`absolute left-0 top-full z-30 mt-1 w-full max-h-60 overflow-y-auto rounded-xl border shadow-xl ${dk ? "border-slate-600 bg-slate-800" : "border-slate-200 bg-white"}`}>
                            {filteredReceiverList.length === 0 ? (
                              ...
                              filteredReceiverList.map((c, idx) => renderCustomerItem(c, idx, (sel) => {
                                setReceiver(sel.name); setReceiverPhone(sel.phone || ""); setReceiverTelegram(sel.telegram || "");
                                setReceiverFilter(""); setShowReceiverList(false); setTransferErrors(p => ({ ...p, receiver: undefined }));
                              }, dk ? "text-slate-200 hover:bg-emerald-400/15 hover:text-emerald-300" : "text-slate-700 hover:bg-emerald-50 hover:text-emerald-600", "bg-gradient-to-br from-emerald-500 to-teal-500"))
                            )}
                            <div className={`h-px ${dk ? "bg-slate-700" : "bg-slate-100"}`} />
                            <div className={`px-3 py-2 text-[10px] text-center ${subText}`}>💼 حساب صرافی همیشه در دسترس است</div>
                          </div>
              ...
              <button onClick={submitTransfer} disabled={isSubmitting} className={`flex h-[50px] w-full cursor-pointer items-center justify-center gap-2 rounded-xl bg-gradient-to-l text-base font-black shadow-lg ${isSubmitting ? 'opacity-70 cursor-not-allowed' : ''} ${dk ? "from-orange-400 to-amber-300 text-slate-950" : "from-orange-500 to-amber-400 text-white"}`}>
                {isSubmitting ? 'در حال ثبت...' : (editingTransferId ? "به‌روزرسانی" : "ثبت")}
                <Ic n="arrowLeft" className="h-5 w-5" />
              </button>
            </section>
          )}
          ...
          {tab === "convert" && (
            <section className={`space-y-4 p-4 md:p-7 ${uiCard}`}>
                    ...
                    {showConvertList && (
                      <div className={`absolute left-0 top-full z-30 mt-1 w-full max-h-60 overflow-y-auto rounded-xl border shadow-xl ${dk ? "border-slate-600 bg-slate-800" : "border-slate-200 bg-white"}`}>
                        {filteredConvertList.length === 0 ? (
                          ...
                          filteredConvertList.map((c, idx) => renderCustomerItem(c, idx, (sel) => {
                            setConvertCustomer(sel.name); setConvertCustomerPhone(sel.phone || ""); setConvertCustomerTelegram(sel.telegram || "");
                            setConvertFilter(""); setShowConvertList(false); setConvertErrors(p => ({ ...p, customer: undefined }));
                          }, dk ? "text-slate-200 hover:bg-violet-400/15 hover:text-violet-300" : "text-slate-700 hover:bg-violet-50 hover:text-violet-600", "bg-gradient-to-br from-violet-500 to-purple-500"))
                        )}
                        <div className={`h-px ${dk ? "bg-slate-700" : "bg-slate-100"}`} />
                        <div className={`px-3 py-2 text-[10px] text-center ${subText}`}>💼 حساب صرافی همیشه در دسترس است</div>
                      </div>
              ...
              <button onClick={submitConvert} disabled={isSubmitting} className={`flex h-[50px] w-full cursor-pointer items-center justify-center gap-2 rounded-xl bg-gradient-to-l text-base font-black shadow-lg ${isSubmitting ? 'opacity-70 cursor-not-allowed' : ''} ${dk ? "from-violet-400 to-purple-400 text-slate-950" : "from-violet-500 to-fuchsia-400 text-white"}`}>
                {isSubmitting ? 'در حال ثبت...' : (editingConvertId ? "به‌روزرسانی" : "ثبت")}
                <Ic n="arrowLeft" className="h-5 w-5" />
              </button>
            </section>
          )}
          ...
          <section className={`overflow-hidden ${uiCard}`}>
            <div className="flex flex-wrap items-center gap-3 p-4 md:p-5">
              ...
              <div className="flex-1 min-w-0"><h2 className={`text-xl md:text-2xl ${heading}`}>آخرین معاملات</h2></div>
          ...
          <div className={`w-full max-w-2xl overflow-hidden rounded-xl md:rounded-2xl border shadow-2xl ${dk ? "border-slate-600 bg-slate-900" : "border-slate-200 bg-white"}`} onClick={e => e.stopPropagation()}>
            ...
            <div className="max-h-[75vh] overflow-y-auto px-4 md:px-5 py-4 space-y-4">
              <div className={`flex flex-wrap items-center justify-between gap-3 rounded-xl border p-3 ${dk ? "border-cyan-400/30 bg-cyan-400/10" : "border-cyan-300 bg-cyan-50"}`}>
                <div>
                  ...
                  <span className={`inline-flex items-center gap-1.5 text-[14px] font-black tabular-nums ${dk ? "text-cyan-300" : "text-cyan-700"}`} dir="ltr">
                    <Ic n="tag" className="h-4 w-4" />{previewData.trackingCode === "PENDING" ? "در حال تولید..." : previewData.trackingCode}
                  </span>
                </div>
                ...
                <div className={`rounded-xl border p-4 ${dk ? "border-cyan-400/20 bg-cyan-400/5" : "border-cyan-200 bg-cyan-50/50"}`}>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-sm">
                    <div>
                      ...
                      {String(previewData.customerId) === String(EXCHANGE_ACCOUNT_ID) && <span className={`mr-2 text-[10px] font-black ${dk ? "text-sky-300" : "text-sky-600"}`}>💰 حساب صرافی</span>}
                      {previewData.customerId && String(previewData.customerId) !== String(CASH_BOX_ID) && <span className={`mr-2 text-[10px] font-black ${dk ? "text-emerald-300" : "text-emerald-600"}`}>✅ مشتری ثبت‌شده</span>}
                  ...
                  <div className={`rounded-xl border p-4 ${dk ? "border-orange-400/20 bg-orange-400/5" : "border-orange-200 bg-orange-50/50"}`}>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-sm">
                      <div>
                        ...
                        {String(previewData.senderId) === String(EXCHANGE_ACCOUNT_ID) && <span className={`mr-2 text-[10px] font-black ${dk ? "text-sky-300" : "text-sky-600"}`}>💰 حساب صرافی</span>}
                        {previewData.senderId && String(previewData.senderId) !== String(CASH_BOX_ID) && <span className={`mr-2 text-[10px] font-black ${dk ? "text-emerald-300" : "text-emerald-600"}`}>✅ مشتری ثبت‌شده</span>}
                  ...
                  <div className={`rounded-xl border p-4 ${dk ? "border-emerald-400/20 bg-emerald-400/5" : "border-emerald-200 bg-emerald-50/50"}`}>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-sm">
                      <div>
                        ...
                        {String(previewData.receiverId) === String(EXCHANGE_ACCOUNT_ID) && <span className={`mr-2 text-[10px] font-black ${dk ? "text-sky-300" : "text-sky-600"}`}>💰 حساب صرافی</span>}
                        {previewData.receiverId && String(previewData.receiverId) !== String(CASH_BOX_ID) && <span className={`mr-2 text-[10px] font-black ${dk ? "text-emerald-300" : "text-emerald-600"}`}>✅ مشتری ثبت‌شده</span>}
              ...
              <div className="flex flex-wrap gap-3 pt-2">
                <button onClick={confirmRegister} disabled={isSubmitting} className={`flex h-[48px] flex-1 min-w-[180px] cursor-pointer items-center justify-center gap-2 rounded-xl text-sm font-black shadow-lg ${isSubmitting ? 'opacity-70 cursor-not-allowed' : ''} ${dk ? "bg-emerald-400 text-slate-950" : "bg-emerald-500 text-white"}`}>
                  {isSubmitting ? 'در حال پردازش...' : 'ثبت نهایی'}
                  <Ic n="check" className="h-4 w-4" />
                </button>
                <button onClick={() => { setPreviewOpen(false); setPreviewData(null); }} disabled={isSubmitting} className={`flex h-[48px] px-6 cursor-pointer items-center justify-center rounded-xl border text-sm font-bold ${dk ? "border-slate-600 text-slate-300" : "border-slate-200 text-slate-600"}`}>انصراف</button>
              </div>
            </div>
          </div>
        </div>
