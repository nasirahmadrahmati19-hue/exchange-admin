  ...
  const ActionButtons = memo(function ActionButtons({ tx }: { tx: Transaction }) {
    ...
    return (
      <div className="flex items-center gap-1.5">
        <button title="مشاهده" onClick={() => viewTransaction(tx)} className={`${btn} ${dk ? "border-cyan-400/30 text-cyan-300 hover:bg-cyan-400/15" : "border-cyan-200 text-cyan-600 hover:bg-cyan-50"}`}>
          <Ic n="eye" className="h-3.5 w-3.5" />
        </button>
        {!isVoided && (
          <>
            <button title="ویرایش" onClick={() => editTransaction(tx)} className={`${btn} ${dk ? "border-sky-400/30 text-sky-300 hover:bg-sky-400/15" : "border-sky-200 text-sky-600 hover:bg-sky-50"}`}>
        ...
        <button title="چاپ" onClick={() => printReceipt(tx)} className={`${btn} ${dk ? "border-emerald-400/30 text-emerald-300 hover:bg-emerald-400/15" : "border-emerald-200 text-emerald-600 hover:bg-emerald-50"}`}>
