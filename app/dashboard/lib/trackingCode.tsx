...
// ✅ اصلاح ۱: استفاده از "PENDING" به جای getNextTrackingCode()
...
// ✅ اصلاح ۲: استفاده از "PENDING" به جای getNextTrackingCode()
...
// ✅ اصلاح ۳: استفاده از "PENDING" به جای getNextTrackingCode()
...
// ✅ اصلاح ۴: تولید کد پیگیری فقط در حالت ثبت جدید (نه ویرایش)
const confirmRegister = useCallback(async () => {
if (!previewData) return;

let finalTrackingCode = previewData.trackingCode;
const isNewRecord = !editingExchangeId && !editingTransferId && !editingConvertId;

// فقط در حالت ثبت جدید کد پیگیری واقعی تولید کن
if (isNewRecord) {
try {
finalTrackingCode = await consumeTrackingCode();
} catch (error) {
console.error("خطا در تولید کد پیگیری:", error);
const year = new Intl.DateTimeFormat("en-US-u-ca-persian-nu-latn", { year: "numeric" }).formatToParts(new Date()).find(p => p.type === "year")?.value || "1405";
finalTrackingCode = `TR-${year}-${String(Date.now()).slice(-5)}`;
}
}
