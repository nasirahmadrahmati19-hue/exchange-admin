...
export default function CashBoxPage() {
  ...
  useEffect(() => {
    try { const saved = window.localStorage.getItem("cashbox-theme"); if (saved === "dark" || saved === "light") setTheme(saved); } catch {}
  }, []);
  useEffect(() => { try { window.localStorage.setItem("cashbox-theme", theme); } catch {} }, [theme]);
  const dk = theme === "dark";
  ...
  const handleSubmit = useCallback(() => {
    ...
    let customer = customers.find(c => c.id === selectedCustomerId || c.name === selectedCustomerName.trim());
