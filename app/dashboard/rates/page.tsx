...
export default function CashPage() {
  ...
  const saveClose = useCallback(() => {
    ...
    for (const cur of currencies) {
      ...
      const entry: CashEntry = {
        id: generateId(),
        trackingCode: consumeTrackingCode(),
        date: nowDate.toISOString(),
        type: "adjustment",
        currency: cur,
        amount: Math.abs(r.diff),
        direction: r.diff > 0 ? "in" : "out",
        reason: `اصلاح صندوق - ${r.diff > 0 ? "اضافه" : "کسری"} شمارش واقعی`,
        balanceAfter: r.actual,
        closeActual: r.actual,
        closeDiff: r.diff,
      };
    ...
    if (newEntries.length > 0) {
      ...
      showToast("اصلاحات صندوق ثبت شد.");
    } else {
      showToast("موجودی صندوق دقیق است. نیازی به اصلاح نیست.");
    }
    setCloseResult(null);
          ...
          <header className="cs-up flex flex-wrap items-center justify-between gap-3">
            <div className="flex items-center gap-2.5 md:gap-3.5 min-w-0">
              <div className="relative grid h-11 w-11 md:h-14 md:w-14 shrink-0 place-items-center rounded-xl md:rounded-2xl bg-gradient-to-br from-emerald-500 via-teal-500 to-cyan-400 text-white shadow-lg shadow-emerald-500/30 ring-1 ring-white/30">
                ...
                <span className={`absolute -bottom-1 -left-1 md:-bottom-1.5 md:-left-1.5 grid h-4 min-w-4 md:h-5 md:min-w-5 place-items-center rounded-full bg-gradient-to-br from-amber-400 to-orange-400 px-1 text-[7px] md:text-[8px] font-black text-white ring-2 ${dk ? "ring-[#0f172a]" : "ring-[#ecfdf5]"}`}>CS</span>
              </div>
          ...
          {activeTab === "register" && (
            <section className={`cs-up space-y-4 md:space-y-5 p-4 md:p-7 ${uiCard}`} style={{ animationDelay: "160ms" }}>
              ...
              <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
                ...
                {fld("نوع ارز *", (
                  <div className="relative">
                    <select value={form.currency} onChange={e => setField("currency", e.target.value)} className={`${uiInput} cursor-pointer appearance-none pl-9`}>
                ...
                {fld("مبلغ *", (
                  <input type="text" inputMode="decimal" dir="ltr" value={form.amount} onChange={e => setField("amount", toNumericText(e.target.value))} placeholder="0" className={`${uiInput} text-left tabular-nums ${errors.amount ? errInput : ""}`} />
                ))}
                {fld("کد پیگیری", (
                  <div className="relative">
                    <input readOnly dir="ltr" value={nextCode} className={`${uiInput} ${roInput} pl-14 text-left tabular-nums font-black`} />
                    <span className="pointer-events-none absolute left-2.5 top-1/2 -translate-y-1/2 rounded-lg bg-gradient-to-r from-emerald-500 to-teal-500 px-2 py-1 text-[9px] font-black text-white">TR</span>
                  </div>
                ))}
              </div>
              ...
              <div className={`flex items-center gap-3 rounded-xl border p-4 ${isInType ? dk ? "border-emerald-400/25 bg-emerald-400/[0.07]" : "border-emerald-200 bg-emerald-50" : dk ? "border-rose-400/25 bg-rose-400/[0.07]" : "border-rose-200 bg-rose-50"}`}>
                <span className={`grid h-10 w-10 place-items-center rounded-xl ${isInType ? dk ? "bg-emerald-400/15 text-emerald-300" : "bg-emerald-100 text-emerald-600" : dk ? "bg-rose-400/15 text-rose-300" : "bg-rose-100 text-rose-600"}`}>
                  <Ic n={isInType ? "arrowDown" : "arrowUp"} className="h-5 w-5" />
                </span>
                <div>
                  <b className={`text-sm font-black ${isInType ? dk ? "text-emerald-300" : "text-emerald-700" : dk ? "text-rose-300" : "text-rose-700"}`}>
                    {isInType ? "افزایش موجودی صندوق" : "کاهش موجودی صندوق"}
                  </b>
              ...
              <button onClick={handleSubmitClick} className={`group flex h-[50px] md:h-[52px] w-full cursor-pointer items-center justify-center gap-2 rounded-xl bg-gradient-to-l text-base font-black shadow-lg transition-all duration-300 hover:shadow-xl hover:brightness-110 active:scale-[0.985] ${dk ? "from-emerald-400 to-teal-400 text-slate-950" : "from-emerald-500 via-teal-500 to-cyan-500 text-white"}`}>
                ثبت عملیات
          ...
          {activeTab === "close" && (
            <section className={`cs-up space-y-4 md:space-y-5 p-4 md:p-7 ${uiCard}`} style={{ animationDelay: "160ms" }}>
              <div className="flex flex-wrap items-center gap-3">
                ...
                <div className="flex-1 min-w-0">
                  <h2 className={`cs-display text-xl md:text-2xl leading-none ${heading}`}>بستن صندوق و شمارش واقعی</h2>
                  <p className={`mt-1 text-[11px] font-bold ${subText}`}>مقایسه موجودی ثبت‌شده با شمارش واقعی در پایان روز</p>
                </div>
              </div>
              ...
              <div className={`rounded-xl border p-4 ${dk ? "border-amber-400/25 bg-amber-400/[0.07]" : "border-amber-300 bg-amber-50"}`}>
                <div className="flex items-start gap-2">
                  ...
                  <span className={`text-xs leading-6 ${dk ? "text-amber-200" : "text-amber-800"}`}>
                    موجودی واقعی هر ارز را که در صندوق فیزیکی شمارش کرده‌اید وارد کنید. سیستم اختلاف را محاسبه و به‌صورت خودکار در روزنامچه ثبت می‌کند.
                  </span>
                </div>
              </div>
              ...
              {closeResult && (
                <div className={`rounded-xl border p-4 ${dk ? "border-slate-700 bg-slate-800/60" : "border-slate-200 bg-white"}`}>
                  <div className="flex items-center gap-2 mb-3">
                    <Ic n="check" className={`h-4 w-4 ${dk ? "text-emerald-300" : "text-emerald-600"}`} />
                    <b className={`text-sm font-black ${heading}`}>نتیجه شمارش</b>
                  </div>
                  <div className="overflow-x-auto cs-scroll">
                    <table className="w-full text-xs">
                      <thead>
                        <tr className={`border-b ${dk ? "border-slate-700" : "border-slate-200"}`}>
                          ...
                          <th className="px-3 py-2 text-right font-black text-slate-400">موجودی سیستم</th>
                          <th className="px-3 py-2 text-right font-black text-slate-400">موجودی واقعی</th>
                          <th className="px-3 py-2 text-right font-black text-slate-400">اختلاف</th>
                          <th className="px-3 py-2 text-right font-black text-slate-400">وضعیت</th>
                        </tr>
                      </thead>
                      <tbody>
                        {currencies.map(cur => {
          ...
          <div className={`cs-up w-full max-w-lg overflow-hidden rounded-xl md:rounded-2xl border shadow-2xl ${dk ? "border-slate-600 bg-slate-900" : "border-slate-200 bg-white"}`} onClick={(e) => e.stopPropagation()}>
            <div className={`flex items-center justify-between border-b px-4 md:px-5 py-3 md:py-4 ${dk ? "border-slate-700 bg-slate-800/60" : "border-slate-100 bg-slate-50"}`}>
              <b className={`flex items-center gap-2 text-sm ${dk ? "text-slate-100" : "text-slate-800"}`}>
                <span className={`grid h-8 w-8 place-items-center rounded-lg ${dk ? "bg-emerald-400/10 text-emerald-300" : "bg-emerald-100 text-emerald-600"}`}><Ic n="doc" className="h-4 w-4" /></span>
                تأیید عملیات صندوق
              </b>
