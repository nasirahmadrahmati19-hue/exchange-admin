...
const defaults: Ticket[] = [
  { id: 1, user: "علی محمدی", subject: "مشکل در برداشت", date: "۱۴۵/۰۵/۱۲", status: "باز", reply: "" },
  { id: 2, user: "سارا احمدی", subject: "تغییر رمز عبور", date: "۱۴۰/۰۵/۱۰", status: "بسته", reply: "رمز شما بازنشانی شد." },
];

export default function TicketsPage() {
  const [tickets, setTickets] = useState<Ticket[]>(defaults);
  const [replyId, setReplyId] = useState<number | null>(null);
  const [text, setText] = useState("");

  useEffect(() => { const s = localStorage.getItem("db_tickets"); if (s) setTickets(JSON.parse(s)); }, []);
  useEffect(() => { localStorage.setItem("db_tickets", JSON.stringify(tickets)); }, [tickets]);

  const send = () => {
    if (!text.trim() || !replyId) return;
          ...
          <div key={t.id} className="card p-5">
            <div className="flex flex-wrap items-center justify-between gap-3">
              ...
              <div className="flex items-center gap-2">
                ...
                <button className="btn-gold !py-1.5 !px-4 text-xs" onClick={() => { setReplyId(t.id); setText(t.reply); }}>پاسخ</button>
              </div>
            </div>
      ...
      {replyId && (
        <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4">
          <div className="card w-full max-w-md p-6 fade-up">
            ...
            <textarea className="input min-h-[120px]" value={text} onChange={e => setText(e.target.value)} placeholder="متن پاسخ..." />
            <div className="flex gap-2 mt-4">
              <button className="btn-gold flex-1" onClick={send}>ارسال و بستن تیکت</button>
              <button className="flex-1 rounded-xl border border-slate-200 text-sm font-bold hover:bg-slate-50" onClick={() => setReplyId(null)}>انصراف</button>
            </div>
          </div>
        </div>
      )}
